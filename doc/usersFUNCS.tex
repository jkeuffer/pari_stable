...
For the time being only curve with multiplicative reduction (split or
non-split) are supported, i.e. $v_p(j) < 0$, for $p\neq 2$. In this
case the curve is analytically isomorphic to $\bar{\Q}_p^*/q^\Z :=
E_q(\bar{\Q}_p)$, for some $p$-adic integer $q$ (Tate's theorem). In
...
\item \tet{tate} returns $[u^2,u,q,w]$ in the notation of Henniart \& Mestre
...
$\phi$. Finally, $w^2 = 12e_1^2 + 2b_2e_1 + 2b_4$. % FIXME: remove w
