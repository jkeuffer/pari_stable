...
library (often, variants are provided corresponding to the various flag
...
\kbd{NULL} pointer, or -1 for a variable number.
...
true, 0 if it is false. For the purpose of comparison, \typ{STR} objects are
...
\teb{issquarefree}. Note that it relies on a (fairly strong) probabilistic
primality test, see \kbd{ispseudoprime}.
...
factored form, which is a convenient way to avoid coefficient explosion.
To recover the conventional expanded form, try \tet{nffactorback}.
...
matrix in HNF, whose column express generators of $H$ on the given generators
...
always expressed as column vectors on the integral basis \kbd{rnf.nf.zk}.
...
\item A \tev{pseudo-matrix} is a pair $(A,I)$ where $A$ is a relative matrix
...
$\Z_K$-module, the \tev{projective module} generated by $(A,I)$ is the
...
\var{nrpid}, you must give some values as well for $c$ and $c_2$, and provide
...
\bprog
  GEN (*eval)(GEN,void*)
  void *E;  \\ context: eval(x, E) must evaluate your function at x.
@eprog\noindent
...
origin being at the upper left and the $y$-coordinates going downwards.
   ...
   Plotting functions are platform independant, but a number of graphical
