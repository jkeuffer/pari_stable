...
An elliptic curve is given by a Weierstrass model\sidx{Weierstrass
equation}
$$
...
such that $\tau=\dfrac{\omega_1}{\omega_2}$ belongs to Poincar\'e's
...
roots $e_1$ of the right hand side $g(x)$ of the associated $b$-model $Y^2
...
is such that $\phi^* dx/(2y + a_1x+a3) = u dt/t$, where $\phi: E(q)\to E$
...
Functions relative to height computations (\kbd{ellheight}, \kbd{ellbil} and
\kbd{ellheighmatrix}) require that the curve be in \emph{minimal} Weierstrass
form: this is achieved by the function \kbd{ellminimalmodel}. Using a
non-minimal model in such a routine will raise an exception. Even if other
functions do not require a minimal model, it is a good idea to stick
to minimal models, whenever $\gcd(c_4,c_6)$ is easy to factor (otherwise
\kbd{ellminimalmodel} will be slow). The
construction
\bprog
