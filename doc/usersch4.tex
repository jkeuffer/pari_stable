...
\fun{void*}{gpmalloc}{size_t size} calls \kbd{malloc} to allocate
...
\fun{void*}{gprealloc}{void* ptr, size_t size} as \kbd{gpmalloc} but calls
\kbd{realloc} instead of \kbd{malloc}.

\fun{void*}{gpcalloc}{size_t size} as \kbd{gpmalloc}, setting the
memory to zero.

\fun{void}{gpfree}{void* ptr} calls \kbd{free} to liberate the memory space
pointed to by \kbd{ptr}, which must have been allocated by \kbd{malloc}
(\kbd{gpmalloc}) or \kbd{realloc} (\kbd{gprealloc}). The \kbd{SIGINT} signal
...
section 3.2). It can also happen that \kbd{y} is not large enough or does not
have the proper tree structure to receive the object \kbd{x}. For instance,
let \kbd{y} the zero integer with length equal to 2; then \kbd{y} is too
small to accommodate any non-zero \typ{INT}. In general common sense tells
...
\fun{long}{mod2BIL}{GEN x} returns the least significant word of $|x|$, still
assuming that $x\neq 0$.
...
\subsec{Output to screen or file}.\sidx{output}

General output functions return nothing but print a character string as a
side effect. Low level routines are available:
...
\noindent But the most versatile such function is

\fun{void}{pariprintf}{const char *fmt, ...}

\noindent where \kbd{fmt} is a character string similar to the one
...
format types (see \tet{printf}), you can use the \emph{length modifier}~\kbd{Z}
to specify that an argument is a \kbd{GEN}. For instance, the following are
valid conversions for a \kbd{GEN} argument
\bprog
    %Zs     @com convert to \kbd{char*} (will print an arbitrary \kbd{GEN})
    %Z.10s  @com convert to \kbd{char*}, truncated to 10 chars
    %Z.2f   @com convert to floating point format with 2 decimals
    %Z4d    @com convert to integer, field width at least 4
@eprog
For instance we can write:
\bprog
    pariprintf("x[%d] = %Zs is not invertible!\n", i, gel(x,i));
@eprog\noindent
Here \kbd{i} is an \kbd{int}, \kbd{x} a \kbd{GEN} which is not a leaf and
this would insert the value of its $i$-th \kbd{GEN} component: \kbd{gel(x,i)}.

Note that \kbd{stdout} is usually buffered; it may be desirable to explicitly
flush such buffers at times, using

\fun{void}{pariflush}{}: flush output stream.

\noindent A simple variant to \kbd{pariprintf} is

\fun{void}{output}{GEN x}, which prints \kbd{x}, followed by a newline and a
...
These functions write on (PARI's idea of) standard output. It is also possible
to write to an arbitrary \kbd{FILE*}:

\fun{void}{parifprintf}{FILE *file, const char *fmt, ...}

Note that we have \tet{va_list} analogs of the functions seen so far:

\fun{void}{parivprintf}{const char *fmt, va_list ap}

\fun{void}{parivfprintf}{FILE *file, const char *fmt, va_list ap}

\misctitle{Remark}. The \tet{pariprintf} output is done on the stream
\tet{pari_outfile}, which is initialized to \tet{stdout}. You can change PARI's idea of its standard output, redirecting it to another file, using
...
Similarly, errors are sent to the stream \tet{pari_errfile} (\tet{stderr} by
default), and input is done on the stream \tet{pari_infile}, which you can
change using

\fun{void}{switchin}{const char *name}, analogous to \tet{switchout} for
\var{in}-files.

\misctitle{Advanced remark}. All output is done according to the values of
the \teb{pariOut}~/ \teb{pariErr} global variables which are pointers to
structs of pointer to functions. If you intend to use these, this
probably means you are rewriting \kbd{gp}. In that case, have a look at the
code in \kbd{language/es.c} (\kbd{init80()} or \kbd{GENtostr()} come to mind).

\subsec{Output to string}.\sidx{output}

The following functions convert a PARI \kbd{GEN} to a \kbd{malloc}'ed ASCII
...
\fun{char}{*parisprintf}{const char *fmt, ...} produces a string from the
remaining arguments, according to the PARI format \kbd{fmt} (see \tet{printf}).

\fun{char}{*parivsprintf}{const char *fmt, va_list ap} the \tet{va_list}
analog.
...
operands, as in the \tet{pariprintf} function. For instance:
\bprog
  pari_err(talker, "x[%d] = %Zs is not invertible!", i, gel(x,i));
@eprog\noindent
...
functions, \tet{output} and \tet{pariprintf} mainly. Corresponding to the
...
\bprog
  ...
  for (i = 1; i < 10; i++) {
    ...
    msgTIMER(&T, "for i = %ld (L[i] = %Zs)", i, L[i]);
  }
@eprog
...
\fun{void}{pariprintf}{const char *fmt, ...}: write following arguments to the
...
\tet{push_lex} and \tet{pop_lex} functions). None of this is documented.
Check and adapt the source code in \kbd{language/sumiter.c} and
\kbd{language/intnum.c}.
