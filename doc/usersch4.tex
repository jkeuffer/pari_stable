...
\item \tet{forpart} is a variant of forvec which iterate over partitions.
See the documentation of the corresponding GP function for detail.
It is available in both forms: the prototype of the function with callback is

\fun{void}{forpart}{void *data, long (*call)(void*,GEN), long k, GEN a, GEN n}

On the other hand, the iterator is initialised by

\fun{void}{forpart_init}{forpart_t *T, long k, GEN a, GEN b}

the next partition being obtained by

\fun{GEN}{forpart_next}{forpart *T}
which returns \kbd{NULL} after the last partition is returned.

the previous partition being obtained by

\fun{GEN}{forpart_prev}{forpart *T}
which returns \kbd{NULL} after the last partition is returned.
