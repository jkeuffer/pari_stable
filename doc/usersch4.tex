...
one is used by GP for internal garbage collecting, you will not have to worry
about it.
...
being the constant term and so on. Note that the \idx{degree} of the
polynomial is equal to its length minus three. The function
...
that all the coefficients of the polynomial are equal to zero (as they should
for a zero polynomial), but not all of these zeros are exact zeros, and more
precisely the leading term \kbd{z[lg(z)-1]} is not an exact zero.
...
\noindent The next two types were introduced for specific GP use, and you
...
\kbdsidx{t_STR}\sidx{character string} \fun{char *}{GSTR}{z} (= \kbd{(z+1)})
points to the first character of the (\kbd{NULL}-terminated) string.
...
Under GP, one need not worry too much since the interpreter will define the
...
variable in library mode and cannot be input under GP. No documented library
...
Def} in the directory \kbd{desc}. Then, you can recompile GP.
...
@eprog
\noindent which means we have a function \kbd{ClassGroupInit} under GP, which
