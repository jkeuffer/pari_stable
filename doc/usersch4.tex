...
$\bullet$ a PARI \tev{stack} containing nothing. This is a big
...
word level (\kbd{long}s and \kbd{ulong}s), Level 1 with \typ{INT} and \typ{REAL}
...
\fun{GEN}{Fp_add}{GEN x, GEN y, GEN m}: returns the sum of $x$ and $y$
modulo $m$. Here $x, y, m$ are \typ{INT}s (this is not checked). The operation
...
\kbd{addii} on a \typ{FRAC} argument will cause disasters. Type mismatches may
...
exact as possible, enforced for generic functions like \kbd{gadd} or \kbd{gmul},
is dropped in kernel routines of Level $1$, where it is replaced by the much
simpler rule: the result is a \typ{INT} if and only if all arguments
are integer types (\typ{INT} but also C \kbd{long} and \kbd{ulong}) and a
\typ{REAL} otherwise. For instance, multiplying a \typ{REAL} by a \typ{INT}
always yields a \typ{REAL} if you use \kbd{mulir}, where \kbd{gmul} returns the
\typ{INT} \kbd{gen\_0} if the integer is $0$.
...
\fun{void}{pari_free}{void* ptr} calls \kbd{free} to liberate the memory space
pointed to by \kbd{ptr}, which must have been allocated by \kbd{malloc}
(\tet{pari_malloc}) or \kbd{realloc} (\tet{pari_realloc}). The \kbd{SIGINT} signal
is blocked until \kbd{free} returns.
...
non-zero \typ{INT}. In general common sense tells you what is possible, keeping in
...
is not restricted to \typ{REAL}s, it also applies for $p$-adics for instance.)
...
\kbd{Mod(0,2)}) is possible for constant polynomials, and an inexact $0$
(like \kbd{0.E-28}) is always possible. (The reason for this is that
...
leading term \kbd{z[lg(z)-1]} is not an exact integer zero. More precisely,
\kbd{gequal0(x)} is true for all coefficients \kbd{x} of the polynomial,
and \kbd{isrationalzero(x)} is false for the leading coefficient. The same
...
library mode and subject to change each time the GP language evolves. Hence we
...
specifications. As long as this is in effect, the following strings chars is
rendered in color, possibly in bold or underlined (see \secref{se:colors}).
...
The argument \kbd{s} is either \kbd{NULL} (string allocated on the PARI
stack), or preallocated storage (in which case, it must be able to hold
at least 16 chars, including the final \kbd{\bs 0}).

\subsec{Output contexts}

The behavior of the functions \tet{pari_putc}, \tet{pari_puts},
\tet{pari_flush} and \tet{pari_printf} can be customized by modifying
the output contexts used for printing, which are \tet{pariOut} for
normal output and \tet{pariErr} for error messages.  \tet{pariOut} and
\tet{pariErr} are pointers to a \tet{PariOUT}, which is a \kbd{struct}
...
@eprog\noindent
The functions \tet{putch} and \tet{puts} are used to print a character
or a string respectively.  The function \tet{flush} is called to finalize a
messages.

By default, we have \kbd{pariOut = defaultOut}, which writes output to a
\kbd{FILE*}, \tet{pari_outfile}, initialized to \tet{stdout}.  The
low-level printing functions of \kbd{defaultOut} are actually the
standard \kbd{putc} / \kbd{fputs}, plus some magic to handle a log file
if one is open.  Similarly, the default for \kbd{pariErr} is
\tet{defaultErr}, which prints to \tet{stderr}.  If you intend to change
\kbd{pariOut} or \kbd{pariErr}, diverting output in non-trivial ways,
this probably means you are rewriting \kbd{gp}. In that case, have a
look at the code in \kbd{language/es.c} (\kbd{pariOut80} comes to mind).

Still keeping the default output context \kbd{defaultOut}, you can change
PARI's standard output, redirecting \tet{pari_outfile} to another file, using
...
Similarly, by default, \kbd{pariErr = defaultErr} and errors are sent to the
stream \tet{pari_errfile} (\tet{stderr} by default). Finally, input is read
from the stream \tet{pari_infile}, which you can change using

\fun{void}{switchin}{const char *name}, analogous to \tet{switchout} for
\var{in}-files.
...
\fun{void}{dbg_block}{} Behave as if \kbd{DEBUGLEVEL = 0}, effectively
blocking diagnostics linked to \kbd{DEBUGLEVEL}.

\fun{void}{dbg_release}{} Stop blocking diagnostics.

This pair is useful when your code uses high level functions like
\kbd{bnfinit} which produce a lot of diagnostics even at low
\kbd{DEBUGLEVEL}s. You may want to brace the corresponding function calls
with a \kbd{dbg\_block()} and \kbd{dbg\_release()} pair to suppress those.
...
\fun{void}{TIMERstart}{pari_timer *T}

\fun{long}{TIMER}{pari_timer *T}

\fun{long}{msgTIMER}{pari_timer *T, char *format,...}
...
\bprog
  ...
  for (i = 1; i < 10; i++) {
    ...
    msgTIMER(&T, "for i = %ld (L[i] = %Ps)", i, L[i]);
  }
@eprog
...
\item \tet{forell} is not yet provided with a usable public interface, but
the code should be straightforward to copy and adapt to your needs.

\item \tet{forsubgroup} is currently not implemented as an iterator but as a
 traversal procedure with callbacks.

\fun{void}{traversesubgroups}{GEN G, GEN B, void fun(GEN, void*), void *E}
goes through the same subgroups as \tet{forsubgroup(H = G, B,)}, calling
\tet{fun(H, E)} for each subgroup $H$.
...
\fun{GEN}{forvec_start}{GEN v, long flag, GEN *D, GEN (**next)(GEN,GEN)}
where $D$ (state vector) and \kbd{next} (iterator function, depends
on \kbd{flag} and the types of the bounds in $v[i]$) are set by the call.
This returns the first element in the \kbd{forvec} sequence, or \kbd{NULL} if
...
simple direct backtracking algorithm using $C$ \kbd{long}s.

\subsec{Iterating over primes}

After \kbd{pari\_init(size, maxprime)} is called, a ``prime table'' is
...
choosing \kbd{maxprime}; it is $500000$ by default under \kbd{gp}. In any
case, the implementation requires that $\tet{maxprime} < 2^{\B} - 2048$,
whatever memory is available. If you need more primes, use \tet{nextprime}.
...
\tet{NEXT_PRIME_VIADIFF} operates in the same way, but will omit that check,
...
you must free the (\kbd{malloc}ed) precomputed table first; and then all
...
assume \kbd{*ptr} is inside of a \kbd{diffptr} containing the successive
...
\fun{long}{TIMERread}{pari_timer *T} returns the time elapsed since the timer
was started (or last reset), without resetting it.
