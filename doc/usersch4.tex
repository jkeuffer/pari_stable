...
To make the following descriptions valid both for 32-bit and 64-bit machines,
we will set \B\ to be equal to 32 (resp.~64), an abbreviation of
...
is presented in Appendix~B, and a more elaborate one in
\kbd{examples/Makefile}.
...
warnings from the compiler. This is sometimes tedious, but of course is
trivially done. See the discussion on typecasts in the next section.
...
particular the PARI stack which is both a scratchboard and a repository
...
numbers, just put $\tet{maxprime} = 2$. Be careful: lots a PARI functions
need this table, certainly all the ones of interest to number theorists. If
you wind up with the error message ``not enough precomputed primes'', do
increase this value.

\noindent We have now at our disposal:

$\bullet$ a PARI \tev{stack} containing nothing. It is a big
connected chunk of \kbd{size} bytes of memory. All your computations are
going to take place here. When doing large computations, unwanted
intermediate results clutter up memory very fast so some kind of garbage
collecting is needed. Most large systems do garbage collecting when the
memory is getting scarce, and this slows down the performance. PARI
takes a different approach: you must do your own cleaning up when the
intermediate results are not needed anymore. Special purpose routines have
been written to do this; we will see later how (and when) you should use
them.
...
compiler's point of view, many typecasts are necessary when programming
in PARI. To take an example, a vector $V$ of dimension 2 (two components)
is represented by a chunk of memory pointed to by the \kbd{GEN}~\kbd{V}.
...
is where typecasting is necessary: a priori, \kbd{V[i]} (for
$\kbd{i}=1,2$) is a \kbd{long}, but we want to use it as a \kbd{GEN}.
The following two constructions are exceedingly frequent (\kbd{x} and
\kbd{V} are \kbd{GEN}s):
%
\bprog
  V[i] = (long) x;
...
function typecasts the result to \kbd{long}. Note that \kbd{ldiv} is an ANSI C
...
in GP when \kbd{x} is a matrix. This is a purely syntactical trick
...
is actually equivalent to \kbd{mael(y,x)}. It is suggested that
...
objects (of type \kbd{GEN}), and \emph{creates} the result $x+y$ on the PARI
...
once you know that {\bf i} stands for a \typ{INT}, {\bf r} for a \typ{REAL},
{\bf mp} for i or r, {\bf s} for an ordinary signed long, whereas {\bf z} (as
a suffix) means that the result is not created on the PARI stack but assigned
to a preexisting GEN object passed as an extra argument.

For completeness, Chapter 5 gives a description of these low-level functions.
...
want your programs to run identically on 32-bit and 64-bit machines, you 
...
objects are not needed anymore. This is called ``\emph{garbage collecting}.''
...
screams an error message at you (``significant pointers lost in gerepile'').
...
\kbd{gerepile}: every pointer you may have kept around elsewhere, which
previously pointed into the zone below \kbd{ltop} must be discarded. If you
need to recover more than one object, use one of the \kbd{gerepilemany}
functions below.
...
This is because the \kbd{gerepile} function knows only about \emph{two
connected zones}: the garbage that will be erased (between \kbd{lbot} and
\kbd{ltop}) and the significant pointers that are copied and updated.
If there is garbage interspersed with your objects, disasters occurs when
we try to update them and consider the corresponding ``pointers''. In most
cases of course the said garbage is in fact a bunch of other \kbd{GEN}s, in
which case we simply waste time copying and updating them for nothing.
So be wary when you allow objects to become disconnected.
...
belong to the garbage zone! Documented PARI functions guarantee this.
...
above example, note that \emph{if} we know that x and y are of type real and
of length less than or equal to 5, we can program without using
\kbd{gerepile} at all:
\bprog
  z = cgetr(5); ltop = avma;
...
\bprog
  ...
  z = cgetg(3,t_VEC);
  z[1] = ladd(p1,y);
  z[2] = ladd(p3,x); z = gerepile(ltop,lbot,z);
@eprog\noindent Finishing by \kbd{z = gerepileupto(ltop, z)} would be ok as
well. But when you have the choice, it is usually clearer to brace the
garbage between \kbd{ltop}~/ \kbd{lbot} pairs. Beware that
\bprog
  ltop = avma; p1 = gadd(gsqr(x), y); p3 = gadd(gsqr(y), x);
  z = cgetg(3,t_VEC);
  z[1] = (long)p1;
  z[2] = (long)p3; z = gerepileupto(ltop,z); /*@Ccom ! WRONG ! */
@eprog\noindent
is a disaster since \kbd{p1} and \kbd{p3} are created before
\kbd{z}, so the call to \kbd{gerepileupto} overwrites them, leaving
\kbd{z[1]} and \kbd{z[2]} pointing at random data! On the other hand
\bprog
  ltop = avma; z = cgetg(3,t_VEC);
  z[1] = ladd(gsqr(x), y);
  z[2] = ladd(gsqr(y), x); z = gerepileupto(ltop,z); /*@Ccom INEFFICIENT */
@eprog\noindent
leaves the results of \kbd{gsqr(x)} and \kbd{gsqr(y)} on the stack (and
updates them for naught).
...
p3 = gmul(gadd((GEN)x[1], (GEN)x[2]), gadd((GEN)y[1], (GEN)y[2]));
p4 = gadd(p1,p2); lbot = avma;
z = cgetg(3,t_COMPLEX);
z[1] = lsub(p1,p2);
z[2] = lsub(p3,p4); z = gerepile(ltop,lbot,z);
@eprog

Let us now look at an example where more than one \kbd{gerepile}
is needed in practice. (At the expense of efficiency, one can always use only
one using \kbd{gerepilecopy}.) Suppose that we want to write a function
which multiplies a matrix by a column vector. Such a function is of course
already part of \kbd{gmul}, but let us ignore this for a moment. Then the
...
We leave the details to the reader, who can look at the answer in
\kbd{src/basemath/gen1.c:MC\_mul()}.

\subsubsec{gerepilemany}

Let us now see why we may need the \kbd{gerepilemany} variants. Although it
...
address of \kbd{z2}. This is where we need one of the \idx{gerepilemany}
...
\kbd{avma}, and updates the pointers \kbd{z1} and \kbd{z2}. An equivalent
formulation is to simply write
\bprog
  gerepileall(ltop, 2, &z1, &z2)
@eprog\noindent
so that the array \kbd{gptr} is in fact not needed. Here we do not assume
anything about the stack: the garbage can be disconnected and \kbd{z1},
\kbd{z2} need not be at the bottom of the stack. If all of these assumptions
are in fact satisfied, then we can call \kbd{gerepilemanysp} instead, which
is usually faster since we do not need the initial copy (on the other hand,
...
First, \kbd{gerepile} has turned out to be a very flexible and fast garbage
collector for number-theoretic computations, which compares favorably with
much more sophisticated methods used in other systems. Our benchmarks
indicate that the price paid for using \kbd{gerepile} (and
\kbd{gerepile}-related copies), when properly used, is usually less than 1
percent of the total running time, which is quite acceptable!

Second, it is of course harder on the prorammer, and quite error-prone
if you don't stick to a consistent PARI programming style. If all seems lost,
just use \tet{gerepilecopy} (or \tet{gerepileall}) to fix up the stack for
you. You can always optimize later when you've sorted out exactly which
...
\noindent 3) The macros \teb{lgetg}, \teb{lgeti}, \teb{lgetr} are predefined
as \kbd{(long)cgetg}, \kbd{(long)cgeti}, \kbd{(long)cgetr}, respectively.

\noindent 4) Finally, there are two low-level routines to allocate space on
the PARI stack:

\fun{GEN}{new_chunk}{size\_t n}

allocates a \kbd{GEN} with $n$ components, \emph{without} filling the
required code words. This is the low-level constructor underlying
\kbd{cgetg}, which calls \kbd{new\_chunk}, then sets the first code word. It
works by simply returning the address \kbd{((GEN)avma) - n}, after checking
that it is larger than \kbd{(GEN)bot}.

\fun{char*}{stackmalloc}{size\_t n}

allocates memory on the stack for $n$ chars (\emph{not} $n$ \kbd{GEN}s). This
is faster than using \kbd{malloc}, and easier to use in most situations when
temporary storage is needed. In particular there is no need to \kbd{free}
individually all variables thus allocated: a simple \kbd{avma = oldavma}
might be enough. On the other hand, beware that this is not permanent
independant storage, but part of the PARI stack.

Note that objects allocated through these last two functions cannot be
...
\fun{GEN}{coefs_to_vec}{long n, ...}
returns the \typ{VEC} whose $n$ coefficients (\kbd{GEN}) follow.

\fun{GEN}{coefs_to_col}{long n, ...}
...
\bprog
  ...
  z = coefs_to_vec(3, stoi(3), gzero, gdeux);
...
a bad idea to use \tet{gaffect} statements. There are two exceptions:
...
minimum (otherwise the precision of \kbd{x} is used in all subsequent
...
Make sure at this point that you understand the difference between
\kbd{y = x}, \kbd{y = gcopy(x)}, and \kbd{gaffect(x,y)}.

\subsec{Clones}.\sidx{clone}\label{se:clone}
Sometimes, it may be more efficient to create a \emph{permanent} copy of a
PARI object. This is not created on the stack but on the heap. The function
which does this is called \teb{gclone}, with the predefined macro
...
\noindent
We now go through each type and explain its
...
one hand, \kbd{z} is treated as the C pointer it is (in the context of
...
is nonzero (in which case the \emph{pointer}~\kbd{z} is certainly
...
\misctitle{Remark} The clone bit is there so that \kbd{gunclone} can check it
is deleting an object which was allocated by \kbd{gclone}. Miscellaneous
...
is accepted by your compiler, without your having to explicitly type
...
\subsec{Type \typ{INTMOD} (intmod):}\kbdsidx{t_INTMOD}\sidx{intmod}
...
intmods resulting from operations can point at this common object,
...
must be of \typ{INT}s and $\kbd{z[2]} > 0$.
...
for intmods, \kbd{z[1]} points to the modulus, and \kbd{z[2]} to a
...
other hand a variable of \emph{greater} priority is not acceptable;
...
being the constant term and so on. For an object of type \typ{POL},
the macros \tet{leading_term}, \tet{constant_term}, \tet{degpol}
return a pointer to the leading term (with respect to the main variable of
course), constant term, and degree of the polynomial (with the convention
$\deg(0) = -1$). Applied to any other type, the result is unspecificied. Note
that no copy is made on the pari stack so the returned value is not safe for
a basic \kbd{gerepile} call. Note that $\kbd{degpol(z)} = \kbd{lg(z)} - 3$.
...
leading term \kbd{z[lg(z)-1]} is not an exact zero.
...
\kbdsidx{t_RFRAC}\sidx{rational function} \kbd{z[1]} points to the numerator,
and \kbd{z[2]} on the denominator. The denominator must be of type \typ{POL},
...
\kbd{z[3]} point to the three coefficients of the form. All three are
of type \typ{INT}.
...
polynomial arithmetic and linear alebra over small finite fields, etc.
...
The way an object is considered in formal computations depends entirely
...
doing it; see \secref{se:priority} for a discussion of possible problems in a
similar situation.
...
variable number~$v$. Here is the complete list: \teb{polun}$[v]$ and
...
\bprog
GEN gy = flisexpr("'y"); /*@Ccom returns polx[$v$], for some $v$ */
long v = varn(gy);
@eprog\noindent
But please note the quote \kbd{'y} in the above. Using \kbd{flisexpr("y")}
might work, but is dangerous, especially when programming functions that are
used under \kbd{gp}. It would read the value of \kbd{y}, as currently known
by the \kbd{gp} interpreter (possibly creating it in the process). But if
\kbd{y} has been modified by previous \kbd{gp} commands (e.g \kbd {y = 1}),
then the value of \kbd{gy} is not what you expected it to be and corresponds
instead to the current value of the \kbd{gp} variable (e.g \kbd{gun}).
...
\kbd{polun[v]} and \kbd{polx[v]}. The variables created in this way have no
...
under \kbd{gp}. These functions are called \teb{flisexpr} and \teb{flisseq}. The
...
comments in the input string (that is what the initial \kbd{f} stands for).
...
returns the resulting \kbd{GEN}. This way, you do not have to worry about
...
message. For instance, \kbd{err(typeer,"matexp")} prints the message:

\kbd{ ***   incorrect type in matexp.}

\noindent
Among the codewords are \emph{warning} keywords (all those which start with
the prefix \kbd{warn}). In that case, \teb{err} does \emph{not} abort the
...
use the library mode is to work through a detailed example. We will write a
program which computes the exponential of a square matrix~$x$.  The complete
listing is given in Appendix~B, but each part of the program will be produced
and explained here. We will use an algorithm which is not optimal but is not
far from the one used for the PARI function \teb{gexp} (in fact embodied in
the function \kbd{mpexp1}). This consists in calculating the sum of the
series:
$$e^{x/(2^n)}=\sum_{k=0}^\infty \dfrac{(x/(2^n))^k}{k!}$$
for a suitable positive integer $n$, and then computing $e^x$ by repeated
squarings.  First, we need to compute the $L^2$-norm of the matrix~$x$,
i.e.~the quantity:
$$z=\|x\|_2=\sqrt{\sum |x_{i,j}|^2}.$$
We then choose the integer $n$ such that the $L^2$-norm of $x/(2^n)$ is
less than or equal to~1, i.e.
$$ n = \left\lceil{\ln(z)}\big/{\ln(2)}\right\rceil $$
if $z\ge1$, and~$n=0$ otherwise. Then the series converges at least as
fast as the usual one for $e^1$, and the cutoff error is easy to estimate. In
fact a larger value of $n$ would be preferable, but this is slightly machine
dependent and more complicated, and is left to the reader.
...
caters for input and output, and a function which we shall call \kbd{matexp}
which does the real work. The main program can be something like this:
\bprog
#include <pari.h>
...
are equally valid inputs). Finally, \kbd{outbeaut} is an output
routine. Now let us attack the main course, the function \kbd{matexp}:
%
\bprog
GEN
...
stack, we save the value of the stack pointer \kbd{avma} in \kbd{ltop}. Note
that we are going to assume throughout that the garbage does not overflow the
currently available stack. If it ever did, we would have two options~---
allocate a larger stack in the main program (for instance change 1000000 into
2000000) or do some \kbd{gerepile}ing along the way.

Secondly, the \teb{err} function is the general error handler for the
PARI library. This aborts the program after printing the required message.

Thirdly, notice how we handle the special case $\kbd{lx} = 1$ (empty matrix)
\emph{before} accessing \kbd{lg(x[1])}. Doing it the other way round could
produce a fatal error. Indeed, if \kbd{x} is of length 1, then \kbd{x[1]} is
not a component of \kbd{x}. It is just the contents of the memory cell which
happens to follow the one pointed to by \kbd{x}, and thus has no reason to be
a valid \kbd{GEN}. Now recall that none of the codeword handling macros do
any kind of type checking (see \secref{se:impl}), thus \teb{lg} would
consider \kbd{x[1]} as a valid address, and try to access \kbd{*((GEN)x[1])}
(the first codeword) which is unlikely to be a valid memory address.
...
have needed a double loop. In fact, \kbd{gnorml2} works as desired on
matrices, so we really did not need this loop at all: \kbd{s = gnorml2(x)}
...
we would have added the instruction \kbd{s = greal(s);} just after the
\kbd{for} loop.
...
In the fifth place, it is unnecessary to take the square root at all since we
only want to test whether the norm exceeds~$1$. So the statement \kbd{s =
gsqrt(s,3)} could be removed. But it has the virtue that \kbd{s} is then
guaranteed to be a \typ{REAL}. Namely, in the above snippet, we determine the
approximate size of a real number by looking at its binary exponent. Hence we
need to have \kbd{s} actually represented as a \typ{REAL}, and not as a
\typ{INT} or a \typ{FRAC}. The result of transcendental functions is
guaranteed to be of type \typ{REAL}, or complex with \typ{REAL} components,
thus this is indeed the case after the call to \kbd{gsqrt} since its argument
is a nonnegative (real) number.

In the sixth place, and this will solve the problem above plus another one we
had not mentioned yet, note that we initialized the sum \kbd{s} to
\teb{gzero}, which is an exact zero. This has many disadvantages: if all the
entries of the matrix are integers (or worse rational numbers), the
computation will take rather long, about twice as long as with real numbers
of the same length. Since computing an $L^2$ norm is a perfectly stable
operation, there is no need for exact computations or for high precision. It
is better to initialize \kbd{s} to a real zero, using for instance
\bprog
  s = realzero(3);
@eprog
\fun{GEN}{realzero}{long prec} returns a real zero to accuracy
$-\tet{bit_accuracy}(\var{prec})$.
...
\noindent and the effect is to multiply \kbd{x} by $2^{\text n}$,
...
\fun{GEN}{realun}{long prec}, which returns a real $1$ with \var{prec} words
of accuracy, but chose to give an example of assignment used as a conversion
device.

To sum the series, we use three variables: a variable \kbd{p2} which at
stage $k$ contains $\kbd{p1}^k/k!$, a variable \kbd{y} which contains
$\sum_{i=0}^k \kbd{p1}^i/i!$, and a variable \kbd{r} which contains the size
estimate $\kbd{s}^k/k!$. Note that we do not use Horner's rule. This is
because we are lazy and do not want to compute in advance the number of terms
that we need. We leave this modification (and many other improvements!) to
the reader. The program continues as follows:
...
including the codeword is in fact \kbd{m+1}.  There is a corresponding
...
use of \teb{gerepile}, we keep the value of \kbd{avma} in \kbd{lbot} just
\emph{before} the squaring, so that if it is the last one, \kbd{lbot} is
indeed the bottom address of the garbage pile, and \kbd{gerepile}
works. Note that it takes a completely negligible time to do this in each loop
compared to a matrix squaring.  However, when \kbd{n} is initially equal to
...
copying \kbd{y} again to the top of the stack. This is inefficient, but does
the trick. If we wanted to avoid this using only \kbd{gerepile}, the best
thing to do would be to put the instruction \kbd{lbot=avma} just before both
occurrences of the instruction \kbd{y=gadd(p2,y)}. Better, we may rewrite the
...
is~0). It is safe to use \kbd{gerepileupto} here as \kbd{y} has been
...
checks have been omitted, the efficiency is far from optimal, and we could
have used a number of more specialized library routines. An evident
...
\teb{entree} type, which is not documented. Check the comment before the
function list at the end of \kbd{language/init.c} and the source code in
\kbd{language/sumiter.c}. You should be able to make something of it.
\smallskip
...
returns a \kbd{GEN} and you want it to be accessible to \kbd{gp}, you have to
...
\fun{void}{pariputs}{char *s}: write \kbd{s} to the \kbd{gp} output stream.

\fun{void}{fprintferr}{char *s}: write \kbd{s} to the \kbd{gp} error stream (this
...
Read that file from your \kbd{gp} session (from your \idx{preferences file} for
instance, see \secref{se:gprc}), and that's it, you can use the new
function \var{gpname} under \kbd{gp} (and we would very much like to hear about
it!).

\subsec{Integration the hard way}

If \tet{install} is not available for your Operating System, things are more
complicated: you have to hardcode your function in the \kbd{gp} binary (or
install \idx{Linux}). Here is what needs to be done:
...
start by a whitespace character.) At this point you can recompile \kbd{gp},
