...
\fun{void*}{pari_realloc}{void* ptr, size_t size} as \kbd{pari_malloc} but
calls \kbd{realloc} instead of \kbd{malloc}.

\fun{void*}{pari_calloc}{size_t size} as \kbd{pari_malloc}, setting the
memory to zero.
...
(\kbd{pari_malloc}) or \kbd{realloc} (\kbd{pari_realloc}). The \kbd{SIGINT} signal
...
Simple variants of \kbd{pari\_sprintf} convert a \kbd{GEN} to a
...
By default, we have \kbd{pariOut = defaultOut}, the output ``stream'' is an
actual \kbd{FILE*}, \tet{pari_outfile}, initialized to \tet{stdout}, and
the low-level printing functions are actually the standard \kbd{putc} /
\kbd{puts}, plus some magic to handle a logfile if one is open. If you intend
to change \kbd{pariOut}, diverting output in non-trivial ways, this probably
means you are rewriting \kbd{gp}. In that case, have a look at the code in
\kbd{language/es.c} (\kbd{pariOut80} comes to mind).
