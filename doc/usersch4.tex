...
\bprog
  ...
  z = cgetg(3,t_VEC); z[1] = (long)p2; z[2] = (long)p4;
@eprog
\noindent
...
\bprog
  ...
  z = cgetg(3,t_VEC); z[1] = ladd(p1,y); z[2] = ladd(p3,x);
...
\bprog
  ...
  z = cgetg(3,t_VEC); z[1] = (long)p1; z[2] = (long)p3
...
z = cgetg(3,t_COMPLEX); z[1] = lsub(p1,p2); z[2] = lsub(p3,p4);
z = gerepile(ltop,lbot,z);
@eprog
\noindent
...
type integer. In principle \kbd{z[1]} $>$ 0 and 0 $\le$ \kbd{z[2]} $<$
\kbd{z[1]}, but this rule does not have to be strictly obeyed by the user.
Any integermod obtained as the result of a PARI function call will satisfy
these conditions.
...
\subsec{Type \typ{PADIC} ($p$-adic numbers):}\sidx{p-adic
number}\kbdsidx{t_PADIC} this type has a second codeword \kbd{[1]} which
contains the following information: the $p$-adic precision (the exponent of
$p$ modulo which the $p$-adic unit corresponding to \kbd{z} is defined if
\kbd{z} is not~0), i.e.~one less than the number of significant $p$-adic
digits, and the biased exponent of \kbd{z} (the bias being equal to
\kbd{HIGHVALPBIT} here). This information can be handled using the
following functions:
...
polynomial. However, one must obey the rules explained in Chapter 2
concerning the hierarchical structure of the variables of a polymod.
...
\kbd{z[1]} points to the numerator, and \kbd{z[2]} on the denominator. Both
must be of type polynomial. Note that a type \typ{RFRACN} rational function can
be converted to irreducible form using the function \teb{gred}.
...
accuracy $-\idx{bit_accuracy}\var{prec}$.
