...
\subsec{Output}.\sidx{output}

\noindent
Output functions come in three groups: functions returning a
\kbd{char*}, functions returning nothing but printing a character string as a
side effect, and display an object internal structure for debugging.

\noindent $\bullet$ In the first group, we convert a PARI \kbd{GEN} to
a \kbd{malloc}'ed ASCII string, which you should \kbd{free} after use.
...
\noindent $\bullet$ In the second group, the simplest function is

\fun{void}{output}{GEN x}, which prints \kbd{x}, followed by a newline and a
buffer flush.

\noindent This is also quite useful during debugging: for instance, some PARI
developpers add the following \kbd{gdb} macro to their \kbd{.gdbinit}:
\bprog
  define i
    call output((GEN)$arg0)
  end
@eprog\noindent
then typing \kbd{i obj} prints the \kbd{GEN} \kbd{obj} (provided the
optimizer has not put it into a register, but it is rarely a good idea to
debug optimized code).

\misctitle{Remark}. All such output is done on the stream \teb{outfile},
which is initialized to \teb{stdout}. To direct your output to another file,
use 
...
Similarly, errors are sent to the stream \teb{errfile} (\teb{stderr} by
default), and input is done on the stream \teb{infile}, which you can change
using the function \teb{switchin} which is analogous to \teb{switchout}.

\noindent $\bullet$ The third and final one corresponds to the \kbd{gp}
metacommand \kbd{\b x} and outputs the \idx{hexadecimal tree} associated to
an object. This last type of output is only used for debugging purposes.
...
code in \kbd{language/es.c} (\kbd{init80()} or \kbd{GENtostr()} for
instance).
