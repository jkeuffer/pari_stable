...
\fun{void}{pariprintf}{const char *format, ...}

\noindent where \kbd{format} is a character string similar to the one
\kbd{printf} uses. In there, \kbd{\%} characters have a special meaning, and
describe how to print the remaining operands to
\kbd{pariprintf}. In addition to the standard format types (see \kbd{man
printf}), you can use
\kbd{\%Z} to denote a \kbd{GEN} object\footnote{*}{We would have liked to
pick \kbd{\%G} but it was already in use!}. For instance we can write:
\bprog
    pariprintf("x[%d] = %Z is not invertible!\n", i, gel(x,i));
@eprog\noindent
...
\misctitle{Remark}. All such output is done on the stream \tet{pari_outfile},
which is initialized to \tet{stdout}. To direct your default output to
another file, use

\fun{void}{switchout}{char *name} where \kbd{name} is a character string
...
previous output stream, call \kbd{switchout(NULL)}.
...
\bprog
  pari_err(talker, "x[%d] = %Z is not invertible!", i, gel(x,i));
@eprog\noindent
