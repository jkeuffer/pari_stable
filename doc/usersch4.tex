...
\bprog%
  long ltop = avma, limit = stack\_lim(avma,1);
  \dots
\eprog
...
\bprog\obr
  long first = fetch\_var();
  long n1 = fetch\_var();
  long n2 = fetch\_var(); /* prepare three variables for internal use */
  ...
  /* delete all variables before leaving */
  do \obr\ num = delete\_var(); \cbr\ while (num \&\& num <= first);
\cbr\eprog
...
\obr
  ...
  GEN x;
\h
  /* take a stack of a 10^6 bytes, no prime table */
...
\h
  ...
  if (typ(x) != t\_MAT) err(talker,"this expression is not a matrix");
...
\h
  ...
  else \obr n = k+1; p1 = gmul2n(x,-n); setexpo(s,-1); \cbr\eprog
\noindent
...
\h
  ...
  if (!n) \obr\ lbot = avma; y = gcopy(y); \cbr
  else
  \obr
    for (i=0; i<n; i++) \obr\ lbot = avma; y = gsqr(y); \cbr
  \cbr
...
\obr
  ...
  addhelp(ClassGroupInit, "ClassGroupInit(P,\obr flag=0\cbr,\obr data=[]\cbr):
