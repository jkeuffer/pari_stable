...
\kbd{MAXVARN} should be enough for most cases, but you can create more
...
\kbd{*entree})\cr
\+& \kbd{I} & string containing GP code (useful for control statements, %
 to be processed with \kbd{lisexpr}\cr
\+&&\quad or \kbd{lisseq})\cr
...
\cbr
\eprog
which means we have a function \kbd{ClassGroupInit} under GP, which calls the
library function \kbd{bnfinit0} . The function has one mandatory
argument (\kbd{V} is non-zero), and possibly two more (two \kbd{'D'} in the
code), plus the current real precision. More precisely, the first argument
is a \kbd{GEN}, the second one is converted to a \kbd{long} using
\kbd{itos} (\kbd{0} is passed if it is omitted), and the third one is also
a \kbd{GEN}, but we pass \kbd{NULL} if no argument was supplied by the
user. This matches the C prototype (from \kbd{paridecl.h}):
%
\bprog%
