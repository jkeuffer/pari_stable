...
uses these timers, the timings will be meaningless. To handle timing in a
reentrant way, PARI defines a dedicated data type, \tet{pari_timer}. The
functions

\fun{void}{timer_start}{pari_timer *T}

\fun{long}{timer_delay}{pari_timer *T}

\fun{long}{timer_printf}{pari_timer *T, char *format,...}
...
\bprog
  ...
  for (i = 1; i < 10; i++) {
    ...
    timer_printf(&T, "for i = %ld (L[i] = %Ps)", i, L[i]);
  }
@eprog
...
\tet{fun(E, ell)} for each curves \kbd{ell}, stopping if \kbd{fun} returns a
non-zero value.
...
\fun{long}{timer_get}{pari_timer *T} returns the time elapsed since the timer
