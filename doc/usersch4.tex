...
are of type \tet{ulong} (unsigned long) and not of type \kbd{GEN} as would
seem more natural.
...
\fun{GEN}{gerepilecopy}{ulong ltop, GEN x)}
...
\fun{void}{gerepileall}{ulong ltop, int n, ...}
...
\fun{void}{gerepilemanysp}{ulong ltop, ulong lbot, GEN *gptr[], int n}
...
(on the other hand, it is less cache friendly). The whole thing could (and
...
dangerous since the addition can overflow, which would result in \kbd{limit}
being always smaller than \kbd{avma}. This will prevent garbage collection in
the loop. To avoid this problem, we provide a robust macro
...
properly used, is usually around 1 or 2 percents of the total running time,
...
Lastly, if all seems lost, just use \tet{gerepilecopy} (or
\tet{gerepilemany}) to fix up the stack for you.
