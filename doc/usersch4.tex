...
function will typecast the result to \kbd{long}. Note that \kbd{ldiv} is an ANSI C
...
\misctitle{Notes}: 1)  The main use of this function is to prepare for later
assignments (see \secref{se:assign}). Most of the time you will use \kbd{GEN}
objects as they are created and returned by PARI functions. In this case you
do not need to use \kbd{cgetg} to create space to hold them.
...
is \kbd{bit\_accuracy(DEFAULTPREC)} = 64. This means \kbd{z} can hold
...
numerical values, e.g.~\kbd{cgeti(4)}, but this would have different meanings
on different machines as \kbd{bit\_accuracy(4)} equals 64 on 32-bit machines,
...
are global (declared by \kbd{pari.h}). They are of type \tet{pari_sp},
...
stack and we do not want to give back~\kbd{z}.
...
will have a stack which has the desired results \kbd{z1}, \kbd{z2} (say), and
...
\kbd{avma}, and updates the pointers \kbd{z1} and \kbd{z2}.

An equivalent, slighly slower, formulation is to simply write
\bprog
  gerepileall(ltop, 2, &z1, &z2)
@eprog
so that the array \kbd{gptr} is in fact not needed.
...
\kbd{gerepilemanysp} instead, which will usually be faster since we do not
...
The macro \tet{stack_lim}\kbd{(avma,$n$)} denotes an address where $2^{n-1} /
...
indicate that the price paid for using \kbd{gerepile}, when properly used, is
...
the stack pointer \kbd{avma} at the beginning, and just \emph{before} the
...
Lastly, if all seems lost, just use \tet{gerepilecopy} (or \tet{gerepileall})
...
is nonzero (in which case the \emph{pointer}~\kbd{z} certainly will be
...
first codeword \kbd{z[0]}, which we do not have to worry about since this is
...
\emph{internal type number}\sidx{type number} associated to the symbolic type
...
one is used by \kbd{gp} for internal garbage collecting, you will not have to
worry about it.
...
\fun{void}{int_normalize}{GEN z, long known0} normalizes in place a
non-negative integer (such that $z_n$ is possibly $0$), assuming at least the
first \kbd{known0} words are zero.
...
the macros \teb{leading_term}, \teb{constant_term}, \teb{degpol}
...
stack so the returned value is not safe for a basic \kbd{gerepile} call.

Note that the \idx{degree} of the polynomial is equal to its length minus
three. The function
...
\kbd{z[lg(z)-1]} is not an exact zero.
...
\kbdsidx{t_RFRAC}\sidx{rational function} \kbd{z[1]} points
to the numerator, and \kbd{z[2]} on the denominator. The denominator must be
of type polynomial.

\subsec{Type \typ{QFR} (indefinite binary quadratic form):}%
\kbdsidx{t_QFR}\sidx{indefinite binary quadratic form} \kbd{z[1]},
\kbd{z[2]}, \kbd{z[3]} point to the three coefficients of the form and should
be of type integer. \kbd{z[4]} is Shanks's distance function, and should be
of type real.

\subsec{Type \typ{QFI} (definite binary quadratic form):}%
\kbdsidx{t_QFI}\sidx{definite binary quadratic form} \kbd{z[1]}, \kbd{z[2]},
\kbd{z[3]} point to the three coefficients of the form. All three should be
of type integer.
...
\noindent The next two types were introduced for specific \kbd{gp} use, and you
...
Under \kbd{gp}, one need not worry too much since the interpreter will define
...
As we have seen in \secref{se:intro4}, a plethora of objects is associated to
...
will be used under \kbd{gp}. The code above reads the value of \kbd{y}, as it is
currently known by the \kbd{gp} interpreter (possibly creating it in the
process). All is well and good if \kbd{y} has not been tampered with in
previous \kbd{gp} commands. But if \kbd{y} has been modified (e.g \kbd {y = 1}),
...
to~\kbd{gp} will raise an error. Temporary variables are meant to be used as free
...
destroyed by a \kbd{gerepile} or \kbd{avma = ltop} statement). When you do not
...
returns the resulting \kbd{GEN}. This way, you will not have to worry about
...
\emph{before} accessing \kbd{lx(x[1])}. Doing it the other way round could
...
\kbd{-n}, hence does not normally perform an exact division on integers. The
...
\emph{before} the squaring, so that if it is the last one, \kbd{lbot} will
indeed be the bottom address of the garbage pile, and \kbd{gerepile} will
...
occurrences of the instruction \kbd{y=gadd(p2,y)}. Of course, we could also
...
@eprog
\noindent At this point you can rebuild the database by running \kbd{make
Def} in the directory \kbd{desc}. Then, you can recompile \kbd{gp}.
...
@eprog
\noindent which means we have a function \kbd{ClassGroupInit} under \kbd{gp}, which
calls the library function \kbd{bnfinit0} . The function has one mandatory
argument, and possibly two more (two \kbd{'D'} in the code), plus the current
real precision. More precisely, the first argument is a \kbd{GEN}, the second
one is converted to a \kbd{long} using \kbd{itos} (\kbd{0} is passed if it is
omitted), and the third one is also a \kbd{GEN}, but we pass \kbd{NULL} if no
...
case be completely identical to the GP function \kbd{bnfinit} but \kbd{gp} does
