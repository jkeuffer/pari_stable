...
(respectively called \teb{gen_0}, \teb{gen_1}, \teb{gen_m1} and
\teb{gen_2}), the fraction $\dfrac{1}{2}$ (\teb{ghalf}), the complex number
...
(see~\secref{se:freadexpr}) to execute a GP command and create GP variables
...
long v = varn(gy);
@eprog\noindent
But please note the quote \kbd{'y} in the above. Using \kbd{freadexpr("y")}
...
instead to the current value of the \kbd{gp} variable (e.g \kbd{gen\_1}).
...
under \kbd{gp}. These functions are called \teb{freadexpr} and \teb{freadseq}. The
first one has the following syntax:\label{se:freadexpr}

\fun{GEN}{freadexpr}{char *s}
...
use \kbd{readGEN(stdin)}. This function returns \kbd{NULL} if \kbd{EOF} is
...
to be processed by \kbd{readseq}\cr
