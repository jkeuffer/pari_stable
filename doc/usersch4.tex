...
\misctitle{Note}: due to the requirements mentioned above, it's usually
...
are global (declared by \kbd{pari.h}). They are of type \tet{pari_sp} (pari
stack pointer).
...
\kbd{gerepilemanysp} instead, which will usually be faster since we don't
...
\noindent The next two types were introduced for specific GP use, and you'll
...
\kbd{functions/section/\var{gpname}}
...
\misctitle{Note:} If \kbd{install} is not available, we have to add a file
 ...
 \kbd{functions/number\_fields/bnfinit}
...
C-Name: bnfinit0
Prototype: GD0,L,DGp
Help: bnfinit(P,{flag=0},{tech=[]}): this routine does this and that@com\dots
@eprog
\vfill\eject
