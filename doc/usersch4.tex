...
@eprog
\noindent
(This assumes that PARI headers have been properly installed in a standard
place, such as \kbd{/usr/local/include}.) In particular, this defines the
fundamental type for all PARI objects: the type \teb{GEN}, which is simply a
pointer to \kbd{long}.
...
\kbd{addii} on a \typ{FRAC} argument will cause disasters. Type mismatchs may
...
always yields a \typ{REAL} is you use \kbd{mulir}, where \kbd{gmul} returns the
...
\fun{GEN}{gerepile}{pari_sp ltop, pari_sp lbot, GEN q}
...
\fun{GEN}{gerepileupto}{pari_sp ltop, GEN q}
...
\fun{GEN}{gerepilecopy}{pari_sp ltop, GEN x}
...
\fun{void}{gerepilemany}{pari_sp ltop, GEN *gptr[], int n}
...
\fun{void}{gerepilemanysp}{pari_sp ltop, pari_sp lbot, GEN *gptr[], int n}
...
is not resticted to \typ{REAL}s, it also applies for $p$-adics for instance.)
...
Instead, finite field elements should be created using \kbd{ffgen}.

This type has a second codeword \kbd{z[1]} which determines the storage format
of the element.

The format \tet{t_FF_FpXQ}:\kbd{A=z[2]} and \kbd{T=z[3]} are \kbd{FpX},
...
The format \tet{t_FF_Flxq}:\kbd{A=z[2]} and \kbd{T=z[3]} are \kbd{Flx},
\kbd{l=z[4]} is a \typ{INT}, where $l$ prime number, $T$ is irreducible
modulo $p$, and the degree of $A$ is strictly less than the degree of $T$.
...
\misctitle{Advanced remark}. The functions \tet{pari_putc}, \tet{pari_puts},
\tet{pari_flush}, \tet{pari_printf} output according to the values of the
\teb{pariOut}~/ \teb{pariErr} global variables which provides a global
output context, i.e.~ a set of low-level functions to actually interact with
a ``stream''. By default, we have \kbd{pariOut = defaultOut}, the output
``stream'' is an actual \kbd{FILE*}, \tet{pari_outfile}, initialized to
\tet{stdout}, and the low-level printing functions are actually the standard
\kbd{putc} / \kbd{puts}, plus some magic to handle a logfile if one is open.
If you intend to change \kbd{pariOut}, diverting output in non-trivial ways,
this probably means you are rewriting \kbd{gp}. In that case, have a look at
the code in \kbd{language/es.c} (\kbd{pariOut80} comes to mind).
...
in allow smooth error recovery and \kbd{gp} interaction. You will be better
off not using them.
...
them knows how to print PARI objects by default. Seasoned PARI developpers
