...
\sidx{variable}
(\teb{pol_x}\kbd{[$v$]}), and the polynomials $1_v$ (\teb{pol_1}\kbd{[$v$]}).
...
variable number~$v$. Here is the complete list: \teb{pol_1}$[v]$ and
\teb{pol_x}$[v]$, which you can use in library mode and which represent,
...
\misctitle{Caveat}: it is possible to use \teb{gp_read_str}
(see~\secref{se:gp_read_str}) to execute a GP command and create GP variables
...
error handling routine \teb{pari_err}. The basic syntax is
%
\bprog
...
the prefix \kbd{warn}). In that case, \teb{pari_err} does \emph{not} abort the
computation, just print the requested message and go on. The basic example is

\kbd{pari\_err(warner, "Strategy 1 failed. Trying strategy 2")}

\noindent which is the exact equivalent of \kbd{pari\_err(talker,...)} except
that you certainly do not want to stop the program at this point, just inform
the user that something important has occurred (in particular, this output
would be suitably highlighted under \kbd{gp}, whereas a simple \kbd{printf}
would not).
...
@eprog
\noindent since the \teb{pari_err} function accepts PARI formats. Here \kbd{i}
...
error handling routine \kbd{pari\_err} (see \secref{se:err}). Recall that, apart
