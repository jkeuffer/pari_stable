...
more efficient than a deep copy). In particular, such outputs are not
suitable for \kbd{gerepileupto} and particular care must be taken when
garbage collecting objects which have been input to shallow functions: in
general corresponding outputs also become invalid and should no longer be
accessed.
...
is to far from the base address. If stackbase is NULL, disable the check, else
set the base address to stackbase. It is normally used this way
\bprog
int thread_start (...)
{
...
\fun{void}{consteuler}{long prec} precomputes Euler-Mascheroni's constant
at precision \kbd{prec}.

\fun{void}{constpi}{long prec} precomputes $\pi$ at precision \kbd{prec}.

\fun{void}{constlog2}{long prec} precomputes $\log(2)$ at precision
\kbd{prec}.
...
\fun{GEN}{FpXQ_log}{GEN a, GEN g, GEN ord, GEN T, GEN p} Let $g$ of order
\kbd{ord} in the field $F_p[X]/(T)$. Return $e$ such that $a^e=g$. If $e$
does not exists, the result is currently undefined.
...
components are not copied; in particular, this function is not suitable for
\kbd{gerepileupto}.
...
copied; in particular, this function is not suitable for \kbd{gerepileupto}.
...
\fun{GEN}{gscalmat}{GEN x, long n} scalar matrix, \kbd{x} times the identity.

\fun{GEN}{gscalsmat}{long x, long n} scalar matrix, \kbd{stoi(x)} times
the identity.
...
\fun{GEN}{mulcxI}{GEN x} creates the \typ{COMPLEX} $ix$, assuming 
$x$ is a \typ{INT}, \typ{REAL}, \typ{COMPLEX} or \typ{FRAC}. The result in
general contains data pointing back to the original $x$. Use \kbd{gcopy}
if this is a problem. But in most cases, the result is to be used
immediately, before $x$ is subject to garbage collection.

\fun{GEN}{mulcxmI}{GEN x}, as \tet{mulcxI}, but creates the \typ{COMPLEX}
$-ix$.
...
to~0, 0~(false) otherwise. Note that many PARI functions return \kbd{gen\_0}
...
*perm} sorts \kbd{x} in place, without copying its components. If
...
assuming \kbd{fx} and \kbd{fy} are factorization matrices for $X$ and $Y$
sorted with respect to the comparison function \kbd{cmp} (see
\tet{sort_factor}), returns the factorization of $X * Y$.

Zero exponents in the latter factorization are preserved, e.g. when merging
the factorization of $2$ and $1/2$, the result is $2^0$.
...
\fun{GEN}{gneg_i}{GEN x} shallow function yielding $-\kbd{x}$. The result
is not suitable for gerepileupto.
...
\typ{FFELT}. The result is not suitable for gerepileupto.
...
\fun{GEN}{FF_log}{GEN a, GEN g} the \typ{FFELT} \kbd{g} being a generator fo
the definition field of the \typ{FFELT} \kbd{a}, returns a \typ{INT} $e$ such
that $a^e=g$.  If $e$ does not exists, the result is currently undefined.

\fun{GEN}{FF_order}{GEN a, GEN o} returns the order of the \typ{FFELT} \kbd{a}.
If \kbd{o} is non-\kbd{NULL}, it is assumed that \kbd{o} is a multiple of the
order of \kbd{a}.

\fun{GEN}{FF_primroot}{GEN a} returns a generator of the multiplicative group
of the definition field of the \typ{FFELT} \kbd{a}.
