...
There are currently five modules in GP: general functions
(\tet{functions_basic}), gp-specific functions (\tet{functions_gp}),
gp-specific highlevel functions (\tet{functions_highlevel}),
 and two modules of obsolete functions. The function
...
\fun{void}{gp_context_restore}{struct gp_context* rec} restore the current
GP context. The new context need to be an ancestor of the current context.

\subsec{GP history}

Theses functions allow to control the GP history (the \kbd{\%} operator).
...
\kbd{gerepileupto}), otherwise the following can appear at the start of the
code string:
%
\+& \kbd{i} & return \kbd{int}\cr
\+& \kbd{l} & return \kbd{long}\cr
\+& \kbd{v} & return \kbd{void}\cr
\+& \kbd{m} & return \kbd{GEN}. Here it is allowable to return
a component of the input (used for member functions).\cr

No more than 8 arguments to the C function can be given.
