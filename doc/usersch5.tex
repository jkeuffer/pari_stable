...
\subsec{e\_PREC} Function $s$ fails because input accuracy is too low.
...
\subsec{e\_TYPE} An argument $x$ of function $s$ had an unexpected type.
...
\subsec{e\_DIM} arguments $x$ and $y$ submitted to function $s$ have
inconsistent dimensions. E.g., multiplying matrices of inconsistent dimension,
adding vectors of different lengths,\dots
\bprog
  pari_err(e_DIM, const char *s, GEN x, GEN y)
@eprog\noindent prints the error message: \kbd{inconsistent dimensions in $s$}.

\subsec{e\_VAR} polynomials $x$ and $y$ submitted to function $s$ have
...
\subsec{e\_MAXPRIME} A function using the precomputed list of prime numbers
ran out of primes.
\bprog
...
\subsec{e\_CONSTPOL} An argument of function $s$ is a constant polynomial,
...
\subsec{e\_IRREDPOL} Function $s$ expected an irreducible polynomial,
...
\subsec{e\_ZEROPOL} An argument of function $s$ is a zero polynomial, which
does not make sense. (As in \kbd{polroots(0)}.)
\bprog
  pari_err(e_ZEROPOL, const char *s)
@eprog\noindent prints the error message: \kbd{zero polynomial in $s$}.

\subsec{e\_TYPE2} Forbidden operation between two objects than cannot be
...
\subsec{e\_OP} Impossible operation between two objects than cannot be
...
\subsec{e\_INV} Tried to invert a non-invertible object, which
...
\subsec{e\_INTMOD} Tried to invert a non-invertible \typ{INTMOD}
...
\subsec{e\_NEGVAL} An argument of function $s$ is a power series with
...
\subsec{e\_SQRTN} Trying to compute an $n$-th root which does not exist.
...
\subsec{e\_NOTFUNC} Generated by the PARI evaluator; tried to use a
...
\subsec{e\_SYNTAX} Syntax error, generated by the PARI parser.
\bprog
...
{
  foo x;
  if (a > s_foo.n || b > s_foo.n) pari_err(e_BUG,"swapfoo");
  x        = t_foo[a];
  t_foo[a] = t_foo[b];
  t_foo[b] = x;
}
