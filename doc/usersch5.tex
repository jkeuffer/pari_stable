...
\kbd{gel(S,d)}; and a record of $n$ members, all initialized to $0$.
The arguments $d$ and $n$ must be non-negative.
...
think of it as an ordinary \typ{VEC} with $d$ entries, together with a record
containing \kbd{GEN} members indexed by integers $1,\dots,n$, all initially
set to $0$.

The second argument \var{tag} is the index of the struct member (from $1$ to
$n$) we operate on. We recommand to define an \kbd{enum} and use descriptive
...
The first $d$ components one are ordinary \typ{GEN} entries, which you can
read or assign to in the customary way. But the last component $\kbd{gel(S,
d)}$, a \typ{VEC} of length $n$ initialized to \kbd{zerovec}$(n)$, must
...
interacting safely with intermediate \kbd{gerepile} calls.

This mechanism allows to simulate C \kbd{struct}s, in a simpler way than
general hashtables, while remaining compatible with the GP language, which
knows neither structs nor hashtables. It also serialize the structure in an
...
is non empty, return it. Otherwhise return \kbd{NULL}. The \typ{INT} $0$
...
\fun{GEN}{obj_checkbuild_prec}{GEN S, long tag, GEN (*build)(GEN,long), long prec}
