...
\fun{GEN}{ishiftr}{GEN x, long n} given a \typ{REAL} $x$, returns
\kbd{truncr(shiftr(x,n))}, but faster and without leaving garbage on the stack.
...
shift $s[m], s[m+1],\ldots s[M], f$ left by $n$ bits.

\fun{void}{shift_right}{GEN t, GEN s, long m, long M, ulong f, ulong n}
shift $f, s[m], s[m+1],\ldots s[M]$ right by $n$ bits.
...
are lifted representatives of elements of $R[X]/(T)$ for some base ring $R$
(number field elements in most applications). Return a vector where all
