...
\+& \kbd{m} & return \kbd{GEN}. Here it is allowed to directly return a
component of the input (obviously \cr
\+&&\quad  not suitable for \kbd{gerepileupto}). Used for member functions, to
...
\kbd{*entree})\cr
\+& \kbd{W} & a \kbd{GEN} which is a lvalue which will be modified in place
(for \typ{LIST})\cr
...
prime factors of the \typ{INT} \kbd{n} are less or equal to $lim$.
...
$\pi(x)$, using $\pi(x) < x / (\log x - 1.11)$ for $x\geq 4$.
...
\fun{int}{forprime_init}{forprime_t *T, GEN a, GEN b}

\fun{ulong}{u_forprime_next}{forprime_t *T}

\fun{GEN}{forprime_next}{forprime_t *T}

\fun{void}{u_forprime_restrict}{forprime_t *T, ulong c} let $T$ an iterator
initialized via \kbd{u\_forprime\_init(\&T, a, b)}, possibly followed by a
number of calls to \tet{u_forprime_next}, and $a \leq c \leq b$. Restrict the
range of primes considered to $[a,c]$.

\fun{int}{u_forprime_arith_init}{forprime_t *T, ulong a, ulong b, ulong c,
ulong q} initialize an iterator over primes in $[a,b]$, congruent to $c$
...
\fun{long}{F2m_rank}{GEN x} rank of $x$.
...
\kbd{FpX\_red} to each leaves and returns the result (and we obtain a tree
of \kbd{FpX}s).
...
\fun{GEN}{FpX_rem_Barrett}{GEN x, GEN B, GEN T, GEN p}, returns
\kbd{x} modulo \kbd{T}, where \kbd{B} is the Barrett inverse of \kbd{T}.

\fun{GEN}{FpX_divrem_Barrett}{GEN x, GEN B, GEN T, GEN p, GEN *pr},
identical to \kbd{FpX\_divrem} except that \kbd{B} must be the Barrett inverse
of \kbd{T}.
...
Assumes that \kbd{p} is prime or $4$.
...
\subsec{Preconditioned reduction}

For faster reduction, the modulus \kbd{T} can be replaced by an extended modulus,
which is an \kbd{FpXT}, in all \kbd{FpXQ}- and \kbd{Fq}-classes functions,
and in \kbd{FpX\_rem} and \kbd{FpX\_divrem}.
...
a \kbd{FpX} to an extended modulus of for the corresponding \kbd{Flx}.
...
\fun{GEN}{FpXY_evaly}{GEN Q, GEN y, GEN p, long vy} $Q$ an \kbd{FpXY}, returns
the \kbd{FpX} $Q(X,y)$, where $X$ is the second variable of $Q$.
...
\fun{GEN}{FqXY_eval}{GEN Q, GEN y, GEN x, GEN T, GEN p} $Q$ an \kbd{FqXY}, i.e.~a
\typ{POL} with \kbd{Fq} or \kbd{FqX} coefficients representing an element of
$\F_q[X][Y]$. Returns the \kbd{Fq} $Q(x,y)$.

\fun{GEN}{FqXY_evalx}{GEN Q, GEN x, GEN T, GEN p} $Q$ being an \kbd{FqXY}, returns the
\kbd{FqX} $Q(x,Y)$, where $Y$ is the main variable of $Q$.
...
\fun{GEN}{FpXQXQ_matrix_pow}{GEN x, long m, long n, GEN S, GEN T, GEN p}, as
\kbd{FpXQXQ\_powers}$(x, n-1,S, T, p)$, but returns the powers as a an
$m\times n$ matrix.
...
\fun{GEN}{FqXQ_matrix_pow}{GEN x, long m, long n, GEN S, GEN T, GEN p}, as
\kbd{FqXQ\_powers}$(x, n-1,S, T, p)$, but returns the powers as a an
$m\times n$ matrix.
...
\fun{long}{FpXQX_nbfact}{GEN S, GEN T, GEN p} returns the number of irreductible
factor of the polynomial $S$ over the finite field $\F_q$ defined by $T$ and $p$.

\fun{long}{FqX_nbfact}{GEN S, GEN T, GEN p} as above but accept \kbd{T=NULL}.

\fun{long}{FpXQX_nbroots}{GEN S, GEN T, GEN p} returns the number of roots of the
polynomial $S$ over the finite field $\F_q$ defined by $T$ and $p$.
...
$A(X)^{{q-1}\over 2}\pmod{S(X)}$ over the finite field $\F_q$ defined by $T$
...
\fun{GEN}{Flx_mulu}{GEN y, ulong x, ulong p} as \kbd{Flx\_Fl\_mul} but do not assume
that $x<p$.
...
\fun{GEN}{Flx_rem_Barrett}{GEN x, GEN mg, GEN T, ulong p}, returns \kbd{x}
modulo \kbd{T}, assuming that $\deg x\leq 2\*(\deg{T} - 1)$ where \kbd{mg} is
the Barrett inverse of \kbd{T}.
...
\fun{int}{Flx_is_smooth}{GEN f, long r, ulong p} return $1$ if all irreducible factors of $f$
are of degree at most $r$, $0$ otherwise.
...
MA,GEN MB}, as \kbd{FpX\_ffintersect}
...
\fun{GEN}{FlxT_red}{GEN V, ulong p} reduces each leaves with \kbd{Flx\_red}.
...
\fun{GEN}{FlxY_evalx}{GEN x, GEN y, ulong p}
...
\fun{GEN}{ZpXQ_invlift}{GEN b, GEN T, GEN p, long e} let
$p$ be a prime \typ{INT} and $b$ be a \kbd{FpXQ}s(modulo $T$).
...
$v_p(f'(a))=0$.  Return a \typ{INT} which are the root of $f$ mod $p^e$
...
\fun{GEN}{ZpXQX_liftroot_vald}{GEN f, GEN a, long v, GEN T, GEN p, long e} as
\tet{ZpXQX_liftroot}, when $v$ is the valuation of the content of $f'$ and it
is required that $v_p(f(a))>v$ and $v_p(f'(a))=v$.
...
\fun{GEN}{ZpXQ_log}{GEN a, GEN T, GEN p, long e} $T$ being a \kbd{ZpX} irreducible modulo $p$,
return the logarithm of $a$ in $\Z_p[X]/(T)$ to precision $e$, assuming that
$a\equiv 1 \pmod{p\Z_p[X]}$ if $p$ odd or $a\equiv 1 \pmod{4\Z_2[X]}$ if $p=2$.
...
$E:y^2=x^3+a_4\*x+a_6$, with $a_4$ and $a_6$ in $\F_p$. A \kbd{FpE} is a point of
$E(\F_p)$.  Since an affine point and $a_4$ determine an unique $a6$, most
functions do not take $a_6$ as an argument.
A \kbd{FpE} is either the point at infinity (\kbd{ellinf()}) or a $FpV$ whith two
components. The parameters $a_4$ and $a_6$ are given as \typ{INT}s when required.
...
\fun{int}{zvV_equal}{GEN x, GEN y} returns $1$ if the two \kbd{zvV} (vectors of \kbd{zv})
...
(over $\Z$) by the sparse vector $B$. The result is an \kbd{ZC}, i.e.~a
dense vector.
...
solve equation $f(X) = B$ over $\F_p$, where $B$ is a \kbd{FpV}, and $f$ is a
...
\fun{GEN}{ZX_shifti}{GEN x, long n} shifts all coefficients of \kbd{x} by $n$ bits,
which can be negative.

\fun{GEN}{ZX_Z_divexact}{GEN x, GEN y} returns $x/y$ assuming all divisions
are exact.

\fun{GEN}{ZX_remi2n}{GEN x, long n} reduces all coefficients of \kbd{x} to $n$ bits,
using \tet{remi2n}.
...
\fun{GEN}{ZXX_Z_divexact}{GEN x, GEN y} returns $x/y$ assuming all integer divisions
are exact.
...
\fun{GEN}{mkcol4s}{long x, long y, long z, long t} creates a 4-dimensional \typ{COL}
containing \kbd{[stoi(x), stoi(y), stoi(z), stoi(t)]~}.
...
\fun{GEN}{mkvec4s}{long x, long y, long z, long t} creates a 4-dimensional \typ{VEC}
containing \kbd{[stoi(x), stoi(y), stoi(z), stoi(t)]}.
...
\fun{GEN}{mkmat3}{GEN x, GEN y, GEN z} creates a 2-column \typ{MAT} with columns
$x$, $y$, $z$ (\typ{COL}s of the same length).

\fun{GEN}{mkmat4}{GEN x, GEN y, GEN z, GEN t} creates a 2-column \typ{MAT} with columns
$x$, $y$, $z$, $t$ (\typ{COL}s of the same length).

\fun{GEN}{mkmat5}{GEN x, GEN y, GEN z, GEN t, GEN u} creates a 2-column \typ{MAT} with columns
$x$, $y$, $z$, $t$, $u$ (\typ{COL}s of the same length).
...
returns the $5$-dimensional \typ{VEC} \kbd{[x,y,z,t,u]}.
...
returns the $5$-dimensional \typ{COL} \kbd{[x,y,z,t,u]~}.
...
returns the $2$-column \typ{MAT} with colums \kbd{x}, \kbd{y}.

\fun{GEN}{retmkmat3}{GEN x, GEN y, GEN z}
returns the $3$-dimensional \typ{MAT} \kbd{[x,y,z]}.

\fun{GEN}{retmkmat4}{GEN x, GEN y, GEN z, GEN t}
returns the $4$-dimensional \typ{MAT} \kbd{[x,y,z,t]}.

\fun{GEN}{retmkmat5}{GEN x, GEN y, GEN z, GEN t, GEN u}
returns the $5$-dimensional \typ{MAT} \kbd{[x,y,z,t,u]}.
...
\fun{GEN}{gen_pow_fold}{GEN x, GEN n, void *D, F1 sqr, F1 msqr} as
...
the previous \tet{gen_pow_fold}

\fun{GEN}{gen_powers}{GEN x, long n, long use_sqr, void *D, F1 sqr, F2 mul, F0 one}
returns $[\kbd{x}^0, \dots, \kbd{x}^\kbd{n}]$ as a \typ{VEC};
\kbd{mul(D, $x$, $y$)} implements the multiplication
in the underlying monoid; \kbd{sqr} is a (presumably optimized) shortcut for
\kbd{mul(D, $x$, $x$)}; \kbd{one} returns the monoid unit.
The flag \kbd{use\_sqr} should be set to $1$ if squaring are faster than
multiplication by $x$.
...
\fun{GEN}{cxexp1}{GEN x} returns $\exp(x)-1$, for a \typ{COMPLEX} $x$.
...
\subsec{Functions to handle elliptic curves over finite field}

In this subsection, $E$ is assumed to be an \tet{ellff} structure generated by
\tet{ellffinit}, when the second argument is a \typ{FFELT}.

\fun{GEN}{FF_ellgens}{GEN E} returns the generators of the group $E$.

\fun{GEN}{FF_elllog}{GEN E, GEN P, GEN G, GEN o} Let \kbd{G} be a point of
order \kbd{o}, return $e$ such that $e.P=G$. If $e$ does not exists, the result
is currently undefined.

\fun{GEN}{FF_ellmul}{GEN E, GEN P, GEN n} returns $n.P$ where $n$ is an integer
and $P$ is a point on the curve $E$.

\fun{GEN}{FF_ellorder}{GEN E, GEN P, GEN o} returns the order of the point $P$,
where $o$ is a multiple of the order of $P$, or its factorization.
...
\fun{GEN}{mpsincos1}{GEN x, long *qmod8} compute $y\in[-\pi/4,\pi/4]$
such that $y = x + q \pi/2$, $q\in \Z$ and return $\cos(y) - 1$; this is
more accurate than \kbd{subrs(mpcos(y), 1)}, which suffers from catastrophic
cancellation if $|x|$ is very small. Set \kbd{*qmod8} to the integer in
...
\bprog
  pari_err(e_DOMAIN, const char *f, const char *v, const char *op, GEN lim, GEN x)
  pari_err_DOMAIN(const char *f, const char *v, const char *op, GEN lim, GEN x)
...
(*eq)(void*,void*), int use_stack}\hfil\break
...
of keys and test keys for equality, respectively. If \kbd{use\_stack}
...
\fun{void}{pari_stack_alloc}{pari_stack *s, long nb} makes room for \kbd{nb} more
...
\fun{void}{pari_stack_delete}{pari_stack *s} frees \kbd{data} and resets the stack
to the state immediately following \kbd{stack\_init} (\kbd{s->n} and
\kbd{s->alloc} are set to $0$).

\fun{void *}{pari_stack_pushp}{pari_stack *s, void *u} This function assumes that
