...
in a program, and cleanup without exiting the whole program. Unless you
need the full GP analyzer, you should be fine with the first ones.
...
mentionned (functions handling \kbd{growarray} below), no PARI function
will function properly before such an initialization.
...
\subsec{The \kbd{entree} structure}
...
Built-in pari functions are defined in \emph{modules} \tet{functions_basic},
\tet{functions_gp}, \tet{functions_highlevel}, which are arrays of
\kbd{entree} structs, the last of which satisfying \kbd{name = NULL}
(sentinel).
...
The rest have to do with the GP analyzer, in particular the declaration of
functions, and partial initializations. A routine in GP is described to the
analyzer by an \kbd{entree} structure. Routines are grouped in modules,
which are arrays of \kbd{entree} structs. There are currently six modules
in GP: general functions, gp-specific functions, gp-specific highlevel
functions, member functions, and two modules of obsolete functions. See
e.g. \kbd{src/language/init.h} for the modules of general functions.

\fun{void}{pari_init_opts}{size\_t size, ulong maxprime, ulong
opts, growarray MOD, \hskip 0pt plus 5pt growarray OLD} as \kbd{pari\_init}, with
extra flexibility. \kbd{opts} is a mask of flags among the
following:
...
Finally, we need to explain \kbd{MOD} and \kbd{OLD}. Each is 
\kbd{growarray} (see below) containing the modules of extra routines to
made known to the GP analyzer besides the general ones. You probably want to
set them to \kbd{NULL} (no extras). The calculator \kbd{gp} adds gp-specific
routines. If you want to do the same, \kbd{MOD} lists the extra routines, and
\kbd{OLD} obsolete routines which are used when the \kbd{compatible}
default is set.

\fun{void}{pari_close_opts}{ulong init\_opts} as \kbd{pari\_close},
for a library initialized with a mask of options using
\kbd{pari\_init\_opts}. The same mask must be provided for initialization
and closing.
...
A \kbd{growarray} is a container type, wich enlarges itself as one appends
...
Here is an example from the gp code. the following routine would initialize
the pari library with an interpreter knowing the same routines as GP, 
without support for obsolete names:
\brog
void
gp_init_opts(size_t size, ulong maxprime, ulong opts)
{
  growarray new;
  grow_init(new);
  grow_append(new, functions_gp);
  grow_append(new, functions_highlevel);
  pari_init_opts(size, maxprime, opts, new, NULL);
}
@eprog\noindent
...
@eprog\noindent
has almost the same effect, except that it does not organize
functions in the interpreter as modules of up-to-date and obsolete
functions. As a result, changing \kbd{compatible} setting back and forth
will cause the analyzer to forget about the functions added by the latter
construction.
...
\fun{ulong}{gener_Fl_local}{ulong p, GEN L}, see \kbd{Fp\_gener\_local},
\kbd{L} is an \kbd{Flv}.
...
\fun{GEN}{Z_factor_limit}{GEN n, GEN lim} as \kbd{Z\_factor}, but factors
the \typ{INT} \kbd{n} until the unfactored part is smaller than
\kbd{lim}. In particular, the last entry o nthe first factorization
column is in general a composite number.
