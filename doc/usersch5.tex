...
\typ{FRAC}s.
  ...
  \kbd{R}:  a commutative ring, whose elements can be
...
\fun{GEN}{FpX_ffisom}{GEN P, GEN Q, GEN p}. Assumes \kbd{p} is prime,
...
MA, GEN MB)}. Assumes \kbd{p} is prime, \kbd{P}, \kbd{Q} are \kbd{ZX}s, both
irreducible mod \kbd{p}, and \kbd{n} divides both the degree of \kbd{P} and
\kbd{Q}. Compute \kbd{SP} and \kbd{SQ} such that the subfield of $\F_p[X]/(P)$
generated by \kbd{SP} and the subfield of $\F_p[X]/(Q)$ generated by \kbd{SQ}
are isomorphic of degree \kbd{n}. the polynomials \kbd{P} and \kbd{Q} do not
need to be of the same variable.  if \kbd{MA} (resp. \kbd{MB}) is not
\kbd{NULL}, it must be the matrix of the Frobenius map in $\F_p[X]/(P)$ (resp.
$\F_p[X]/(Q)$ ).
...
\fun{GEN}{RX_mul}{GEN x, GEN y} multiplies the two \typ{POL} (in the same
variable) \kbd{x} and \kbd{y}. Uses Karatsuba algorithm.

\fun{GEN}{RX_sqr}{GEN x} squares the \typ{POL} \kbd{x}. Uses Karatsuba
algorithm.
...
with arbitrary components. Similarly, an \kbd{RXV} is a \typ{VEC} or
\typ{COL} with \kbd{RX} components, etc.
...
\fun{GEN}{RX_to_RV}{GEN x, long N} converts the \typ{POL}~\kbd{x} to a
...
\fun{GEN}{RM_to_RXV}{GEN x, long v} converts the \kbd{RM}~\kbd{x} to a
\typ{VEC} of \kbd{RX}, by repeated calls to \kbd{RV\_to\_RX}.
