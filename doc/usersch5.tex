...
\fun{ulong}{F2v_coeff}{GEN x, long i} returns the coefficient $i$ of $x$.

\fun{ulong}{F2v_set}{GEN x, long i} sets the coefficient $i$ of $x$ to $1$.

\fun{ulong}{F2m_coeff}{GEN x, long i, long j} returns the coefficient $(i,j)$
of $x$.

\fun{ulong}{F2m_set}{GEN x, long i, long j} sets the coefficient $(i,j)$ of $x$
to $1$.
...
\fun{void}{F2v_add_inplace}{GEN x, GEN y} replaces $x$ by $x+y$.
...
or \kbd{NULL} if a flow change occured.
...
result otherwise, or \kbd{NULL} if a flow change occured.
...
\fun{long}{did_break}{void} returns a non-zero value is a flow change is
pending.
...
Function using the prototype code 'V' need to manually create and delete a
lexical variable for each code 'V', which will be given a number $-1, -2,
\ldots$.
