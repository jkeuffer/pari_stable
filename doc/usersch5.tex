...
\fun{long}{uissquareall}{ulong n} as \kbd{Z\_issquare},
for an \kbd{ulong} operand \kbd{n}.
...
parameter $a_2$ is actually the \typ{VEC} $[a_3,a_4,a_3^{-1}]$.
...
\subsubsec{e\_INTMOD} Tried to invert a non-invertible \typ{INTMOD}
...
@eprog\noindent prints the error message: \kbd{impossible inverse modulo: $x$}.

\subsubsec{e\_INV} Tried to invert a non-invertible object, which
is not a \typ{INTMOD}. Indeed, in the latter case, we raise \kbd{e\_INTMOD}
instead.
\bprog
  pari_err(e_INV)
@eprog\noindent prints the error message: \kbd{division by a non-invertible
object}.
...
\subsubsec{e\_NEGVAL} An argument of function $s$ is a power series with
negative valuation, which does not make sense since the function is not
analytic at infinity. (As in \kbd{cos(1/x)}.)
\bprog
  pari_err(e_NEGVAL, const char *s)
  pari_err_NEGVAL(const char *s)
@eprog\noindent prints the error message: \kbd{negative valuation in $s$}.
...
\subsubsec{e\_SQRTN} Tried to compute an $n$-th root of $x$, which does not exist.
