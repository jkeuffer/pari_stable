...
\fun{GEN}{get_characteristic}{GEN x} returns the ``characteristic'' of the
base ring over which the polynomial is defined. If \typ{PADIC}s are not
present in the structure, this is the usual characteristic (as defined by
\typ{INTMOD} and \typ{FFELT} components). If a $p$-adic number is present
on the other hand, the function return $p$ (instead of the expected $0$).
The function raises an exception if incompatible primes arise from \typ{FFELT}
and \typ{PADIC} components.
