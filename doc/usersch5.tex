...
\noindent $\tet{BYTES_IN_LONG}  = 2^{\tet{TWOPOTBYTES_IN_LONG}}$: number of
bytes in a \kbd{long}

\noindent $\tet{BITS_IN_LONG}  = 2^{\tet{TWOPOTBITS_IN_LONG}}$: number of
bits in a \kbd{long}

\noindent \tet{BITS_IN_HALFULONG}: \kbd{BITS\_IN\_LONG} divided by $2$.

\noindent \tet{DEFAULTPREC}:    the length (\kbd{lg}) of a \typ{REAL} with 64
bits of accuracy

\noindent \tet{MEDDEFAULTPREC}: the length (\kbd{lg}) of a \typ{REAL} with
128 bits of accuracy

\noindent \tet{BIGDEFAULTPREC}: the length (\kbd{lg}) of a \typ{REAL} with
192 bits of accuracy

\noindent \tet{HIGHBIT}: the largest power of $2$ fitting in an \kbd{ulong}.

\noindent \tet{LOWMASK}: bitmask yielding the least significant bits, to be
used in \fun{ulong}{HIGHWORD}{ulong a}

\noindent \tet{HIGHMASK}: bitmask yielding the most sicgnificant bits, to be
used in \fun{ulong}{LOWWORD}{ulong a}

The following constants are obsolete:

\noindent \tet{VERYBIGINT}: the largest positive \kbd{long}. Was introduced
before C89 standardized \kbd{INT\_MAX}. Use the latter!

\noindent \tet{MAXULONG}: the largest \kbd{ulong}. Was introduced before C89
standardized \kbd{UINT\_MAX}. Use the latter!

\noindent \tet{BIGINT}:  sentinel returned by \kbd{gvar(x)} when \kbd{x} does
not contain any polynomial; has a lower priority than any valid variable
number. FIXME: name not appropriate.
