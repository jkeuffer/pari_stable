...
initialized to \kbd{NULL}. If not \kbd{NULL}, it is called just before PARI
clean-up from an error. It is not required to return.  The error number is
passed as argument, unless the PARI stack has been destroyed, in which case it
...
\fun{ulong}{usqrtn}{ulong a, ulong n} returns the floow of the $n$-th root
of $a$.
...
\fun{GEN}{zlm_echelon}{GEN M, long early_abort, ulong p, ulong pm} \kbd{Zlm}
variant of \kbd{ZpM\_echelon}.
...
\fun{GEN}{mkmat4}{GEN x, GEN y, GEN z, GEN t} creates a 2-column \typ{MAT}
with columns $x$, $y$, $z$, $t$ (\typ{COL}s of the same length).

\fun{GEN}{mkmat5}{GEN x, GEN y, GEN z, GEN t, GEN u} creates a 2-column
...
of \kbd{x} and~\kbd{y}, and puts (the addresses of) objects $u$ and~$v$ such
that $u\kbd{x}+v\kbd{y}=\text{Res}(\kbd{x},\kbd{y})$ into \kbd{*U}
and~\kbd{*V}.
...
non-memory-clean variant of \tet{gen_pow}.

\fun{GEN}{gen_powu_i}{GEN x, ulong n, void *E, F1 sqr, F2 mul}
non-memory-clean variant of \tet{gen_powu}.
...
non-memory-clean variant of \tet{gen_pow_fold}.
...
non-memory-clean variant of \tet{gen_powu_fold}.
...
returns the monoid unit. The flag \kbd{use\_sqr} should be set to $1$ if
