...
\fun{GEN}{ZV_copy}{GEN x} returns a copy of \kbd{x}.
...
\fun{GEN}{ZV_Z_mul}{GEN x, GEN y} multiplies the \kbd{ZV}~\kbd{x}
(which can be a column or row vector) by the \typ{INT}~\kbd{y}.

\fun{GEN}{ZV_lincomb}{GEN u, GEN v, GEN x, GEN y} returns $ux + vy$, where
$u$, $v$ are \typ{INT} and $x,y$ are \kbd{ZC}.
...
\fun{GEN}{FpV_center}{GEN z, GEN p, GEN pov2} returns a vector (of the same
type as \kbd{z}) whose entries are the \kbd{Fp\_center(gel(z,i))}.
