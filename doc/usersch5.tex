...
of $f$ (without multiplicity, as a \typ{VECSMALL}). Assumes that $p$ is prime
or $4$.
...
\fun{int}{F2x_is_irred}{GEN f}, as \kbd{FpX\_is\_irred}.
...
\subsec{Elliptic curves over $\F_{2^n}$, not supersingular}
Let $T$ be an irreducible \kbd{F2x} and $E$ the
elliptic curve given by the equation $E:y^2+x*y=x^3+a_2\*x^2+a_6$ with $a_2$ and $a_6$
being \kbd{F2x} in $\F_2[X]/(T)$.  A \kbd{F2xqE} is a point of $E(\F_2[X]/(T))$.
