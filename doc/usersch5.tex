...
\fun{GEN}{FpV_to_mod}{GEN z, GEN p}, \kbd{z} a \kbd{ZV}. Returns \kbd{z *
...
\fun{GEN}{Flx_shift}{GEN T, long n, ulong p}, returns \kbd{T} multiplied by $\kbd{x}^n$.
...
\fun{GEN}{ZXV_to_FlxV}{GEN v, ulong p}, as \kbd{ZX\_to\_Flx}, repeatedly called on
the vector's coefficients.

\fun{GEN}{ZXX_to_FlxX}{GEN B, ulong p, long v}, as \kbd{ZX\_to\_Flx}, repeatedly
...
\fun{GEN}{Flx_to_ZX_inplace}{GEN z}, same as \kbd{Flx\_to\_ZX}, in place (\kbd{z} is
destroyed).
...
\fun{GEN}{zx_to_ZX}{GEN z}, as \kbd{Flx\_to\_ZX}, without assuming coefficients are non-negative.

\fun{GEN}{Flv_to_ZC}{GEN z}, converts to \kbd{ZC} (\typ{COL} of non-negative
\typ{INT}s in this case)

\fun{GEN}{Flv_to_ZV}{GEN z}, converts to \kbd{ZV} (\typ{VEC} of non-negative
\typ{INT}s in this case)

\fun{GEN}{Flm_to_ZM}{GEN z}, converts to \kbd{ZM} (\typ{MAT} with non-negative
\typ{INT}s coefficients in this case)

\fun{GEN}{zv_ZC}{GEN z} as \kbd{Flv\_to\_ZC}, without assuming coefficients are
non-negative.

\fun{GEN}{zv_ZV}{GEN z} as \kbd{Flv\_to\_ZV}, without assuming coefficients are
non-negative.

\fun{GEN}{zm_ZM}{GEN z} as \kbd{Flm\_to\_ZM}, without assuming coefficients are
non-negative.
...
\fun{GEN}{Fl_to_Flx}{ulong x, long sv} converts from scalar to degree~$1$
...
\fun{GEN}{Flv_to_Flx}{GEN x, long sv} converts from vector (coefficient array)
to (normalized) polynomial in variable $v$.

\fun{GEN}{Flm_to_FlxV}{GEN x, long sv} converts the colums of \kbd{Flm}~\kbd{x}
to an array of \kbd{Flx} (repeated calls to \kbd{Flv\_to\_Flx}).
...
converted in reverse order compared to \kbd{gtopoly} (coefficients are given
by increasing degree). E.g.~\kbd{[2,3]} goes to \kbd{3*v + 2}. In other words
...
\kbd{gerepile} calls. Recall that an \kbd{RV} (resp.~an \kbd{RX}, resp.~an
\kbd{RM}) is a \typ{VEC} or \typ{COL} (resp.~a \typ{POL}, resp.~a \typ{MAT})
with arbitrary components. Similarly, an \kbd{RXV} is a \typ{VEC} or \typ{COL}
with \kbd{RX} components, etc.

\fun{GEN}{RV_to_RX}{GEN x, long v} converts the \kbd{RV}~\kbd{x} to a
(normalized) polynomial in variable~\kbd{v} (as \kbd{gtopolyrev}, without
copy).

\fun{GEN}{RX_to_RV}{GEN x, long N} converts the \typ{POL}~\kbd{x} to 
a \typ{COL}~\kbd{v} with \kbd{N} components. Other types than \typ{POL} are
...
\fun{GEN}{RM_to_RXV}{GEN x, long v} converts the \kbd{RM}~\kbd{x}
to a \typ{VEC} of \kbd{RX}, by repeated calls to \kbd{RV\_to\_RX}.

\fun{GEN}{RXV_to_RM}{GEN v, long N} converts the vector of \kbd{RX}~\kbd{v}
to a~\typ{MAT} with \kbd{N}~rows, by repeated calls to \kbd{RX\_to\_RV}.

\fun{GEN}{RM_to_RXX}{GEN x, long v,long w} converts the \kbd{RM}~\kbd{x} into
a \typ{POL} in variable~\kbd{v}, whose coefficients are \typ{POL}s in
variable~\kbd{w}. This is a shortcut for
\bprog
  RV_to_RX( RM_to_RXV(x, w), v );
@eprog\noindent
...
\fun{GEN}{RXX_to_RM}{GEN x, long N} converts the \typ{POL}~\kbd{x} with
\kbd{RX} (or constant) coefficients to a matrix with \kbd{N} rows.

\fun{GEN}{RXY_swap}{GEN P, long n, long w} converts the bivariate polynomial
$\kbd{P}(u,v)$ (a \typ{POL} with \typ{POL} coefficients) to
$P(\kbd{polx[w]},u)$, assuming \kbd{n} is an upper bound for
$\deg_v(\kbd{P})$.
...
\fun{GEN}{vec_Cei}{long n, long i} creates a \typ{VEC} with \kbd{n}
components set to \kbd{gzero}, but the \kbd{i}-th one which us set to
\kbd{C}. \emph{No copy is made}, so if \kbd{C} is not a permanent object, the
result is not suitable for \kbd{gerepileupto}.
