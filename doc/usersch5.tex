...
\noindent \tet{PARI_VERSION_CODE} encodes in a single \kbd{long}, the Major
...
\noindent \tet{PARI_VERSION_SHIFT} The number of bits used to store each of
the integers $M$, $m$, $p$ in the version code.

The two character strings \kbd{PARIVERSION} and \kbd{PARIINFO}, correspond to
the first two lines printed by \kbd{gp} just before the Copyright message.

\fun{GEN}{pari_version}{} returns the version number as a PARI object, a
\typ{VEC} with \typ{INT} components.
...
\fun{int}{isonstack}{GEN x} \kbd{true} iff \kbd{x} belongs to the stack.
...
\fun{void}{flusherr}{void} flush error stream.
...
\kbd{*entree}\cr
\+& \kbd{V} & variable (as \kbd{S}, but rejects builtin function names)\cr
...
\idx{preferences file} for instance (\secref{se:gprc}), and that's it. You
...
\fun{long}{ZV_pvalrem}{GEN x, GEN p, GEN *r} $x$ being a vector of \typ{INT}s,
return the min $v$ of the valuations of its components and set \kbd{*r} to
$x/p^v$. Infinite loop if $x$ is the zero vector.
...
\fun{long}{minss}{long x, long y} returns the \kbd{min} of $x$ and $y$.

\fun{long}{maxss}{long x, long y} returns the \kbd{max} of $x$ and $y$.
...
\fun{GEN}{Z_factor_limit}{GEN n, long lim} trial divide $x$ by all primes $p
...
If \kbd{lim} is zero, the effect is the same as setting $\kbd{lim} =
\kbd{maxprime()}$: use all precomputed primes.

\fun{GEN}{boundfact}{GEN x, long lim} as \tet{Z_factor_limit}, applying to
...
\fun{long}{millerrabin}{GEN n, long k} performs strong Rabin-Miller
compositeness tests on the \typ{INT} $n$, for $k$ random bases. This function
also caches square roots of $-1$ that are encountered during the successive
tests and stops as soon as three distinct square roots have been produced; we
have in principle factored $n$ at this point, but unfortunately, there is
currently no way for the factoring machinery to become aware of it. (It is
highly implausible that hard to find factors would be exhibited in this way,
though.) This should be slower than \tet{BPSW_psp} for $k\geq 4$ and we would
...
\section{Naming scheme}. A function name is built in the following way:
...
behaviour: \kbd{NULL} (do not store the remainder, used to implement
...
\fun{GEN}{F2m_F2c_mul}{GEN x, GEN y} as \kbd{Flm\_Flc\_mul} for $p = 2$.
...
\fun{void}{F2v_add_inplace}{GEN x, GEN y} as \kbd{Flv\_add\_inplace} for
$p = 2$.
...
\fun{ulong}{F2v_sum}{GEN x} $x$ being a \typ{VECSMALL} with 0/1 entries,
returns the sum of the entries modulo $2$.
...
\fun{GEN}{FqX_Fq_mul_to_monic}{GEN P, GEN U, GEN T, GEN p}
...
\fun{GEN}{RgXQV_to_mod}{GEN V, GEN T} $V$ a vector of \kbd{RgXQ}, which
are lifted elements of $\Q[X]/(T)$ (number field elements in most
applications). Return a vector where all non-rational entries are
converted to \typ{POLMOD}. Used to normalize the output of \kbd{nfroots}.

\fun{GEN}{RgXQXV_to_mod}{GEN V, GEN T} $V$ a vector of polynomials whose
coefficients are \kbd{RgXQ}. Analogous to \kbd{RgXQV\_to\_mod}.
...
\kbd{FqX}, paіrwise coprime in $\F_q[X]$, whose product is congruent to
...
accuracies, for a given target:
\fun{ulong}{quadratic_prec_mask}{long n} we want to reach accuracy
$n$, starting from accuracy 1, using a quadratically convergent algorithm.
For instance, to reach $n = 9$, it is better to use accuracies
$[1,2,3,5,9]$ instead of $[1,2,4,8,9]$. The idea is to essentially double
the accuracy at each step, while not overshooting in the end.

Let $a_0$ = 1, $a_1 = 2$, $a_2,\ldots, a_k = n$, be the desired sequence of
accuracies. To obtain it, we work backwards and set
$$ a_k = n,\quad a_{i-1} = (a_i + 1) \bs 2.$$
But we do not want to store $a_i$ explicitly, even as a \typ{VECSMALL},
...
bits indicate the places where we do \emph{not} double the accuracy.
...
usually more eficient than computations using a priori bounds.
...
\fun{GEN}{RgC_Rg_add}{GEN x, GEN y} assuming $x$ is a column vector
...
happens when the matrix does not have maximal rank (or when roundoff errors
make it appear so).
...
simpifying assumptions: the first $r_1$ rows of $M$ and $y$ are real
...
shape, using \kbd{gcmp1} and \kbd{gcmp0} to test components. (This makes a
...
integers $C$ and $L$. If $\kbd{L}$ is not \kbd{NULL}, set \kbd{*L} to $L$.
Not suitable for gerepileupto.

\fun{GEN}{ZX_primitive_to_monic}{GEN pol, GEN *L} as \tet{ZX_to_monic} except
\kbd{pol} is assumed to have trivial content, which avoids recomputing it.
The result is suboptimal if \kbd{pol} is not primitive ($L$ larger than
...
\fun{long}{RgX_equal}{GEN x, GEN y} returns $1$ if $x$ and $y$ have the same
degree and their coefficients are equal (as per \tet{gequal}). Variable
...
\fun{GEN}{RgXQ_caract}{GEN x, GEN T, long v} returns the characteristic
...
$p$-adic number of precision~$l$. Works componentwise on recursive objects
(e.g.~\typ{POL} or \typ{VEC}).
...
\kbd{gcmp0}). This rougher normalization is used when we have no other
...
\subsec{Generic routines}. The following routines allow to use an
...
\kbd{x} and~\kbd{y} computed using Sylverster's matrix (inexact inputs), a
...
the trivial denominatory $1$. Returns their product (either a \typ{INT} or
\kbd{NULL}).
...
\fun{GEN}{gadd}{GEN x, GEN y[, GEN z]}

\fun{GEN}{gmul}{GEN x, GEN y[, GEN z]}

\fun{GEN}{gdiv}{GEN x, GEN y[, GEN z]}

\fun{GEN}{gaddgs}{GEN x, long s[, GEN z]}

\fun{GEN}{gaddsg}{GEN s, GEN x[, GEN z]}

\fun{GEN}{gsubgs}{GEN x, long s[, GEN z]}

\fun{GEN}{gsubsg}{GEN s, GEN x[, GEN z]}

\fun{GEN}{gmulgs}{GEN x, long s[, GEN z]}

\fun{GEN}{gmulsg}{GEN s, GEN x[, GEN z]}

\fun{GEN}{gdivgs}{GEN x, long s[, GEN z]}

\fun{GEN}{gdivsg}{GEN s, GEN x[, GEN z]}
...
\fun{GEN}{quad_disc}{GEN x} returns the discriminant of the \typ{QUAD}
\kbd{x}.
...
The return type is \typ{COMPLEXE} unless the imaginary part is equal to $0$
...
p[i]$; unless mentionned otherwise, this is the form used in the funcitons
...
small group \var{grp} of domain \var{n} as smalls.
...
\fun{long}{group_isA4S4}{GEN grp} return $1$ if the small group
\var{grp} is isomorphic to $A_4$, $2$ if it is isomorphic to $S_4$ and
$0$ else. This is mainly to deal with the idiosynchrasy of the format.

\fun{GEN}{group_leftcoset}{GEN G, GEN g} where $G$ is a small group and $g$ a
small of the same domain, the the left coset $gG$ as a vector of small.

\fun{GEN}{group_rightcoset}{GEN G, GEN g} where $G$ is a small group and $g$ a
small of the same domain, the the right coset $Gg$  as a vector of small.

\fun{long}{group_perm_normalize}{GEN G, GEN g} where $G$ is a small group and
$g$ a small of the same domain, return $1$ if $gGg^-1=G$, else $0$.
...
\fun{char*}{path_expand}{const char *s} perform tilde and environment expanѕion
...
\section{Vectors of bits}
These functions manipulate vectors of bits (stored in \typ{VECSMALL}).
...
\emph{flow change}. When that occurs, the functions below generally returns
\kbd{NULL}. The caller can then adopt three positions:

\item raise an exception (\kbd{closure\_evalnobrk}).

\item pass through (by returning NULL itself).

\item handle the flow change (undocumented yet, done by loop statements).
...
an exception if a flow change occurs. Meant for pseudo-iterators where
interrupting the closure is meaningless, e.g.~\kbd{intnum} or \kbd{sumninm}.
...
\fun{void}{closure_reset}{void} reinitializes the byte-code evaluator.
This is used by the error recovery code.

Functions to deal with lexical variables introduced through the prototype
code 'V':

Such functions need to manually create and delete a lexical variable for
each code 'V', which will be given a number $-1, -2, \ldots$.

\fun{void}{push_lex}{GEN a} creates a new lexical variable whose initial value
is $a$ on the top of the stack. This variable get the number $-1$, and the number of the other variables is decreased by one unit.

\fun{void}{pop_lex}{void} deletes topmost lexical variable, increasing the
number of other variables by one unit.
...
\fun{GEN}{genclosure}{entree *ep} creates a closure associated to a PARI
function.
...
\fun{GEN}{setdefault}{const char *s, const char *v, long flag} low-level
function underlying \kbd{default0}. If $s$ is empty, call all default ѕetting
functions with empty string argument and flag \tet{d_ACKNOWLEDGE}. Otherwise,
check whether $s$ corresponds to a default and call the corresponding default
setting function with arguments $v$ and \fl.

We shall describe these functions below: an non-empty $v$ means that we change
the value of the default to $v$, possibly after some translation work. The
flag is one of

\item \tet{d_INITRC} called while reading the \kbd{gprc} (print and return
nothing, possibly defer until \kbd{gp} actually starts).

\item \tet{d_RETURN} return the current value, as a \typ{INT} if possible, as
a \typ{STR} otherwise.

\item \tet{d_ACKNOWLEDGE} print the current value.
...
\fun{GEN}{sd_rl}{const char *v, long flag}
