...
\fun{void}{diviuuexact}{GEN x, ulong y, ulong z} returns the Euclidean
...
\fun{int}{is_Z_factor0}{GEN f} returns $1$ if $f$ looks like the factorization
of a non-negative integer, and $0$ otherwise. Useful for sanity checks but not
100\% foolproof. Specifically, either \tet{is_Z_factor}$(f)$ is true
or $f$ encodes the ``factorization'' $0^1$.

\fun{GEN}{check_arith_arg}{GEN n, const char *f} check whether $n$ is suitable
for arithmetic function $f$

\fun{GEN}{check_arith_arg0}{GEN n, const char *f}
...
of degree $n$. Allow \typ{POL}, \typ{SER}, \typ{RFRAC} and scalars.
...
is defined as the gcd of all moduli \typ{INTMOD}s occuring in the structure,
