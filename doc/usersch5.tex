...
\fun{GEN}{FlxX_resultant}{GEN u, GEN v, ulong p, long evx} Returns
$\text{Res}_X(u, v)$, which is an \kbd{Flx}. Assume that
\kbd{evx = evalvarn(vx)} for some variable number \kbd{vx}.

\fun{GEN}{Fly_FlxY_resultant}{GEN a, GEN b, ulong p, long evx}
Returns $\text{Res}_Y(a, b)$, which is an \kbd{Flx}. Assume that \kbd{evx =
evalvarn(vx)} for some variable number \kbd{vx}.
...
\fun{GEN}{Flm_to_FlxX}{GEN x, long sv,long w} converts the columns of
...
always return \typ{COL} and \kbd{vec} functions always return \typ{VEC}.
