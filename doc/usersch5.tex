...
identify a genuine bottleneck, or you need some special behaviour the generic
...
\subsec{Stack)independant binary objects}

\fun{GENbin*}{copy_bin}{GEN x} copies $x$ into a malloc'ed structure suitable
for stack-independant binary transmission or storage. The object obtained
is architecture independant provided, \kbd{sizeof(long)} remains the same
...
that the binary object is independant of the multiprecision kernel. Slower
than \kbd{copy\_bin}.
...
private low-level routines must be called first to restore absolute adresses.

\fun{void}{shiftaddress}{GEN x, long dec} converts relative adresses to
absolute ones.

\fun{void}{shiftaddress_canon}{GEN x, long dec} converts relative adresses to
...
digits conincide up to the length of the shortest of the two, and the
...
is $x = 0$, in that cas we can deduce that the result is an exact $0$,
...
(as per \kbd{gcmp0}), except possibly the first one. The name comes from
...
$x$ of degree $> 0$ are $0$ (as per \kbd{gcmp0}).
...
\kbd{gcmp0}; in particular, they may have different apparent degrees provided
...
\fun{GEN}{RgX_RgXQ_eval}{GEN f,GEN x,GEN T} returns
...
$T^d$ of the result is not zero (as per \kbd{gcmp0}). The
remainder is $O(T^{d+l})$.
...
is a \typ{INTMOD}, in which cas we return \kbd{Mod(1, N)} for a suitable
...
is inconsistant here, since we somethimes use the prefix \kbd{mp} even though
