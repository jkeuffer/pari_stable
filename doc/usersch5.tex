...
These private functions are more efficient than the library functions
wrappers, but much sloppier on argument checking and damage control. Use them
at your own risk!
...
PARI was originally implemented. The type \tet{ulong} is defined as
\kbd{unsigned long}. Note that in the prototypes below this is sometimes
implicitly typecast to \kbd{int} or \kbd{long}.
...
will be inlined at each invocation by the compiler. If you need to use these
lowest-level operations directly, make sure you know your way through the
PARI kernel sources, and understand the architecture dependencies.
...
\fun{ulong}{addll}{ulong x, ulong y} adds the \kbd{x} and \kbd{y},
returns the lower \B\ bits and puts the carry bit into \kbd{overflow}.

\fun{ulong}{addllx}{int x, ulong y} adds \kbd{overflow} to the sum of the
...
\B\ bits into \kbd{hiremainder}. We must have $1\le\kbd{y}\le\B$.
...
\fun{ulong}{adduumod}{ulong x, ulong y, ulong m} returns the smallest
positive representative of $x + y$ modulo $m$.

\fun{ulong}{subuumod}{ulong x, ulong y, ulong m} returns the smallest
positive representative of $x - y$ modulo $m$.

\fun{ulong}{muluumod}{ulong x, ulong y, ulong m} returns the smallest positive
representative of $x y$ modulo $m$.

\fun{ulong}{invumod}{ulong x, ulong m} returns the smallest
positive representative of $x^{-1}$ modulo $m$. If $x$ is not invertible
mod~$m$, return $0$.

\fun{long}{invsmod}{long x, ulong m} returns the smallest
positive representative of $x^{-1}$ modulo $m$. If $x$ is not invertible
...
it is a little slower than \kbd{invumod}.

\fun{ulong}{divuumod}{ulong x, ulong y, ulong m} returns the smallest
positive representative of $x y^{-1}$ modulo $m$. Ix $y$ is not invertible 
mod $m$, return $0$.

\fun{ulong}{powuumod}{ulong x, ulong n, ulong m} returns the smallest
positive representative of $x^n$ modulo $m$.

\fun{ulong}{powusmod}{ulong x, long n, ulong m} as \kbd{powuumod}, but $n$
...
\noindent
In this section as elsewhere, \kbd{long} denotes a \B-bit signed long integer.
...
is not a \typ{REAL}. When \kbd{x} is an exact~0, this returns
\hbox{\kbd{-HIGHEXPOBIT}}.

\fun{long}{expi}{GEN x} returns the binary exponent of the real number equal
to the \typ{INT}~\kbd{x}. This is a special case of \kbd{gexpo} above.
...
or~1, and if \kbd{x} is a polynomial or a power series, \kbd{s} must be equal
to 0 or~1.
...
\fun{GEN}{realun}{long prec} create a \typ{REAL} equal to $1$ to \var{prec}
words of accuracy.
...
\kbd{x} and \kbd{z} are \typ{INT} or \typ{REAL}).

\fun{void}{affsz}{long s, GEN z} assigns the long \kbd{s} into the \typ{INT}
or \typ{REAL}~\kbd{z}.

\fun{void}{affsi}{long s, GEN z} assigns the long \kbd{s} into the
\typ{INT}~\kbd{z}. This routine assumes that \kbd{lg(z) > 2}.

\fun{void}{affui}{ulong u, GEN z} assigns the unsigned long \kbd{u} into the
\typ{INT}~\kbd{z}. This routine assumes that \kbd{lg(z) > 2}.
...
\misctitle{Warning} the function \kbd{affrs} and \kbd{affri} do not exist. So
don't use them.
...
\fun{long}{itos}{GEN x} converts the \typ{INT} \kbd{x} to a C long if
possible, otherwise raise an exception.

\fun{long}{itos_or_0}{GEN x} converts the \typ{INT} \kbd{x} to a C long if
possible, otherwise return $0$.

\fun{GEN}{stoi}{long s} creates the \typ{INT} corresponding to the long~\kbd{s}.

\fun{GEN}{stor}{long s, long prec} assigns the long \kbd{s} into a
\typ{REAL} of length \kbd{prec} and return the latter.

\smallskip

\fun{GEN}{mptrunc[z]}{GEN x[, GEN z]} truncates the \typ{INT} or
\typ{REAL}~\kbd{x} (not the same as the integer part if \kbd{x} is
\typ{REAL} and negative).

\fun{GEN}{mpent[z]}{GEN x[, GEN z]} integer part of the \typ{INT} or
...
\fun{GEN}{shifts}{long s, long n} converts the long \kbd{s} into a \typ{INT} 
and shifts the value by~\kbd{n}.
...
\fun{int}{cmpsi}{long s, GEN x} compares the long \kbd{s} to the
\typ{INT}~\kbd{x}.

\fun{int}{cmpsr}{long s, GEN x} compares the long \kbd{s} to the
\typ{REAL}~\kbd{x}.
...
\typ{REAL}~\kbd{x} and the long~\kbd{s}.
...
is a \typ{REAL} unless \kbd{x} is an exact $0$. A division-by-zero error
occurs if \kbd{y} is equal to zero.
...
\kbd{x} and \kbd{y} are longs or \typ{INT}s. The result is the true Euclidean
...
dividend (\kbd{res}). All \kbd{GEN}s, whether returned directly or through a
...
remainder is always of the sign of the dividend~\kbd{x}.
...
\typ{REAL}~\kbd{z}, putting the address of the remainder into~\kbd{*r} (unless
\kbd{r} is equal to \kbd{NULL} or \kbd{ONLY\_REM} as above).

\fun{void}{diviiexact}{GEN x, GEN y} returns the Euclidean quotient $\kbd{x}
...
quotient and remainder of the long \kbd{s} by the \typ{INT}~\kbd{x}. As
\kbd{sdivss\_rem} otherwise.

\fun{GEN}{divis_rem}{GEN x, long s, long *r} computes the Euclidean quotient
and remainder of the \typ{INT}~\kbd{x} by the long~\kbd{s}. As
\kbd{sdivss\_rem} otherwise.

\fun{long}{smodis}{GEN x, long y} computes the true Euclidean
remainder of the \typ{INT}~\kbd{x} by the long~\kbd{y}. This is the
...
remainder of the long~\kbd{x} by a \typ{INT}~\kbd{y}.

\fun{ulong}{umodiu}{GEN x, ulong y} computes the true Euclidean
remainder of the \typ{INT}~\kbd{|x|} by the unsigned long~\kbd{y}.

\fun{ulong}{umodui}{ulong x, GEN y} computes the true Euclidean
remainder of the unsigned long~\kbd{x} by the \typ{INT}~\kbd{|y|}.
...
in general not fit into a C \kbd{long}. Use either \kbd{umodui} or
\kbd{modsi}.

\subsec{Miscellaneous functions}

\fun{int}{divise}{GEN x, GEN y} if the \typ{INT}~\kbd{y} divides the
\typ{INT}~\kbd{x}, returns 1 (true), otherwise returns 0 (false).

\fun{int}{mpdivis}{GEN x, GEN y, GEN z} if the \typ{INT}~\kbd{y} divides the
...
The remaining ones are macros, \emph{not} functions (see \secref{se:typecast}
...
\fun{long}{mael2}{GEN x, long $a_1$, long $a_2$} stands for
\kbd{x[$a_1$][$a_2$]}, with all the necessary typecasts. Analogous
macros \tet{mael3}, \tet{mael4}, \tet{mael5} are available.

\subsec{Copying and conversion}

\fun{GEN}{cgetp}{GEN x} creates space sufficient to hold the
$p$-adic~\kbd{x}, and sets the prime $p$ and the $p$-adic precision to those
of~\kbd{x}, but does not copy (the $p$-adic unit or zero representative and
the modulus of)~\kbd{x}.
...
\fun{GEN}{dbltor}{double x} converts the C double \kbd{x} into a \typ{REAL}.

\fun{double}{gtodouble}{GEN x} if \kbd{x} is a real number (not necessarily
of type \typ{REAL}), converts \kbd{x} into a C double if possible.

\fun{long}{gtolong}{GEN x} if \kbd{x} is an integer (not a C long, but not
necessarily of type \typ{INT}), converts \kbd{x} into a C long if possible.
...
into a polynomial with main variable number~\kbd{v}, but vectors are
...
words.% absolutely forbid line brk at hyphen here [GN]
...
\fun{long}{gexpo}{GEN x} returns the binary exponent of \kbd{x} or the
...
\kbd{x} is an \typ{INT}, \typ{REAL} or \typ{FRAC}. Raises an
...
\fun{long}{gcmpsg}{long s, GEN x} comparison of the long \kbd{s}
with~\kbd{x}.
...
\fun{int}{pvaluation}{GEN x, GEN p, GEN *r} applied to non-zero \typ{INT}s
\kbd{x} and~\kbd{p}, returns the highest exponent $e$ such that $\kbd{p}^{e}$
divides~\kbd{x}, creates the quotient $\kbd{x}/\kbd{p}^{e}$ and returns its
address in~\kbd{*r}. In particular, if \kbd{p} is a prime, this returns the
valuation at \kbd{p} of~\kbd{x}, and \kbd{*r} will obtain the
prime-to-\kbd{p} part of~\kbd{x}.
...
\fun{GEN}{mpround}{GEN x} rounds towards~$+\infty$ the \typ{REAL} \kbd{x} to
the nearest integer.
...
except that \kbd{x} is a long.

\fun{GEN}{gdiventgs[z]}{GEN x, long s[, GEN z]}, as \kbd{gdivent}
except that \kbd{y} is a long.
...
as \kbd{diviiround}. Otherwise, as \key{gdivent}.
