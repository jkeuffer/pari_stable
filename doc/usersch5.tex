...
associated callback \tet{cb_pari_ask_confirm} is \kbd{NULL}. Otherwise
calls
\bprog
...
\fun{long}{ZX_pvalrem}{GEN x, GEN p, GEN *r} $x$ being a \kbd{ZX}
(a \typ{POL} with \typ{INT} coefficients), return the min $v$ of the
valuations of its components and set \kbd{*r} to $x/p^v$. Infinite loop
if $x$ is the zero polynomial.

\fun{long}{ZX_pval}{GEN x, GEN p} as \kbd{ZX\_pvalrem} but only returns the
``valuation''.
...
\fun{GEN}{Z_factor_limit}{GEN n, long lim} trial divide $n$ by all primes $p
...
If \kbd{lim} is $\leq 1$, the effect is the same as setting $\kbd{lim} =
...
\fun{int}{ZM_ishnf}{GEN x} return $1$ if $x$ is upper triangular with
positive diagonal coefficients. Assumes $x$ is a square \kbd{ZM} (has integer
coefficients).
...
integers $C$ and $L > 0$. If $\kbd{L}$ is not \kbd{NULL}, set \kbd{*L} to $L$;
if $L = 1$, \kbd{*L} is set to \kbd{gen\_1}. Not suitable for gerepileupto.
...
of degree $> 0$. Finds the largest $L$ such that
...
assuming $V$ was output by \kbd{RgXQ\_powers(x, n, T)}, with $n\geq \deg f$.
Not memory clean, but suitable for \kbd{gerepileupto}.

\fun{GEN}{QX_ZXQV_eval}{GEN f, GEN nV, GEN dV} as \kbd{RgX\_RgXQV\_eval},
except that $f$ is assumed to be a \kbd{QX}, $V$ is given implicitely 
