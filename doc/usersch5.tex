...
\fun{int}{ZXQX_divides}{GEN x, GEN y, GEN T} let $T$ be a monic irreducible
...
format, producing a string $s$ from the remaining arguments.

\noindent We now inspect the list of valid keywords (error classes) for
\kbd{numerr}, and the corresponding required arguments.

\subsec{bugparier} A bug in the PARI library, in function $s$.
\bprog
  pari_err(bugparier, const char *s)
@eprog\noindent prints the error message: \kbd{Bug in $s$, please report}.

\subsec{alarmer} A timeout, generated by the \tet{alarm} function.
\bprog
  pari_err(alarmer, const char *fmt, ...)
@eprog\noindent prints the error message: $s$.

\subsec{openfiler} Error while trying to open a file.
\bprog
  pari_err(openfiler, const char *what, const char *name)
@eprog\noindent prints the error message: \kbd{error opening \emph{what} file:
`\emph{name}'}.

\subsec{talker} Generic uncategorized error.
\bprog
  pari_err(talker, const char *fmt, ...)
@eprog\noindent prints the error message: $s$.

\subsec{flagerr} A flag argument is out of bounds in function $s$.
\bprog
  pari_err(flagerr, const char *s)
@eprog\noindent prints the error message: \kbd{invalid flag in $s$}.

\subsec{impl} A requested feature $s$ is not implemented.
\bprog
  pari_err(impl, const char *s)
@eprog\noindent prints the error message: \kbd{sorry, $s$ is not yet
implemented}.

\subsec{archer} A requested feature is not available on this architecture
or operating system.
\bprog
  pari_err(archer)
@eprog\noindent prints the error message: \kbd{sorry, not available on this
system}.

\subsec{precer} Function $s$ fails because input accuracy is too low.
(As in \kbd{floor(1e100)} at default accuracy.)
\bprog
  pari_err(precer, const char *s)
@eprog\noindent prints the error message: \kbd{precision too low in $s$}.

\subsec{typeer} An argument $x$ of function $s$ had an unexpected type.
(As in \kbd{factor("blah")}.)
\bprog
  pari_err(typeer, const char *s, GEN x)
@eprog\noindent prints the error message: \kbd{incorrect type in $s$
(\typ{$x$})}, where \typ{$x$} is the type of $x$.

\subsec{consister} Data submitted to function $s$ does not match up.
E.g., multiplying matrices of inconsistent dimension, adding vectors of
different lengths,\dots
\bprog
  pari_err(consister, const char *s)
@eprog\noindent prints the error message: \kbd{inconsistent data in $s$}.

\subsec{user} A user error, as triggered by \tet{error}($g_1,\dots,g_n)$.
\bprog
  pari_err(user, GEN g)
@eprog\noindent prints the error message: \kbd{user error:}, then the
entries of the vector $g$.

\subsec{errpile} The PARI stack overflows.
\bprog
  pari_err(errpile)
@eprog\noindent prints the error message: \kbd{the PARI stack overflows !}
as well as some statistics concerning stack usage.

\subsec{memer} A call to \tet{pari_malloc} or \tet{pari_realloc} failed.
\bprog
  pari_err(memer)
@eprog\noindent prints the error message: \kbd{not enough memory}.

\subsec{overflower} An object in function $s$ becomes too large to be
...
\bprog
  pari_err(overflower, const char *s)
@eprog\noindent prints the error message: \kbd{overflow in $s$}.

\subsec{primer1} A function using the precomputed list of prime numbers
ran out of primes.
\bprog
  pari_err(primer1, ulong c)
...
\subsec{constpoler} An argument of function $s$ is a constant polynomial,
which does not make sense. (As in \kbd{galoisinit(Pol(1))}.)
\bprog
  pari_err(constpoler, const char *s)
@eprog\noindent prints the error message: \kbd{constant polynomial in $s$}.

\subsec{redpoler} Function $s$ expected an irreducible polynomial,
and did not receive one. (As in \kbd{nfinit(x\pow2-1)}.)
\bprog
  pari_err(redpoler, const char *s, GEN x)
@eprog\noindent prints the error message: \kbd{not an irreducible polynomial
in $s$ : $x$}.

\subsec{zeropoler} An argument of function $s$ is a zero polynomial, which
does not make sense. (As in \kbd{polroots(0)}.)
\bprog
  pari_err(zeropoler, const char *s)
@eprog\noindent prints the error message: \kbd{zero polynomial in $s$}.

\subsec{operf} Forbidden operation between two objects than cannot be
typecast to a sensible common domain, because their types do not match up.
(As in \kbd{Mod(1,2) + Pi}.)
\bprog
  pari_err(operf, const char *op, GEN x, GEN y)
...
\emph{addition} for \kbd{"+"}. If \var{op} is not in the above operator,
list, it is taken to be the already spelled out name of a function, e.g.
\kbd{"gcd"}, and the error message becomes \kbd{forbidden} \var{op} \typ{$x$},
\typ{$y$}.

\subsec{operi} Impossible operation between two objects than cannot be
...
\bprog
  pari_err(operf, const char *op, GEN x, GEN y)
@eprog\noindent As \kbd{operf}, replacing \kbd{forbidden} by
\kbd{inconsistent}.

\subsec{gdiver} Tried to invert a non-invertible object, which
is not a \typ{INTMOD}. Indeed, in the latter case, we raise \kbd{invmoder}
instead.
\bprog
  pari_err(gdiver)
...
\subsec{notfuncer} Generated by the PARI evaluator; tried to use a
\kbd{GEN} which is not a \typ{CLOSURE} in a function call syntax (as in
\kbd{f = 1; f(2);}).
\bprog
  pari_err(notfuncer, GEN fun)
@eprog\noindent prints the error message: \kbd{not a function in a function
call}.

\subsec{syntaxer} Syntax error, generated by the PARI parser.
\bprog
  pari_err(syntaxer, const char *msg, const char *e, const char *entry)
