...
need the full GP analyzer, you should be fine with

\fun{void}{pari_init}{size\_t parisize, ulong maxprime} initialize the
library, with a stack of \kbd{parisize} bytes and a prime table
...
\fun{void}{pari_init_opts}{size\_t parisize, ulong maxprime, ulong
init\_opts, entree **newfun, entree **oldfun} as \kbd{pari\_init}, with
extra flexibility. \kbd{init\_opts} is a mask of flags among the
following:
...
This one \emph{must} be enabled once. If you close pari in the meantime,
it is not mandatory to reinitialize it (and last values are restored).

Finally, \kbd{newfun} and \kbd{oldfun} are list of modules containing
extra routines which you want to make known to the GP analyzer. You
probably want to set them to \kbd{NULL} (no extras). The calculator
\kbd{gp} uses \kbd{pari\_addfunctions} to bundle a series of new routines
in a module list, to add high level gp-specific routines. If you want to
do the same, \kbd{newfun} lists the routines, and \kbd{oldfun} the
compatibility routines which are used when the compatible default is set.
...
\fun{void}{pari_addfunctions}{entree ***list, entree *func, char **help}
add list of routines \kbd{func} list of modules \kbd{*list} (which is
allowed to be \kbd{NULL} for an empty list).
