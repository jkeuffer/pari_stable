...
\fun{GEN}{ZX_sqr}{GEN x,GEN p} returms $\kbd{x}^2$. 
...
\fun{GEN}{FpX_sqr}{GEN x,GEN p} returms $\kbd{x}^2$. 
...
\fun{GEN}{dummycopy}{GEN x} returns a \typ{GEN} whose components are the
...
\fun{GEN}{dummytrans}{GEN x} returns the transpose of $x$, \emph{without}
...
\fun{GEN}{dummyconcat}{GEN x, GEN y} concatenate $x$ and $y$, \emph{without}
...
empty matrix. See \tet{dummyconcat}.
...
\fun{GEN}{rowextract_i}{GEN A, long i1, long i2} return the \typ{MAT}
formed by the $i_1$-th through $i_2$-th rows of \typ{MAT} $A$. Assume $i_1
\leq i_2$.

\fun{GEN}{rowextract_p}{GEN A, GEN p}, $p$ being a \typ{VECSMALL}
representing a list $[p_1,\dots,p_n]$ of rows of \typ{MAT} $A$, returns the
matrix whose rows are $A[p_1,],\dots, A[p_n,]$.

\fun{GEN}{rowextract_ip}{GEN A, GEN p, long x1, long x2}, short for
\bprog
  rowextract_i(rowextract_p(A,p), x1, x2)
@eprog\noindent
(more efficient).

\fun{GEN}{vecextract_i}{GEN A, long j1, long j2}, return $A[j_1], \dots,
...
\fun{GEN}{vecextract_p}{GEN A, GEN p} $p$ is a \typ{VECSMALL} representing
...
\fun{GEN}{vecextract_ip}{GEN A, GEN p, long y1, long y2} short for 
\bprog
  vecextract_i(vecextract_p(A,p), y1, y2)
@eprog\noindent
(more efficient).
