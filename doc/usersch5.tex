...
\fun{GEN}{RgX_disc}{GEN x} returns the discriminant
of the \typ{POL} \kbd{x} with respect to its main variable.
...
flow change. When that occurs, the functions below generally returns \kbd{NULL}.
...
The permutation groups facility is meant to deal with subgroups of Galois
groups obtained by \tet{galoisinit} and thus is currently limited to weakly
super-solvable groups.

A permutation group \var{grp} is a subgroup of a transitive subgroup of $S_n$
for some $n$ which is called below the \emph{domain} of the subgroup.  The
trivial group does not have a domain.

The permutation group \var{grp} is represented by a \typ{VEC} with two
components:
...
permutation group or a Galois group. Returns the underlying permutation
group and set \var{elts} to the list of elements or to \kbd{NULL} if it is not
known.

\fun{GEN}{galois_group}{GEN gal} return the underlying permutation group of the
...
\emph{non-trivial} permutation group \var{grp}.

\fun{GEN}{group_elts}{GEN grp, long n} returns the list of elements of the
permutation group \var{grp} of domain \var{n} as permutations.

\fun{long}{group_order}{GEN grp} returns the order of the permutation group
\var{grp} (which is the product of the relative orders).

\fun{long}{group_isabelian}{GEN grp} returns $1$ the the permutation group
...
\fun{long}{group_isA4S4}{GEN grp} return $1$ if the permutation group
...
\fun{GEN}{group_leftcoset}{GEN G, GEN g} where $G$ is a
permutation group and $g$ a permutation of the same domain,
the the left coset $gG$ as a vector of permutation.

\fun{GEN}{group_rightcoset}{GEN G, GEN g} where $G$ is a
permutation group and $g$ a permutation of the same domain,
the the right coset $Gg$  as a vector of permutation.

\fun{long}{group_perm_normalize}{GEN G, GEN g} where $G$ is a
permutation group and $g$ a permutation of the same domain, return $1$ if 
$gGg^-1=G$, else $0$.

\fun{GEN}{group_quotient}{GEN G, GEN H}, where $G$ is a permutation group and
...
quotient group $G/H$ as a permutation group.
...
permutation group \var{grp}.

\fun{GEN}{groupelts_abelian_group}{GEN elts} where \var{elts} is the list of
elements of an \emph{Abelian} permutation group, returns the corresponding
permutation group.

\fun{GEN}{groupelts_center}{GEN elts} where \var{elts} is the list of elements
of a permutation group, returns the list of elements of the center of the
group.

\fun{GEN}{group_export}{GEN grp, long format} exports a permutation group
to another format, see \tet{galoisexport}.

\fun{long}{group_ident}{GEN grp, GEN elts} returns the GAP4 index of the group
\var{grp}, see \tet{galoisidentify}. If \var{elts} is no \kbd{NULL}, it must be
