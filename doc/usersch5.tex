...
\fun{GEN}{ZV_neg}{GEN x} returns $-\kbd{x}$.

\fun{void}{ZV_neg_inplace}{GEN x} negates \kbd{x} in place, by replacing
each component by its opposite. If you want to save even more memory by
...
sign of its integer components. Universal constants \kbd{gen\_1}, \kbd{gen\_m1}
and \kbd{gen\_2} are handled specially and will not be corrupted.
...
\fun{GEN}{ZV_content}{GEN v} returns the GCD of all the components
of~\kbd{x}.
...
from \kbd{d = ZM_snfall(x, \&U, \&V)}, where $U$ or $V$ may be \kbd{NULL},
