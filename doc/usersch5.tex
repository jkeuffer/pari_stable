...
\subsec{Functions specific to the GP interpreter}
...
in \kbd{functions\_basic}. You may declare further functions on a cas by
...
opts, growarray MOD, \hskip 0pt plus 5pt growarray OLD} as
\kbd{pari\_init}, with extra flexibility. \kbd{opts} is a mask of flags
among the following:
...
made known to the GP analyzer besides the general ones. You may set 
them to \kbd{NULL} (no extras). The calculator \kbd{gp} adds gp-specific
routines. If you want to do the same, \kbd{MOD} lists the extra routines,
and \kbd{OLD} obsolete routines which are used when the \kbd{compatible}
default is set.
