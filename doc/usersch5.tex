...
\fun{long}{ndec2nlong}{long x} convert a number of decimal digits to a number
...
codewords, i.e 2 + \kbd{nbits2nlong(x)}.
...
\fun{ulong}{Fl_gener}{ulong p} returns a \idx{primitive root} modulo \kbd{p},
...
\fun{GEN}{utoineg}{ulong s} converts the \empn{non-zero} \kbd{ulong}~\kbd{s}
into a \typ{INT} and return the latter.
...
individually with \kbd{cgiv(R)}. If the remainder is $0$ set \kbd{R = gzero}.
...
\fun{GEN}{FqV_to_FlxC}{GEN x, GEN T, GEN p}, converts coefficients in Fq to
coefficient in Flx, result being a column vector.

\fun{GEN}{FqM_to_FlxM}{GEN x, GEN T, GEN p}, converts coefficients in Fq to
coefficient in Flx.
...
\fun{GEN}{FlxV_to_ZXC}{GEN x}, converts a vector of \kbd{Flx} to a column
...
\fun{GEN}{Flm_id}{long n} creat an \kbd{Flm} which is an $n \times n$
identity matrix.
...
(\typ{INT}, \typ{REAL}, \typ{FRAC}, \typ{COMPLEX} or \typ{QUAD}) to either
...
\fun{GEN}{RgX_to_RgM}{GEN v, long N} converts the vector of \kbd{RgX}~\kbd{v}
...
\fun{GEN}{vecsmall_const}{long n, long c} returns a \typ{VECSMALL}
...
\subsec{Functions to handle bit-vectors}
