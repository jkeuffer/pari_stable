...
Return the group structure $D$ of the group $E(\F_p)$,
which is assumed to be of order $N$ and set $*pt_m=m$.

\fun{GEN}{Fp_ellgens}{GEN a4, GEN a6, GEN ch, GEN D, GEN m, GEN p}
Returns generators of the group $E(\F_p)$ with the base change \kbd{ch}
(see \kbd{FpE\_changepoint}), where $D$ and $m$ are as returned by
\kbd{Fp\_ellgroup}.
