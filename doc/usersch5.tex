...
\fun{ulong}{sqrtumod}{ulong x, ulong p} returns the square root of \kbd{x}
...
\fun{ulong}{powiumod}{GEN x, ulong n, GEN m} raises \kbd{x} to the \kbd{n}-th
power modulo \kbd{p} (smallest non-negative residue).

\fun{GEN}{powmodulo}{GEN x, GEN n, GEN m} returns $\kbd{x}^\kbd{n}$
modulo \kbd{p} (smallest non-negative residue).

\fun{GEN}{mpinvmod}{GEN a, GEN m} returns an inverse of \kbd{a} modulo \kbd{m}
(smallest non-negative residue). Raise an error if \kbd{a} is not invertible.

\fun{GEN}{mpinvmodsafe}{GEN a, GEN m} as \kbd{mpinvmod}, but return
...
\fun{GEN}{mpsqrtmod}{GEN x, GEN p} returns a square root of \kbd{x} modulo
...
\fun{GEN}{mpsqrtnmod}{GEN x, GEN n, GEN p, GEN *zn} returns an \kbd{n}-th
...
depending on the function name. (All \kbd{Fq} routines accept \kbd{FqXQ}s by
...
reduce compoents mod \kbd{p} to \kbd{FpX}s or \kbd{Fp}s.)
...
coefficients are \kbd{FqX}s. So the only difference between \kbd{FqX} and
...
\fun{GEN}{Flx_redmontgomery}{GEN x, GEN mg, GEN T, ulong p}, returns
...
\fun{GEN}{Flx_polx}{long sv} returns the variable $v$ as degree~1~\kbd{Flx}.

\fun{GEN}{Flx_zero}{long sv} returns a zero \kbd{Flx} in variable $v$.
