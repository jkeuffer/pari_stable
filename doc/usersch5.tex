...
\kbd{git-}\emph{commit} if compiled from a git repository, etc.
...
\fun{void}{pariOut_putc}{PariOUT *out, char c} essentially equivalent to
\kbd{out->putc(c)}. In addition, registers whether the last character printed
was a \kbd{\bs n}.

\fun{void}{pariOut_puts}{PariOUT *out, const char *s} essentially equivalent to
\kbd{out->puts(s)}. In addition, registers whether the last character printed
was a \kbd{\bs n}.

The function \kbd{pariOut\_flush} does not exist since it would be identical
to \kbd{out->flush()}

\fun{void}{pariOut_printf}{PariOUT *out, const char *fmt, ...}

\fun{void}{pariOut_vprintf}{PariOUT *out, const char *fmt, va_list ap}

\fun{int}{pari_last_was_newline}{void} returns a non-zero value if the last
character printed via \tet{pariOut_putc} or \tet{pariOut_puts} was \kbd{\bs
n}, and $0$ otherwise.
...
\subsec{Default output context} It is defined by the global variable
\tet{pariOut} for normal output, and you probably do not want to change it.
If you \emph{do} intend to change \kbd{pariOut}, diverting output in
non-trivial ways, this probably means that you are rewriting \kbd{gp}.
For completeness, we document in this section what the default output context
does, and how it can be manipulated (without being rewritten and replaced by
something else altogether).

The defaut value writes output to the \kbd{FILE*} \tet{pari_outfile}
(initialized to \tet{stdout}).  The low-level methods are actually the
standard \kbd{putc} / \kbd{fputs}, plus some magic to handle a log file if
one is open. There is also a default context associated to errors and
warnings, called \kbd{pariErr}, which prints to the \kbd{FILE*}
\tet{pari_errfile} (initialized to \tet{stderr}).

Still keeping the default output context you can change
...
\fun{void}{pariOut_term_color}{PariOUT *out, long c} as \tet{term_color},
...
\fun{long}{closure_context}{long start} saves and restore the evaluation
context in the recursive debugger. Used to assign the correct values to
lexically scoped variables in a given frame.

\fun{void}{closure_err}{void} prints a backtrace of all stack frames.

\subsec{Wrappers for pseudo-iterators}
Pseudo-iterators like \kbd{intnum} or \kbd{sumnum} take as their first two
argument a context \kbd{void *E}, and an evaluation function
\kbd{GEN(*eval)(void*,GEN)}, called at each evaluation point, and returning
the function value (taking the context into account if necessary). The
following functions are trivial wrapper around closure evaluation routines:
they interpret their first argument (context) as a closure, via a simple
typecast. Then evaluate it at their second argument. (They are for
instance used by the \kbd{gp2c} compiler.)

\fun{GEN}{gp_call}{void *E, GEN x} calls \tet{closure_callgen1}.

\fun{long}{gp_callbool}{void *E, GEN x} calls \tet{closure_callgen1};
returns \kbd{true} if its result is non-zero, and \kbd{false} otherwise.

\fun{long}{gp_callvoid}{void *E, GEN x} calls \tet{closure_callvoid1}, and
returns the value of \tet{loop_break}.

\fun{GEN}{gp_eval}{void *E, GEN x} calls \tet{closure_evalnobrk}.

\fun{long}{gp_evalvoid}{void *E, GEN x} calls \tet{closure_evalvoid}, and
returns the value of \tet{loop_break}.
...
\fun{GEN}{sd_ulong}{const char *v, long flag, const char *s, ulong *ptn, ulong Min, ulong Max, const char **msg}
