...
\fun{GEN}{cgetg}{long l, long t} allocates (the root of) a \typ{GEN}
...
\fun{GEN}{cgetg_copy}{long l, GEN x} fast version of \kbd{cgetg}: assuming
\kbd{lg(x)} is $l$, allocate a \kbd{GEN} with the same type and length as
$x$. This is a little faster than \kbd{cgetg} since we may reuse the bitmask
in $x[0]$ instead of recomputing it, and we do not need to check that $l$
does not overflow the possibilities of the implementation.
...
\fun{GEN}{gclone}{GEN x} creates a new permanent copy of $x$ on the heap. The
\emph{clone bit} of the result is set.

\fun{void}{gunclone}{GEN x} delete the clone~$x$ (created by \kbd{gclone}).
...
is still provided for bacward compatibility.
...
\fun{ulong}{minuu}{ulong x, ulong y} returns the \kbd{min} of $x$ and $y$.

\fun{ulong}{maxuu}{ulong x, ulong y} returns the \kbd{max} of $x$ and $y$.
...
\fun{GEN}{sqrr}{GEN x} squares the \typ{REAL}~\kbd{x}
...
\fun{GEN}{powrs}{GEN x, long n}, assumes $x$ is a \typ{REAL} and returns $x^n$
...
\fun{GEN}{Flm_hess}{GEN x, ulong p} characteristic polynomial of $x$.
...
\fun{GEN}{Flxq_conjvec}{GEN x, GEN T, ulong p} returns the vector of conjugates
$[x,x^p,x^{p^2},\ldots,x^{p^{n-1}}]$ where $n$ is the degree of $T$.
...
$l$ one iterations outputs a result correct to accuracy $2l$.
...
\noindent Only require that \kbd{mul} by an associative binary operator,
which need not correspond to a true multiplication. \kbd{D} is meant to contain 
...
\fun{GEN}{leftright_pow}{GEN x, GEN n, void *D, F1 sqr, F2 mul} $n > 0$ a
...
\kbd{mul(D, $x$, $x$)}. Uses a left-right binary powering algorithm.

\fun{GEN}{leftright_pow_u}{GEN x, ulong n, void *D, F1 sqr, F2 mul} $n > 0$,
returns $x^n$. See \tet{leftright_pow}.

\fun{GEN}{leftright_pow_fold}{GEN x, GEN n, void *D, F1 sqr, F1 msqr} as
\tet{leftright_pow}, \kbd{mul} being replaced by \kbd{msqr}, with \kbd{msqr(D,
...
\noindent \kbd{mul(D, $x$, $x$)} and \kbd{pow(D, $x$, $n$)}
...
\fun{void}{vec_isconst}{GEN v, GEN x} $v$ a pre-existing vector. Set all its
components to $x$.
...
low-level function underlying \kbd{default0}. If $s$ is empty, call all
default setting functions with empty string argument and flag
