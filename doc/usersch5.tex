...
\fun{GEN}{RgM_Rg_div}{GEN x, GEN y}

\fun{GEN}{RgM_Rg_mul}{GEN x, GEN y}
...
documentation of \kbd{qflll0} and \kbd{qflllgram0} for details:
...
\fl is an or-ed combination of the following flags
...
\item  \tet{LLL_KEEP_FIRST}. The first vector in the output basis іs the same
...
\fun{GEN}{lllintpartial}{GEN x} as \kbd{lllintpartial\_inplace} (slightly
slower), but returns the base change matrix $U$ from the canonical basis to
the $b_i$, i.e. $x U$ is the output of \kbd{lllintpartial\_inplace}.
