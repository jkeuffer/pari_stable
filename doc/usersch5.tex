...
\fun{GEN}{RgM_solve}{GEN a, GEN b} returns $a^{-1}b$ where $a^{-1}$
denotes a \emph{left} inverse of $a$ and $b$ is a \typ{COL} or \typ{MAT}.
Returns \kbd{NULL} if $a^{-1}$ cannot be computed, see \tet{RgM_inv}. Note
that if $a$ is not invertible, 
