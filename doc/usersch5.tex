...
or \kbd{NULL} if the evaluation was aborted (\kbd{break}).

\fun{GEN}{closure_evalnobrk}{GEN code} as \kbd{closure\_evalgen} but raise
an exception if the evaluation was aborted. Meant for pseudo-iterators where
interrupting the closure ins meaningless, e.g.~\kbd{intnum} or \kbd{sumninm}.
...
\fun{GEN}{closure_trapgen}{long numerr, GEN code} evaluate closure, while
traping error \kbd{numerr}. Return \kbd{NULL} if error trapped, and the result
otherwise..
