...
\fun{GEN}{FpM_inverseimage}{GEN A, GEN B, GEN p} returns a \typ{MAT} $X$ such
that $A X = B$ if one such exists, and \kbd{NULL} otherwise.
...
\fun{GEN}{Flm_inverseimage}{GEN A, GEN B, ulong p} returns a \typ{MAT} $X$
such that $A X = B$ if one such exists, and \kbd{NULL} otherwise.
...
\fun{GEN}{RgM_inverseimage}{GEN A, GEN B} returns a \typ{MAT} $X$ such that
