...
\fun{long}{FpXQ_issquare}{GEN x, GEN T, GEN p} returns $1$ if $x$ is a square
...
\fun{long}{Flxq_issquare}{GEN x, GEN T, ulong p} returns $1$ if $x$ is a square
and $0$ otherwise. Assumes that \kbd{T} is irreducible mod \kbd{p}.
...
\fun{long}{ZV_isscalar}{GEN x} as \kbd{RgV\_isscalar}, assuming $x$ is a
\kbd{ZV}.
...
\fun{long}{RgM_isdiagonal}{GEN x} return 1 is the \typ{MAT} $x$ is a
...
\fun{char*}{pari_strdup}{const char *s}

\fun{char*}{pari_strndup}{const char *s, long n}
...
The name returned is pari_malloc'ed.
