...
\fun{ulong}{gener_Zl}{ulong p} returns a primitive root modulo $p^k$, $k
> 1$, assuming $p$ is an odd prime. On a 64-bit machine,
this function may fail and raise an exception, if $p > 2^{63}$; namely 
when $g := \kbd{gener\_Fl}(p)$ is not a primitive element and $g + p$ no
...
\fun{int}{dvdis}{GEN x, long y} if the \typ{long}~\kbd{y} divides the
\typ{INT}~\kbd{x}, returns 1 (true), otherwise returns 0 (false).

\fun{int}{dvdiu}{GEN x, ulong y, GEN z} if the \typ{ulong}~\kbd{y} divides
the \typ{INT}~\kbd{x}, returns 1 (true), otherwise returns 0 (false).
...
\fun{GEN}{gener_Zp}{GEN p} returns a primitive root modulo $p^k$, $k > 1$,
