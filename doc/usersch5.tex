...
$|\kbd{x}| / \kbd{y}$ (note the absolute value!), assuming $\kbd{y}$ divides
...
\fun{void}{mpbern}{long n, long prec} precomputes even \idx{Bernoulli} numbers
$B_0,\dots,B_{2n-2}$ as \typ{REAL} of precision \kbd{prec}.
...
\noindent Possible letters are:
  ...
  \kbd{Y}: polynomial in $Y\neq X$. This is only used to resolve ambiguity. 
...
This function is suitable for \kbd{gerepileupto} but not memory-clean.
...
\fun{GEN}{Flx_rand}{long d, long sv, ulong p} returns a random \kbd{Flx}
in variable \kbd{v}, of degree less than~\kbd{d}.
...
$x$ is a \typ{INT}, \typ{REAL}, \typ{FRAC} or \typ{COMPLEX}. The result in
...
\subsubsec{Generic routines}

The following routines allows to use an arbitrary comparison function
\kbd{int (*cmp)(void* data, GEN x, GEN y)}
it is assumed that \kbd{cmp(data,x,y)} returns a negative result if $x < y$,
a positive one if $x > y$ and 0 if $x = y$. The \kbd{data} argument is there
in case your \kbd{cmp} requires additional context.

\fun{GEN}{gen_sort}{GEN x, void *E, int (*cmp)(void *,GEN,GEN)}, as \kbd{sort}.

\fun{GEN}{gen_indexsort}{GEN x, void *E, int (*cmp)(void*,GEN,GEN)},
as \kbd{indexsort}.

\fun{void}{gen_sort_inplace}{GEN x, void *E, int (*cmp)(void*,GEN,GEN), GEN
...
assuming \kbd{y} is a factorization matrix, sorts its rows according to the
comparison function \kbd{cmp} applied to its first column.
