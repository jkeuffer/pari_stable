...
\fun{GEN}{realmun}{long prec} create a \typ{REAL} equal to $-1$ to \kbd{prec}
words of accuracy.
...
returns the \kbd{FpV} obtained by applying \kbd{Rg\_to\_Fp} coefficientwise.
...
quotient of the \kbd{Flx}~\kbd{A} by $X - \kbd{a}$, and sets \kbd{rem} to the
remainder $ \kbd{A}(\kbd{a})$.
...
components set to \kbd{gen\_0}, but the \kbd{i}-th one which us set to
\kbd{C}.
