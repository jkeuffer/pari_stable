...
\noindent In the sequel, we use sequences of arguments of the form \kbd{const
char *fmt, ...}, where \kbd{fmt} is a PARI format, producing a string $s$
from the remaining arguments.

\noindent The list of valid keywords (error classes) for \kbd{numerr} is as
follows:

\item \tet{bugparier}: a bug in the PARI library.
\bprog
  pari_err(bugparier, const char *fmt, ...)
@eprog\noindent prints the error message: \kbd{Bug in $s$, please report}.

\item \tet{alarmer}: a timeout, generated by the \tet{alarm} function.
\bprog
  pari_err(alarmer, const char *fmt, ...)
@eprog\noindent prints the error message: $s$.

\item \tet{openfiler}: error while trying to open a file.
\bprog
  pari_err(openfiler, const char *what, const char *name)
@eprog\noindent prints the error message: \kbd{error opening \emph{what} file:
`\emph{name}'}.

\item \tet{talker}: generic uncategorized error.
\bprog
  pari_err(talker, const char *fmt, ...)
@eprog\noindent prints the error message: $s$.

\item \tet{flagerr}: a flag argument is out of bounds.
\bprog
  pari_err(flagerr, const char *fun)
@eprog\noindent prints the error message: \kbd{invalid flag in \emph{fun}}.
...
\item \tet{notfuncer}: generated by the PARI evaluator; tried to use a
\kbd{GEN} which is not a \typ{CLOSURE} in a function call syntax (as in
\kbd{f = 1; f(2);}).

\noindent Prints the error message: \kbd{not a function in a function call}.

\item \tet{syntaxer}: syntax error, generated by the PARI parser.
\bprog
