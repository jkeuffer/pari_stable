...
Assume that $0 \leq x < m$, $\kbd{amax} \geq 0$, and $\kbd{bmax} > 0$ are
\typ{INT}s, and that $2 \kbd{amax} \kbd{bmax} < m$. Attempts to recognize
$x$ as a rational $a/b$, i.e. to find \typ{INT}s $a$ and $b$ such that
...
unchanged; otherwise it returns $1$ and sets $a$, $b$.
...
call \tet{Fp_ratlift} after explicitly checking all preconditions.
...
\kbd{FpM\_ratlift}, for an \kbd{FpX} $P$.
