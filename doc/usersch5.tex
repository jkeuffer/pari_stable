...
\fun{GEN}{Z_issmooth}{GEN n, ulong lim} returns \kbd{NULL} if a prime
...
returns the decision the factoring engine would take abount the compositeness
...
\fun{GEN}{Flx_double}{GEN y, ulong p} returns $2y$.

\fun{GEN}{Flx_triple}{GEN y, ulong p} returns $3y$.
...
\fun{GEN}{zv_neg}{GEN x} negates $x$ in place and return it. No check for
overflow is done, which occurs in the fringe case where an entry is equal to
$2^{\B-1}$.
