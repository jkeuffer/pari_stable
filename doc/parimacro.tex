...
\ifgphelp % disable crossreferences
  ...
  \def\label#1{}
  \def\ref#1{[{\bf ??}]}
\else % start of non-gphelp section
...
{ \let\g=\gdef \global\let\EFF=f \catcode`\f=13
  \gf{\EFF{}} % f won't produce any ligature in verbatim mode
}
...
% argument goes to index (silent)
\def\sidx{\gdef\f@nt{rm}\let\do=\toindex\doverb}
\def\kbdsidx{\gdef\f@nt{tt}\let\do=\toindex\doverb}

% argument to index + set up as plain text
...
\def\unixsubsecunix#1{{\let\do=\subsec\@secunix{#1}}}
...
\def\subsubsecidx{\gdef\f@nt{tt}\let\do=\@subsecidx\doverb}
