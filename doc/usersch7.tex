...
is defined, one of
  ...
  \tet{t_ELL_Fp} a prime finite field.

  \tet{t_ELL_Fq} a non-prime finite field.
...
\fun{GEN}{ellffmul}{GEN E, GEN P, GEN n} returns $[n]P$, assuming that
\tet{ell_get_type}$(E)$ is either \tet{t_ELL_Fp} or \tet{t_ELL_Fq}.
...
a non-prime finite field $\F_q$, ignore $p$ and return $q+1 - \#(\F_q)$.
...
\kbd{ellap}: same conditions as above for $E$, except that curves over
non-prime finite fields are not allowed. The extra flag
...
In this subsection, $E$ is assumed to be defined over a \emph{non-prime}
finite field $\F_q$, i.e. \tet{ell_get_type}$(E)$ is \tet{t_ELL_Fq}.
