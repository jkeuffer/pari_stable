...
\kbd{neg(E,x,y)} returns $-x$.

\kbd{inv(E)} returns the inverse of $x$.

\kbd{equal0(x)} $x$ being in canonical form, returns one if $x=0$ and zero otherwise.

\kbd{s(x)} $x$ being a small signed integer, returns $s$ times the unit element.

A field is thus described by a \kbd{struct bb\_field} as above and auxiliary
data typecast to \kbd{void*}. The following functions operate on black-box
groups:
...
\fun{void}{out_print0}{PariOUT *out, GEN sep, GEN g, long flag} as
