...
A \teb{GP2C array} is generic way to manage stacks of data that need
to grow dynamically.


\subsec{Initialization}

Let suppose we want to create a stack of objects of type \kbd{foo}.
...
The following function push an element on the stack.
\bprog
void push_foo(foo o_foo)
{
...
\bprog
void swapfoo(long n, long m)
{
  if (n>s_foo.n || m>s_foo.n)
    pari_err(bugparier,"swapfoo");
  foo o_foo=t_foo[n];
  t_foo[n]=t_foo[m];
  t_foo[m]=o_foo;
}
...
Let \kbd{s} a GP2C stack and \kbd{data} the data linked to it. The following
public field are defined:

\item \kbd{s.alloc} is the number of elements allocated for \kbd{data}.

\item \kbd{s.n} is the number of elements in the stack.
\kbd{s.n} can be changed as long as $0\leq\kbd{s.n}\leq\kbd{s.alloc}$
holds.

\fun{void}{stack_init}{gp2c_stack *s, size_t size, void **data} links the GP2C
array \kbd{s} to the data pointer \kbd{*data}, where \kbd{size} is the size of
data element. The pointer \kbd{*data} is set to \kbd{NULL}, \kbd{s->n} and
\kbd{s->alloc} are set to $0$.

\fun{void}{stack_alloc}{gp2c_stack *s, long nb} make sure that $\kbd{s.alloc}\geq\kbd{s.n}+nb$ eventually reallocating \kbd{data}.

\fun{long}{stack_new}{gp2c_stack *s} increase \kbd{s.n} by one unit,
eventually reallocating \kbd{data}, and return \kbd{s.n-1}.

The following construction is incorrect because \kbd{stack\_new} can change the
value of \kbd{t\_foo}:
\bprog
t_foo[stack_new(&s_foo)]=o_foo;
@eprog

\fun{void}{stack_delete}{gp2c_stack *s} free  \kbd{data} and reset the stack
to the state after \kbd{stack\_init} (\kbd{s->n} and \kbd{s->alloc} are set to
$0$).

\fun{void **}{stack_base}{gp2c_stack *s} return \kbd{data}, casted to a
\kbd{void **}.

\fun{void *}{stack_pushp}{gp2c_stack *s, void *u} This function assumes that
\kbd{*data} is of pointer type. Push the element \kbd{u} on the stack \kbd{s}.
...
\item \kbd{z[6]} points to a \typ{VEC} with two components which are \typ{STR}.
...
\item \kbd{z[7]} points to a \typ{VEC} which hold the values of all lexical
...
stored in the second component of \kbd{z[6]} for function or true closures.
...
\kbd{cmp1(x)} returns one if $x$ is the neutral element in the group,
and zero otherwise.
...
These functions implement GP functionalities for which the C language or
...
may be generated by \kbd{gp2c}. We document them here for completess.

\subsec{Output}

\fun{void}{print}{GEN g}

\fun{void}{print1}{GEN g} use \tet{pari_printf} or \tet{output}.

\fun{void}{printtex}{GEN g} use \tet{GENtoTeXstr} and \tet{pari_printf}.
...
\fun{void}{localvars_unpack}{GEN pack} for each pair in \kbd{pack}, assign
the value to the variable. Used by \tet{geval} in GP.

\fun{long}{localvars_find}{GEN pack, entree *ep} does \kbd{pack} contain
a pair whose variable corrsesponds to \kbd{ep} ? If so, where is the
...
GP script named \kbd{fun}. Make sure it produces some useful ouput and tests
...
\kbd{make test-ploth} or just much longer than the recommanded minute, you
