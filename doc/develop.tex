...
is implemented by a \kbd{GEN} of length 3: two codewords and one component
containing the actual entries in a malloc'ed vector (so that it can be
realloc'ated without moving the parent \kbd{GEN}). All the entries are
clones, possibly with cloned subcomponents (the must be deleted with
\kbd{killbloc}, not \kbd{gunclone}). This is in practice considered as a
non-recursive type, i.e. copies from \kbd{gcopy} are shallow. Of course, this
...
\kbd{l = lg(v)}, then the elements are \kbd{v[1]}, \dots, \kbd{v[l-1]}.
