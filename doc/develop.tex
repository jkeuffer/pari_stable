...
A \teb{GP2C array} is a generic way to manage stacks of data that need
to grow dynamically. It allocates memory using \kbd{pari\_malloc}, and is
independant of the PARI stack; it even works before the \kbd{pari\_init} call.
...
Let \kbd{s} be a GP2C stack and \kbd{data} the data linked to it. The following
...
\fun{void}{stack_alloc}{gp2c_stack *s, long nb} makes room for \kbd{nb} more
elements, i.e.~makes sure that $\kbd{s.alloc}\geq\kbd{s.n} + \kbd{nb}$, possibly
reallocating \kbd{data}.

\fun{long}{stack_new}{gp2c_stack *s} increases \kbd{s.n} by one unit,
...
\fun{void}{stack_delete}{gp2c_stack *s} frees \kbd{data} and resets the stack
to the state immediately following \kbd{stack\_init} (\kbd{s->n} and
\kbd{s->alloc} are set to $0$).

\fun{void **}{stack_base}{gp2c_stack *s} returns \kbd{data}, typecast to a
\kbd{void **}.
