...
(return value $-1$, $0$ or $1$) (\kbd{universal\_cmp} is suitable for this field).
...
\subsection{Black box groups with pairing}

Theses functions handle group of rank at most $2$ equipped with a family of
...
returns the elementary divisorrs $[d_1, d_2]$ of the group, assuming it is of order exactly
...
pairing required to verify a generating set which is to be used with \kbd{gen_ellgens}.

\fun{GEN}{gen_ellgens}{GEN d1, GEN d2, GEN m, void *E, const struct bb_group *grp,
             GEN pairorder(void *E, GEN P, GEN Q, GEN m, GEN F)}
the parameters $d_1$, $d_2$, $m$ being as returned by gen_ellgroup, returns a pair
