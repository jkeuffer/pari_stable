...
\kbd{ellheight} assumes that
\kbd{e} is \emph{minimal}!) This is non-zero, hence \kbd{q} is not
...
that calling (for example) \kbd{ellan(e,100000);} is much faster than
...
$1+p-a_p$.  (At present, calling \kbd{ellap} is the only way of
obtaining the order of an elliptic curve over $\F_p$ in \kbd{gp}.)
