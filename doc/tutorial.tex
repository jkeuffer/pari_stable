...
\vskip1.truecm
\authors
\centerline{last updated August 10, 2004}
...
icon, or some menu item). It says hello in its particular manner, and
...
read them, the explanation is simply that \kbd{gp} is unable to compute the
...
all that junk!), then \kbd{dr = matdet(hr)}. You notice two things. First the
computation, is much faster than in the rational case. (If your computer is
too fast for you to notice, try again with \kbd{h = mathilbert(40)}.) The
...
function is redundant).  The \kbd{i} and \kbd{j} represent dummy variables
...
zero entries, as in \kbd{matrix(10,20)}.
...
less than the product of elements of \kbd{L} in absolute value, which it is.)
...
The function \kbd{zero(r)} will then count the number of zeros of modulus
less than \kbd{r}: we simply made the change of variable $z = r*\exp(i*t)$,
...
\kbd{pridinit} exist.)
  ...
  Let's try it. The discriminant \kbd{e.disc} is equal to 37, hence the
conductor of the curve is 37. Of course in general it is not so trivial. In
fact, the equation of the curve is clearly not minimal, so type \kbd{e =
ellminimalmodel(e0)}, which gives the \kbd{ell} structure associated with the
minimal model, exactly as if we had used \kbd{ellinit} on the minimal
equation. For some related data, type \kbd{r = ellglobalred(e0)}. The first
component \kbd{r[1]} tells us that the conductor is 37 as we already knew.
The second component is a 4-component vector which allows us to get the
minimal equation: in fact \kbd{e} is \kbd{ellchangecurve(e0, r[2])}. You can
for the moment ignore the third component \kbd{r[3]} (for the impatient
...
canonical Neron-Tate height of \kbd{q}. (Note that \kbd{ellheight} assumes
that \kbd{e} is \emph{minimal}!) This is non-zero, hence \kbd{q} is not
...
@eprog\noindent
we see that \kbd{q} is a point of order 5 (note that the point at infinity is
represented as \kbd{[0]}). More simply, you can type \kbd{ellorder(e,
q)}.\smallskip
...
3. We are thus led to strongly believe that the curve has rank exactly 3, and
this can be proved to be the case.\smallskip

Let's explore a few more elliptic curve related functions. Keep
...
ellinit([0,-1,1,0,0])} where we had seen that the point \kbd{q = [0,0]} was
...
\kbd{torsell(e)} shows that it is of order exactly 5. So type
\kbd{ls * 25/e[15]}. This shows that $\sha$ must be the trivial group.
...
\kbd{nfsubfields} (we'll do that later on).
...
polmod \kbd{C}. Try it. The result has a big denominator (147) essentially
because our initial polynomial \kbd{T} was so bad. By the way, to get the 147,
...
precomputed prime table, including \kbd{addprimes} offsprings. You may then
input the resulting basis to \kbd{nfinit}, as \kbd{nfinit([T, B])}.
...
\kbd{idealprimedec} in Chapter~3) and all ideal-related functions will accept
them. On the other hand, the function \kbd{idealtwoelt} can be used to find a
...
ideal under \kbd{gp}, and most functions will choke on it (or worse, take it
...
initialize a Big Number Ray, or \kbd{bnr}, structure, associated to a
...
G=galoisinit(x^4+1);
@eprog
to compute $G=Gal(K/\Q)$.  The command \kbd{galoisisabelian(G)} return
\kbd{[2,0;0,2]} so $G$ is an abelian group, isomorphic to $(\Z/2\Z)^2$, generated by
$\sigma$=\kbd{G.gen[1]} and $\tau$=\kbd{G.gen[2]}. Theses automorphisms are given by theirs actions on the roots of $x^4+1$ in a suitable $p$-adic extension.
To get the explicit action on $\zeta$, we use \kbd{galoispermtopol(G,G.gen[i])} for $i=1,2$. we get $\sigma(\zeta)=-\zeta$ and $\tau(\zeta)=\zeta^3$.
The last non-trivial automorphism is $\sigma\tau$=\kbd{G.gen[1]*G.gen[2]} and 
...
could use \kbd{nffactor}, but here we have a much simpler option, to ask for the
relative factorisation by setting flag=2 in \kbd{galoisfixedfield}:
\kbd{galoisfixedfield(G,G.gen[1],2)} output 
\bprog
...
To compute the Galois group $G$ of $K/\Q$ we issue \kbd {G=galoisinit(P);}, and
since \kbd{galoisinit} succeeded, the extension $K/\Q$ is Galois. 
This time \kbd{galoisisabelian(G)} return $0$, so the 
extension is not abelian, however we can still put a name on the
underlying abstract group, use \kbd{galoisidentify(G)}, which return
...
we can ask PARI to compute them using \kbd{galoissubgroups(G)}.

We are looking for a polynomial $Q$ with the property that $K$ is the splitting
...
irreducible with \kbd{polisirreducible(Q)} (if it is not the case, we would
...
field of $Q(x^2)$.  We can check that with \kbd{nfisisom(P,subst(Q,x,x\^{}2))}.
