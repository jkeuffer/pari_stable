...
larger ones, using \kbd{vecextract}. For instance, if \kbd{h} is the
...
\bprog
  M = H * qflll(H)
@eprog
Its column give an LLL-reduced basis for $\Lambda$ (\kbd{qflll(H)} itself
...
coefficients then convert it to a polynomial using \kbd{Pol} or {Polrev}
(\kbd{Pol([1,2])} is $x+2$, \kbd{Polrev([1,2]) is $2x + 1$}) :
\bprog
  T = Polrev( vector(100, i, i) );

  for (i=1, 10^4, Polrev( vector(100, i,i) ) )   \\@com time: 124ms
  for (i=1, 10^4, sum(i = 1, 100, (i+1) * x^i) ) \\@com time: 3,985ms
...
\bprog
  r = polroots(pol)
@eprog\noindent We obtain the 8 complex roots of \kbd{pol}, given to 28
...
$L^2$-norm of \kbd{rc - rr} is around $3.10^{-28}$, reasonable enough when we
work with 28 significant digits! Note that the function \kbd{norml2},
...
intelligently over which ring you want to factor: try
\bprog
  pol = x^2 + 1;
  factor(pol)
  factor(pol *1.)
...
transfered it onto the minimal model \kbd{e}, using \kbd{ellchangepoint} and
...
a spceific prime~\kbd{p}, use the function \kbd{elllocalred(E,p)}; the second
...
with the declaration \kbd{my(t = ...)}. Also, we have used an interesting
...
Let us now ``idelize'' \kbd{pr3} by typing
\bprog
   id3 = [pr3, [0,0]]
   r1 = idealred(nf, id3)
@eprog
(We need $r_1+r_2=2$ components for the second vector.) The new ideal
\kbd{r1} is equivalent to \kbd{id3} (modulo the principal ideals). The
Archimedean component is non-trivial and gives the distance from the reduced
ideal to the original one (see H. Cohen \emph{A Course in Computational
Algebraic Number Theory}, GTM {\bf 138} for details, especially Sections 5.8.4
...
\bprog
  ach = prod(i=1, #v, x-v[i])
  ch = round(ach)
@eprog\noindent
Since the coefficients of \kbd{ach} should be very close to integer, we rounded
it above. We can also get directly the representation of the unit on the
...
it is a square. Type 
\bprog
  f1 = factor(subst(ch, x, x^2))
@eprog\noindent We see that \kbd{ch(x\pow2)} is a product of 2 polynomials of
degree 4, hence $u$ is a square. (Why?) We now want to find its square root.
...
A better method which avoids sign ambiguities is as follows. Type
\bprog
  r = f1[1,1] % (x^2 - nfbasistoalg(subst(nf,x,y),u))
@eprog\noindent
...
need to \kbd{subst(nf,x,y)} ? The reason is that data related to \kbd{nf}
is given in terms of the variable \kbd{x}, seen modulo \kbd{nf.pol}; but we
need \kbd{x} as a free variable for our polynomial divisions. Hence the
substitution of \kbd{x} by \kbd{y} in our \kbd{nf} data.

  A last method is to use \kbd{nffactor(nf, y\pow 2 + u)}. Except that this
won't work for the same technical reason as above: the main variable of the
polynomial to be factored must have \emph{higher} priority than the number
field variable. This won't be possible here since \kbd{nf} was defined using
the variable \kbd{x} which has the highest possible priority. So we need to
substitute variables around:
\bprog
  NF = subst(nf,x,y);
  nffactor(NF, x^2 - nfbasistoalg(NF,u))
@eprog
\smallskip
...
of the variable \kbd{y}, to avoid later problems variable priorities.
...
the keyword \kbd{my}. One is usually better off avoiding global variables
