...
For definiteness, we will assume the latter henceforth. Of course, you
...
approximately the same as when computing this quantity, here $1-0.5*10^{-30}$
where $0.5*10^{-30}$ is considered with 28 significant digit accuracy. Hence the
...
result, \kbd{gp} gives you $1.000\dots$ with as many mantissa word as the current
precision.
\medskip
...
arguments!), but we could refine it if the need arose. You can also
consider $(u)$ as a principal ideal, and just type
\bprog
  idealnorm(nf,u)
@eprog\noindent
...
a foolproof test: \kbd{polred} could have returnes some other polynomials.)
...
\misctitle{Additional comment} (you're no longer supposed to skip this,
but do as you wish):
...
implies factoring the discriminant of the polynomial, which is often not
feasible. The situation may be improved in many ways:
