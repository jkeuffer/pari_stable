...
operations and functions available in PARI and \kbd{gp} will be described in
...
described in \secref{se:emacs}. Specific features of this Emacs shell will
be indicated by an EMACS sign in the left margin.
...
If the line before the copyright message indicates that extended help
...
command accepts compressed files in \idx{compress}ed (\kbd{.Z}) or
\idx{gzip}ped (\kbd{.gz} or \kbd{.z}) format. They will be uncompressed on
...
Note that the \kbd{install} command enables you to concentrate on critical
parts of your programs only (which can of course be written with the help of
other mathematical libraries than PARI!), and to efficiently import foreign
functions for use under \kbd{gp} (see~\secref{se:install}).
...
%
by St\'efane Fermigier. Finaly, Michael Stoll has integrated PARI into
\tet{CLISP}, which is a Common Lisp implementation by Bruno Haible, Marcus
Daniels and others. These provide interfaces to \kbd{gp} functions for use in
\kbd{perl}, \kbd{python} or \kbd{Lisp}\sidx{Perl}\sidx{Python}\sidx{Lisp}
programs. Only the \kbd{perl} interface is being actively maintained.
