...
\kbd{f.p}: returns the characteristic of the finite field.

\kbd{f.mod}: returns the minimal polynomial (with reduced integers
...
\kbd{[ ; ]} yields the empty ($0 \times 0$) matrix.
The function \tet{Mat} can be used
...
@eprog\noindent
For instance, assuming that \var{op}$_1$, \var{op}$_2$, \var{op}$_3$ are
standard binary operators with increasing priorities (think of \kbd{+},
\kbd{*}, \kbd{\pow} for instance),
$$ x~\var{op}_1~y~\var{op}_1~z~\var{op}_1~x  $$ is
equivalent to
$$ ((x~\var{op}_1~y)~\var{op}_1~z)~\var{op}_1~x,$$
...
$x$~\var{op}~$y$. Most of them are standard (\kbd{+}, \kbd{\%}), some are
...
\point{Priority 11}
\kbd{->}: \kbd{(\var{vars})->\var{expr}} returns a function object, of type
\typ{CLOSURE}.
...
\point{Priority 9}
%
\kbd{=} and \kbd{\var{op}=}, where \var{op} is any simple binary operator
among \kbd{+}, \kbd{-}, \kbd{*}, \kbd{\%}, \kbd{/}, \kbd{\bs}, \kbd{\bs/},
\kbd{<<}, or
\kbd{>>}. These special unary, postfix operators, following an \emph{lvalue},
create a \emph{prefix} unary operator which will gobble the next argument and
use it to assign a new value to the left hand-side. (See below.)
...
\var{lvalue} \kbd{=} is the assignment operator created by the postfix \kbd{=}
operator (of priority 9) applied to \var{lvalue}. The result of \kbd{x~=~$y$}
is the value of the expression~$y$, which is also assigned to the
variable~\kbd{x}. This is \emph{not} the equality test operator; a statement
like \kbd{x~=~1} is always true (i.e.~non-zero), and sets \kbd{x} to~1. The
right hand side of the assignment operator is evaluated before the left hand
side. If the left hand side is not an lvalue, raise an error. This assignment
operator is right-associative.

\leavevmode
\var{lvalue}\kbd{\var{op}=}, where \var{op} is any simple binary operator
among \kbd{+}, \kbd{-}, \kbd{*}, \kbd{\%}, \kbd{/}, \kbd{\bs}, \kbd{\bs/},
\kbd{<<}, or
\kbd{>>}. This is the assignment operator created by the postfix
\kbd{\var{op}=} operator applied to \var{lvalue}.

The expression \kbd{x~\var{op}=~$y$} assigns $(\kbd{x}~\var{op}~y)$
to~\kbd{x}, and returns the new value of~\kbd{x}. This is \emph{not} a
reference to the \idx{variable}~\kbd{x}, i.e.~an \tev{lvalue}; thus
\bprog
  (x += 2) = 3
@eprog\noindent
...
\misctitle{Remark:} The assignment operators are split in two unary
operators, one prefix, the other postfix, where one would expect a simple
binary operator. This is because, in an expression like \kbd{x = y},
the \kbd{=} has very high priority on the left, and very low on the right.
We want the following interpretation
\bprog
  x = 1 + 1   --->   x = (1 + 1)
  1 + x = 1   --->   1 + (x = 1)
@eprog\noindent and there is no way to assign relative priorities to
\emph{binary} operators \kbd{+} and \kbd{=} yielding the above behaviour.
With our convention, it works:
\bprog
  x = 1 + 1   --->   (x =) (1 + 1)
  1 + x = 1   --->   1 + ((x =) 1)
@eprog\noindent where \kbd{(x = )} is the unary prefix operator assigning its
argument to \kbd{x}.
...
function here, \kbd{calc(x->x\^2, 3)}. Here is a variation:
\bprog
...
runtime error occurs, freezing the \kbd{gp} state, and preventing cleanup
until you get out of the loop. By runtime error, we mean an error from
the library, a user error (from \tet{error}), or user interrupts like
\kbd{C-c} (Control-C), but \emph{not} syntax errors. When a break loop
starts, a prompt is issued (\kbd{break>}). You can type in a \kbd{gp}
command, which is evaluated when you hit the \kbd{<Return>} key, and the
result is printed as during the main \kbd{gp} loop, except that no history of
results is kept. Then the break loop prompt reappears and you can type
further commands as long as you do not exit the loop. If you are using
readline, the history of commands is kept, and line editing is available as
usual. If you type in a command that results in an error, you are sent back
to the break loop prompt (errors do \var{not} terminate the loop).
...
can never have context for runtime errors since the code was first
successfully compiled into bytecode,
...
\misctitle{Important Note:} upon startup, this mechanism is \emph{off}, a
runtime error will send you back to the prompt. Type \kbd{trap()} (or include
it in a script) to start trapping errors in this way.
...
example of an \tev{error handler}: a function that is executed whenever a
runtime error occurs, supposedly to try and recover. The break loop is quite
a satisfactory error handler, but it may not be adequate for some purposes,
for instance when \kbd{gp} runs in non-interactive mode, detached from a
terminal.
...
available: the new handler has replaced the default one.

Besides user-defined handlers as above, there are two special handlers you
can use in \kbd{trap}, which are
...
64-bit machines, respectively): of the form x$.n$, where x is a letter in
$\{\kbd{e},\kbd{f},\kbd{g}\}$, and $n$ is an integer. If x is \kbd{f},
...
The third format is the \tev{prettyprint format}, or beautified format. This
format is now obsolete and identical to the \kbd{prettymatrix}.
