...
%TODO: remove reference to handler/trap, and use catch/throw/exception
 The \tet{iferr} function allow to define a temporary handler used within the
scope of a code fragment, protecting it from errors, by providing replacement
code should the trap be activated. The expression
...
simplified as:
\bprog
? inv3(x) = iferr("e_INV", 1/x, ERR, "oo", errname(ERR) == "e_INV")
...
Here is a final example, to check in a safe way whether \tet{install} works
