...
can use \tet{ffinit} to compute it. Then you must create a generator of the
finite field. This is done using the \tet{ffgen} function, in the following way.
...
The second parameter to \tet{ffgen} is the name used to display $g$, so it is
...
In this section we use \emph{variable} in the usual mathematical
...
Note that GP names are case sensitive. For instance, the symbol \kbd{i} is
perfectly safe to use, and will not be mistaken for $\sqrt{-1}$; analogously,
\kbd{o} is not synonymous to \kbd{O}.

\subsec{Variables and polynomials}
What happens when you use a valid variable name for the first time,\kbd{t}
say, \emph{before} assigning a value into it? This registers a new free
variable with the interpreter (which will be written as \kbd{t}), and
evaluates to a monomial of degree $1$ in the said variable \kbd{t}. It is
important to understand that PARI/GP is \emph{not} a symbolic manipulation
package: variables already have default values%
...
in general preferable to use \kbd{subst}, rather than assigning values to
...
representation for polynomials.) All computations are then just
done formally on the coefficients as if the polynomial was univariate.
...
\section{Variables and Scope}\sidx{variable scope}
This section is rather technical, and strives to explain clearly potentially
...
A user's function, the body of a loop, all define scopes; the whole program
defines the \emph{global} scope. Variables are bound to values within a given
scope. This is traditionnally implemented in two different ways:

\item lexical (or static) scoping: the binding makes sense within a given block
of program text. The value is private to the block and may not be accessed
from outside. This is determined at compile time.

\item dynamic scoping: introducing a local variable, say \kbd{x}, pushes a new
value on a stack associated to the name \kbd{x} (possibly empty at this point),
which is popped out when the control flow leaves the scope. Evaluating \kbd{x}
in any context, possibly outside of the given block, always yields the top
value on this dynamic stack.
...
binding to 0, and the binding to 1 introduced in \kbd{g}, which is still
present on the stack since the control flow has not left \kbd{g} yet.

\subsec{Scoping rules}.

Named parameters in a function definition, as well as all loop
indices\footnote{**}{
More generally, in iterative constructs which use a variable name
(\kbd{for$xxx$}, \kbd{prod$xxx$}, \kbd{sum$xxx$}, \kbd{vector}, \kbd{matrix},
...
@eprog\noindent
Note how the declaration can include (optional) initial values. Variables
for which no explicit default value is given in the declaration are
initialized to $0$. A more complicated example:
\bprog
for (i = 1, 3,
  print("main loop");
...
are the same as before it was started.
...
smallest possible block. Should you forget this, if a function accesses a
variable which is not one of its formal parameters, the value used will be
the one which happens to be on top of the value stack at the time of the
call. This is in general \emph{not} what you want to do. So, be warned.
...
my(}\var{list of local variables, lexically scoped}\kbd{);}
   \var{seq}
...
is comma-separated and allowed to be empty. The \tet{my} statements can be
...
usual \var{seq} is any expression sequence.
...
be what you want. See \secref{se:scope} for details. Local variables can
...
``loop'', and that it is unnecessary to add a \kbd{my(n)} declaration to the
loop body.
...
return $\sin(1)$ (evaluated!). Note that it is crucial to use \kbd{local}
here, since \kbd{eval} evaluates its arguments in global scope. The simpler
\bprog
calc(f, x) = eval( Str(f, "(x)") )
@eprog
would not work, since \kbd{x} is lexically scoped to the function body.
Another approach is
\bprog
calc(f, x) = eval( Str(f, "(", x, ")") )
@eprog
