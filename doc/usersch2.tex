...
\subsecidx{output} (default \kbd{1}): this can take three values: 0
(=~\var{raw}), 1 (=~\var{prettymatrix}), or 2 (=~\var{prettyprint}). This
...
\tev{raw format}, i.e.~a format which is equivalent to what you input,
...
\subsecidxunix{prettyprinter} (default \kbd{""}) the name of an external
prettyprinter to use instead of the default (ugly) one when \kbd{output}
is~2 (\var{prettyprint}). {\bf This is experimental} (for the time being,
