...
directly, as in \kbd{List([1,2,3,4])}; but in most cases, creates
an empty list using {listcreate}, then appends elements using \kbd{listput}.
Elements can be accessed directly as with the vector types described above.

Due to a design flaw, Lists must be stored in GP variables, not as
components; in particular, Lists of Lists or Vectors of Lists are not
supported:
\bprog
  ? L=[0]; L[1] = listcreate()
  ***   sorry, "lists as components" is not yet implemented.
@eprog
...
\kbd{M} is the \kbd{Meta} key), a special shell will be started, which in particular launches \kbd{gp} with
