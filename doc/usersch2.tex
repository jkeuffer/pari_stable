...
? Polrev([1,2,3], x)
%3 = 3*x^2 + 2*x + 1
@eprog\noindent Polynomials are always printed as \emph{univariate}
polynomials, with monomials sorted by degreasing degree:
\bprog
? (x+y+1)^2
%1 = x^2 + (2*y + 2)*x + (y^2 + 2*y + 1)
@eprog\noindent
(Univariate polynomial in \kbd{x} whose coefficients are polynomials in
\kbd{y}.) See \secref{se:variable} for valid variable names, and a discussion
...
\section{Variables and symbolic expressions}\sidx{variable}\label{se:variable}
...
argument, and leave the defaults in between as they stand, use
the ``empty argument'' trick: \kbd{f(6,,1)} would yield
...
fomula used in \kbd{sumalt} with $n$ terms computes $(1-2^{1-s})\zeta(s)$
...
@eprog\noindent As for uer functions, you can redefine your member functions
...
3 alternative approaches to the above member functions: 1) hardcode
