...
Originally, \tet{gp} was designed as a debugging tool for the PARI system
...
explicitly a command of the above form.}. The [\var{files}] argument is a
...
files used to check whether \kbd{gp} has been built correctly (see \b{e}).
...
\kbd{C-u M-x gp}), and a new session of \kbd{gp} will be started after the old one,
...
\emph{if} you have compiled \kbd{gp} with the GNU \idx{readline} library. If you
...
compiling \kbd{gp}. In fact, with \kbd{readline}, even line editing becomes
