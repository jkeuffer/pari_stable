...
the OS). For Windows 95/98 and NT, you can use the \tet{EMX} binary in a
...
\obr
  local(tmp);
  if (type(x) != "t\_VEC", tmp = ellinit(x))
   \bs\bs \var{some further checks}
  tmp
\cbr
...
putting a \kbd{\bs} just before them; this has the following effects:
...
\bprog%
  print()
  print(1+3"a,3" ,4)
  print(a=3, (1 + ((a-3)==print())) (a = (a == 5\bs/2)))
\eprog
...
\h
\b{\bs}  readline wants non-printing characters to be braced between ^A/^B pairs
\#if READL  prompt = "(\%R) ^A\b{e}[1m^Bgp^A\b{e}[m^B > "
\h
...
(setq auto-mode-alist
  (cons '("\bs\bs.gp\$" . gp-script-mode) auto-mode-alist))
\eprog
...
\$if Pari-GP
  ...
  "\b{C}-h": backward-delete-char
  "\b{e}\b{C}-h": backward-kill-word
  "\b{C}-xd": dump-functions
  (: "\b{C}-v()\b{C}-b"        \# can be annoying when copy-pasting~!
  [: "\b{C}-v[]\b{C}-b"
\$endif%
\eprog
%
