...
In short, basic individual operations on these types are fast, but
recursive objects with such components could be handled more efficiently:
programming with libpari will save large constant factors here, compared
to GP.
...
represented by its representative (a \typ{POL} in $R[t]$) of lowest
degree, and computations occur with polynomials in the variable $t$. PARI
will not recognize that \kbd{Mod(y, y\pow2 + 1)} is ``the same'' as
\kbd{Mod(x, x\pow2 + 1)}, since \kbd{x} and \kbd{y} are different
variables.
...
\bprog
    ...
    %1 = Mod(2*x, x^2 + 1)
    ? x + Mod(y, y^2 + 1)
    %2 = x + Mod(y, y^2 + 1)  \\@com in $\Q(i)[x]$, with $i^2=-1$
    ? y + Mod(x, x^2 + 1)
...
\misctitle{Note.} If the main variables are the same, it is allowed to mix
\typ{POL} and \typ{POLMOD}s. The result is the expected \typ{POLMOD}. For
instance 
\bprog
    ? x + Mod(x, x^2 + 1)
    %1 = Mod(2*x, x^2 + 1)
