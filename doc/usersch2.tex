...
in the PARI library. Both kinds will be fatal to your computation sending you
back to the GP prompt.
...
\tet{error}), even user interrupts like \kbd{C-c} (Control-C). During a break
loop, a prompt is issued, commands are read and evaluated, and results
printed as during the main GP loop, except that no history of results is kept
(history of commands is kept, if you are using readline).
...
parameters\dots) in the middle of a long computation (hit \kbd{C-c}, type
in your modifications, then let the computation go on as explained below).

To get out of the break loop, you can use \tet{next}, \tet{break},
\tet{return} of \kbd{C-d} (\kbd{EOF}). In general, terminating a break loop will
send you back to the GP prompt. There is an important exception if the error
is not fatal (in current version \vers, an error is non-fatal iff it was
initiated by a \kbd{C-c} typed by the user). In that case, \kbd{next} will
continue the computation as if nothing had happened, except of course, you
may have changed GP state during the break loop. After a user interrupt
(\kbd{C-c}), entering an empty input line (i.e hitting the return key) has
the same effect as \kbd{next}. An empty line has no effect in case of a fatal
error, to ensure you do not get out of the loop prematurely.
...
\kbd{break>} at the bottow is a prompt, and hitting \kbd{v} then return, we
see:
\bprog
break> v
0
@eprog
\noindent exhibiting the culprit. We could have typed any command, not only
the name of a variable, of course.
...
\misctitle{Technical Note:} When you enter a break loop, the pari stack is
reset so that you can run commands, even when the error was a stack overflow,
but you do not lose the value of any GP variable in the process.
...
\noindent Omitting the error name means we are trapping all errors. The
following can be used to check in a safe way whether \tet{install} works in
the system considered:
\bprog
check_install() = 
{
  trap(archer, return (1),
    install(addii,GG)
  );
  trap(, 2,
    if (addii(1,1) != 2, 3)
...
clause of the \kbd{if}), 1 if the operating system does not support
\kbd{install}, 2 if using an installed function triggers an error, and 3 if
the installed function did not behave as expected.
