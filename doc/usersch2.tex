...
are familiar with the languages they are based on. The first is the
\tet{Math::Pari} Perl module (see any CPAN mirror), written by Ilya
Zakharevich. The second is \tet{PariPython}%
\footnote{*}{see \kbd{http://www.fermigier.com/fermigier/PariPython/}},
%
by St\'efane Fermigier, which is no more maintained. Finaly, Michael Stoll
has integrated PARI into \tet{CLISP}, which is a Common Lisp implementation
by Bruno Haible, Marcus Daniels and others. These provide interfaces to
\kbd{gp} functions for use in \kbd{perl}, \kbd{python} or
\kbd{Lisp}\sidx{Perl}\sidx{Python}\sidx{Lisp} programs, respectively.
