...
? inv2(x) = {
  iferr(1/x,
        ERR, if (errname(ERR) != "e_INV", error(ERR));
             "oo")
...
simplified as:
\bprog
? inv3(x) = iferrname("e_INV", 1/x, ERR, "oo")
@eprog\noindent Rather than trapping everything, then rethrowing whatever we
do not like, we advise to only trap errors of a specific kind, as above. Of
course, sometimes, one just want to trap \emph{everything}:
...
\kbd{component(ERR,1)}, \kbd{component(ERR,2)}, and so on. See
\secref{se:iferrname} for the list of all exception types, and the
corresponding contents of \kbd{ERR}.
...
\noindent$\bullet$ If no gprc was found among the user files mentioned above
we look for \kbd{/etc/gprc} for a system-wide gprc file (you will need root
privileges to set up such a file yourself).

\noindent$\bullet$ Finally, we look in pari's \kbd{datadir} for a file named
