...
situation has now changed somewhat, and \kbd{gp} is very useful as a
stand-alone tool. The operations and functions available in PARI and \kbd{gp}
will be described in the next chapter. In the present one, we describe the
specific use of the \kbd{gp} programmable calculator.

\unix The calculator is developed on UNIX platforms, and depend heavily on
the operating system in use. Whenever a specific feature of the UNIX version
is discussed in a paragraph, a UNIX sign sticks out in the left margin, like
here. Just skip these if you're stranded on a different operating system: the
core GP functions, in particular everything which is even faintly
mathematical in nature, are still available to you. It should also be
possible (and then advisable) to install \idx{Linux} or \idx{FreeBSD} on your
machine.
...
printing its prompt (by default: \kbd{?}).
...
Whether extended on-line help, line editing are available or not is
indicated in the \kbd{gp} banner, between the version number and the
copyright message. Consider investigating the matter with the person who
installed \kbd{gp} on your system if they are not. Do this as well if the GMP
