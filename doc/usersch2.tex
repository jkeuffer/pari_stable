...
indicate end of input. Not so in \kbd{gp}: a final semi-column means the
...
in particular loop bodies are compiled only once but a priori evaluated many
times; then the bytecode is evaluated.

An \idx{expression}\sidx{expression sequence} is formed by combining
constants, variables, operator symbols, functions (in fact, variables can
hold functions, so functions are just variables of an appropriate type) and
control statements. An expression always has a value, which can be any PARI
object:
\bprog
? 1 + 1
...
function definition is, unsurprisingly, a function. 


There is a distinction between lowercase and uppercase. Also,
...
maintained by \kbd{readline}. It only contains their non-void results, in
sequence. Several inputs only act through side effects and produce a void
result, for instance a \kbd{print} statement or a \kbd{for} loop.
...
An opening brace (\kbd{\obr}) signals that you are typing a multi-line
command, and newlines are ignored until you type a closing brace \kbd{\cbr}.
There is an important, but easily obeyed, restriction: inside an open
brace-close brace pair, all your input lines are concatenated, suppressing
any newlines. Thus, all newlines should occur after a semicolon (\kbd{;}), a
comma (\kbd{,}) or an operator (for clarity's sake, never split an identifier
over two lines in this way). For instance, the following program
\bprog
{
  a = b
  b = c
...
If $a$ is a \typ{INTMOD}, the following member function is defined:

\kbd{a.mod}: return the modulus.
...
\subsec{Finite field elements}\sidx{finite field element} (type \tet{t_FFELT}):
first, you must create a irreducible polynomial $T$ defining your finite
field, for instance using \tet{ffinit}. Then the \tet{ffgen} function creates
a generator of the finite field, namely the class of $X$ in $\F_p[X]/(T)$,
from which you can build all other elements. For instance, to create the
field $\F_{2^8}$, we write
\bprog
? T = ffinit(2, 8);
? y = ffgen(T, y);
? y^8
...
generator to. If $f$ is a \typ{FFELT}, the following member function is
defined:
...
variable of \var{expr}. (To check the ordering of the variables, type
\kbd{variable()}.)\kbdsidx{variable}\kbdsidx{variable}
...
\subsec{Lists} (type \tet{t_LIST}):\sidx{list} lists can be input
...
\subsec{Functions} (type \tet{t_CLOSURE}): we will explain this at length
in a later section. For the time being, suffices it to say that functions can
be assigned to variables, as any other object, and the following
...
([\kbd{A-Za-z0-9}]). The built-in function names are reserved and cannot be
used; see the list with \b{c}, including the constants \kbd{Pi}, \kbd{Euler}
and $\kbd{I}=\sqrt{-1}$.

GP names are case sensitive. For instance, the symbol \kbd{i} is perfectly
safe to use, and will not be mistaken for $\sqrt{-1}$; analogously, \kbd{o}
is not synonymous to \kbd{O}.
...
values to them, and to use \kbd{subst} rather than \kbd{eval}. You will avoid
...
argument of \tet{eval} is evaluated in \emph{global} scope.
...
variables. Analogously, \tet{eval} becomes less powerful: since it is
evaluated in global scope, it cannot access values of lexical variables:
\bprog
? x = 0; eval("x")
%1 = 0
? my(x = Pi); eval("x")
%2 = 0    \\@com we see the global \kbd{x}, not the one scoped to this command line.
@eprog
...
\typ{CLOSURE}), the function is called with the proper arguments. In
particular, constructions like \kbd{funcs[i](x)} are perfectly valid,
...
The trailing semi-colon prevents \kbd{gp} from printing the result of the
evaluation, i.e. the function object. The construction
...
\noindent Of course, the sequence \var{seq} can be arbitrarily complicated,
in which case it will look better written on consecutive lines, with properly
scoped variables:
\bprogpart
f($x_0$, $x_1$, @dots) =
{
...
restricted blocks than the whole function body. Arguments are passed by
value, not as variables: modifying a function's argument in the function body
is allowed, but does not modify its value in the calling scope. In fact, a
\emph{copy} of the actual parameter is assigned to the formal parameter when
the function is called. Formal parameters are lexically scoped to the
function body.

It is not allowed to use the same variable name for different parameters of
your function:
\bprog
? f(x,x) = 1
...
Note that there is no distinction between variable and functions names: we
have PARI objects (of type function or otherwise) and variables bound to
them. There is nothing wrong with the following sequence of assignments:
\bprog
? f = 1
%1 = 1;
...
%4 = (x)->x^2
@eprog\noindent
Previously used names can be recycled: you are just redefining the variable.
The previous definition is lost of course.


Built-in function are a different matter since they are read-only (you cannot
overwrite their default meaning), and they use features not available to user
functions. In the present version \vers{}, it is not possible to assign a
built-in function to a variable, nor to use a built-in function name
to create an anonymous function: using \kbd{sin} will not work, you 
must write \kbd{x -> sin(x)} explicitly.

\subsec{Default arguments}

The number of actual parameters supplied may be \emph{less} than the number
of formal variables in the function definition. An uninitialized formal
variable is given an implicit default value of (the integer)~0, i.e.
after the definition
\bprog
  f(x, y) = ...
@eprog\noindent
you may call \kbd{f(1, 2)}, supplying values for the two formal 
parameters, or \kbd{f(1)} (equivalent to \kbd{f(1,0)}), \kbd{f()} (equivalent
to \kbd{f(0,0)}), or even \kbd{f(,1)} (equivalent to \kbd{f(0,1)}).
...
signals to GP it should use a default value. When no input arguments are
left, the defaults are used instead to fill in remaining formal parameters.

Of course, you can change this default value to something more useful than
...
gets evaluated the moment the function is called, but currently may not
involve the preceding function parameters. For instance, after
\bprog
f(x = 1, y = 2, z = 3) = ....
@eprog\noindent
typing in \kbd{f(6,4)} would give
you \kbd{f(6,4,3)}. In the rare case when you want to set some far away
argument, and leave the defaults in between as they stand, leave
an ``empty argument'' trick: \kbd{f(6,,1)} would yield
\kbd{f(6,2,1)}. Of course, \kbd{f()} by itself yields
\kbd{f(1,2,3)} as was to be expected.

%: a default value for $x_i$ may involve $x_j$ for $j < i$.
More specifically,
\bprog
...
%1 = 0
@eprog\noindent
Well, the index $p$ in the \kbd{forprime} loop is lexically scoped to the
loop and is not visible to the outside world. Hence, it does not survive the
\kbd{break} statement. More precisely, at this point the loop index is
...
\subsec{Use}. Once the function is defined, you can use it like any other
function, see the example with \kbd{f} above. In addition, you can also
recall its definition exactly as you do for predefined functions, that is by
writing \kbd{?\var{name}}. This will print the list of arguments, as well as
their default values, the text of \var{seq}, and a short help text if one was
provided using the \kbd{addhelp} function (see \secref{se:addhelp}).
Typing \b{u} will output the list of user-defined functions.
One major difference with predefined functions is that you can never redefine
the built-in functions, nor assign it to a variable.

\misctitle{Note:} Lexically scoped variables were introduced in
...
\bprog
ZETA(s) = sumalt(n=1, (-1)^(n-1)*n^(-s)) / (1 - 2^(1-s))
@eprog
...
(built-in function names are not bound to proper values as would be the case
for user functions). On the other hand,
\bprog
? g(x) = x^2
? calc(g, Pi)
@eprog\noindent works since \kbd{f} is a user function, not a built-in.
Here is a variation:
\bprog
...
whose value is the function \kbd{y->x+y}.
\bprog
? init(5);
? add(2)
%2 = 7
...
\emph{Runtime errors} are more annoying because they occur at a point where
meaningful context no longer exists:
\bprog
...
proper definition (otherwise \kbd{gp} would have protested first). Also they
have more mathematical content, which should be easier to grasp than a
parser's logic. For instance:
\bprog
? 1/0
  *** _/_: division by zero
@eprog\noindent telling us that a runtime error occured while evaluating the
binary \kbd{/} operator (the \kbd{\_} are placeholders).
...
use will be dealt with extensively in Chapter~4. Of course, \kbd{gp} itself
provides a convenient  interpreter to execute rather intricate scripts (see
\secref{se:programming}).
...
\subsec{factor\_proven}\kbdsidx{factor_proven} (default \kbd{0}): by
