...
\sidx{rational number}\sidx{t_FRAC}
...
\sidx{finite field element}\sidx{t_FFELT}
...
\sidx{complex number}\sidx{t_COMPLEX}
...
\sidx{p-adic number}\label{se:padic}\sidx{t_PADIC}
...
\sidx{quadratic number}\sidx{t_QUAD}
...
\sidx{polynomial}\label{se:pol}\sidx{t_POL}
...
\sidx{power series}\sidx{t_SER}\label{se:series}
...
\sidx{rational function}\sidx{t_RFRAC}
...
(\typ{QFR} and \typ{QFI})}%
\sidx{binary quadratic form}\sidx{t_QFR}\sidx{t_QFI}
...
\sidx{row vector}\sidx{column vector}\sidx{t_VEC}\sidx{t_COL})
...
\sidx{matrix}\sidx{t_MAT}
To enter a matrix, type the components line by line, the components being
...
\sidx{string}\sidx{character string}\sidx{t_STR}
...
using \tet{iferr} or \tet{iferrname}, however : we can evaluate some
expression and either recover an ordinary result (no error occurred), or an
exception (an error did occur).
\bprog
...
factorization (cf.~\tet{addpseudoprimes}), where you knowingly use a pseudo
prime $N$ as if it were prime; you may then encounter an ``impossible''
situation, but this would usually exhibit a factor of $N$, enabling you to
refine the
...
provides satisfactory ways out of the second one (the \tet{iferr} and
\tet{iferrname} exception handlers).
...
\subsec{Protecting code}
 The \tet{iferr} and \tet{iferrname} functions allow to define a temporary
handler used within the scope of a code fragment, protecting it from errors,
by providing replacement code should the trap be activated. The expression
...
\bprog
? inv(x) = iferr(1/x, ERR, "oo")
? for (i=-1,1, print(inv(i)))
-1
oo
1
@eprog\noindent Protected codes can be nested without adverse effect, the
last trap seen being the first to spring. Let's now see how \kbd{ERR} can be
used; as written, \kbd{inv} is too tolerant:
\bprog
? inv("blah")
%2 = "oo"
@eprog Let's improve it by checking that we did trap a ``division by 0''
error, and not an unrelated one like the type error \kbd{1 / "blah"}.
\bprog
? inv2(x) = {
  iferr(1/x,
        ERR, if (errname(ERR) != "gdiver", error(ERR));
             "oo")
...
? inv2("blah")
  ...
  ***                 ^------------
  ***   in function inv2: ...f(errname(ERR)!="gdiver",error(ERR));"oo")
  ***                                                 ^-----------------
  *** error: forbidden division t_INT / t_STR.
@eprog\noindent In the last example, the error type was not expected, so we
rethrow the error: \kbd{error(ERR)} triggers the original error that we
mistakenly trapped. Since the recovery code should always check whether the
error is the one expected, this construction is very common and can be
simplified as:
\bprog
? inv3(x) = iferrname("gdiver", 1/x, ERR, "oo")
@eprog\noindent Rather than trapping everything, then rethrowing whatever we
do not like, it is cleaner to only trap errors of a specific kind. Here is a
final example, to check in a safe way whether \tet{install} works correctly
in your \kbd{gp}:
\bprog
broken_install() =
{ \\ can we install ?
  iferrname("archer", install(addii,GG), ERR, return ("OS"));
  \\ can we use an installed function ?
...
\subsec{List of exception types}
...
\kbd{component(ERR,1)}, \kbd{component(ERR,2)}, and so on. We now give the
list of all exception types, and the corresponding contents of \kbd{ERR}.

\kbd{gdiver}: division by non invertible object
