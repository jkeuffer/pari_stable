...
Note that at any point you can type \kbd{Ctrl-C} (that is press
simultaneously the \kbd{Control} and \kbd{C} keys): the current computation
is interrupted and control given back to you at the \kbd{gp} prompt.
...
next section). To suppress printing, end the expression with a \kbd{;} sign.
(See also the \tet{lines} default.)
...
polmod. For example, PARI does not recognize that \kbd{Mod(y, y\pow2 + 1)}
is the same as \kbd{Mod(x, x\pow2 + 1)}. See \secref{se:priority} for a
...
\misctitle{Note.} If the main variables are the same, it is allowed to
mix \typ{POL} and \typ{POLMOD}s. The result is the expected \typ{POLMOD}. For
instance \kbd{x + Mod(x, x\pow2 + 1)} is equal to \kbd{Mod(2*x, x\pow2 + 1)}.
...
([\kbd{\_A-Za-z0-9}]). In addition to the function names which you must not
use (see the list with \b{c}), there are exactly three special variable names
which you are not allowed to use: \kbd{Pi} and \tet{Euler}, which represent
...
expression such as \kbd{x[1,1][,3][4] = 1} is perfectly valid, assuming
...
\subsec{Variables and programming}. We will see in \secref{se:user_defined}
that it is possible to restrict the use of a given variable by declaring it
to be \tet{global} or \tet{local}. This can be useful to enforce clean
...
We are aware of three PARI-related public domain packages to embed PARI in
