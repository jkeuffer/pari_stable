...
\subsecidx{polgalois}$(x,\{\fl=0\})$: \idx{Galois} group of the non-constant
...
If $\fl=0$ (default), the output is a 3-component vector $[n,s,k]$ with the
...
If $\fl=1$, the output is a 4-component vector $[n,s,k,name]$, where $n$, $s$,
$k$ are as above and name is a character string containing name of the
transitive group according to the GAP 4 transitive groups library by Alexander
Hulpke.
...
\syn{polgalois}{x,\fl,\var{prec}}. To enable the new format in library mode,
