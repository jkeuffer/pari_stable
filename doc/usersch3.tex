...
\misctitle{Mneumonics for flags}. Numeric flags as mentionned above are
...
containing symbolic identifiers. For a generic flag, the mneumonic
...
For a binary flag, mneumonics corresponding to the various toggles are given
after each of them. They can be negated by prepending \kbd{no\_} to the
mneumonic, or by removing such a prefix. These toggles are grouped together
...
\noindent so that, instead of $1$, one could use the mneumonic
