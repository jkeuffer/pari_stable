...
absolute value of $x$ if $x$ is rational (\typ{INT} or \typ{FRAC}), and $x$
otherwise. If $x$ is a polynomial (and by extension a power series) or a
vector/matrix it gives the usual content of $x$. If $x$ is a rational
...
\subsecidx{ellidentify}$(E)$: look up the elliptic curve $E$ (over $\Z$) in the
elldata database and return \kbd{[[N, M, G], C]}  where $N$ is the name of the
curve in J.  E.  Cremona database, $M$ the minimal model, $G$ a $\Z$-basis of
the free part of the \idx{Mordell-Weil group} of $E$ and $C$ the coordinates
change (see \kbd{ellchangecurve}).
...
are looked in the elldata database if available. For the time being, only
curves over a prime field $\F_p$ and over the $p$-adic or real numbers
(including rational numbers) are fully supported. Other domains are only
supported for very basic operations such as point addition.
...
("11A") or a conductor "11". This function finds all curves in the elldata
...
Internally, \tet{lindep}$([1,x,\ldots,x^{k-1}], \fl)$ is used. If
...
version of PARI, we suggest to use \emph{positive} values, since it is the
fastest and most robust implementation. See the example in \secref{se:algdep}
(\kbd{algdep}).
...
If $\fl>0$, uses the LLL algorithm with an accuracy of $\fl$ decimal digits.
The parameter $\fl$ should be between 0.6 and 0.9 times the number of correct
decimal digits in the input. This is in general faster than the other variants. 
...
$k$-th subcomponents of the components of~$x$. Note that heapsort is
unstable, hence is allowed to swap "equal" entries (with respect to the
sorting criterion).
