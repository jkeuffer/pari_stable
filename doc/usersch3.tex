...
essential that all such vectors be \emph{column} vectors.
...
$\Z$-generating set. If this is not the case, you \emph{must} absolutely
...
$\bullet$ A \emph{relative matrix} will be a matrix whose entries are
...
often \emph{much} faster to use \kbd{nffactor(nfinit($x$), $y$)} then
\tet{rnfequation}.
...
\emph{absolute} extension $L/\Q$. $\var{vabs}[1]$ is an absolute equation.
...
a rational function. $y$ must be a simple variable name, or a monomial of the
