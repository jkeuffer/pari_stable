...
tye polynomial \var{polrel}. As usual, \var{pol} can in fact be an \var{nf},
or \var{bnf}, etc (the computations will be slightly faster). Computes
technical data needed by \tet{rnfisnorm} to solve norm equations $Nx = a$,
for $x$ in $L$, and $a$ in $K$.
...
respect to $v$ otherwise. This routine also applies to power series, scalars
(polynomial of degree $0$), and to rational functions provided that the
denominator is monomial.
