...
\subsecidx{galoissubcyclo}$(N,H,\{fl=0\},\{v\})$: 
This function compute the abelian extension of the rational field given by the
modulus \var{N} and the congruence subgroup \var{H}.

The modulus can be: 

\quad$\bullet$ a integer \var{n}.

\quad$\bullet$ The output of \kbd{znstar(n)}.

\quad$\bullet$ The output of \kbd{bnrinit(bnfinit(y),m,1))} where m is a module which may eventually have an infinite part.

The subgroup \var{H} can be given by:

\quad$\bullet$ a generator, i.e. an integer or an integer modulo \var{n}.

\quad$\bullet$ a vector of generators.

\quad$\bullet$ a matrix, being taken to be a HNF left divisor of \var{N}, or \kbd{znstar(N)} if N is an integer.

If $fl=0$, compute a polynomial (in the variable \var{v}) defining the subfield
of $\Q(\zeta_n)$ fixed by the subgroup \var{H} of $(\Z/n\Z)^*$. 
...
Alternatively this one can be used instead
...
\syn{galoissubcyclo}{N,H,fl,v} where \var{fl} is a C long integer, and \var{v} a variable number.
...
@eprog
\noindent In the last example, $L$ corresponds to the 5 subfields of
$\Q(\zeta_{120})$, of degree $8$ and conductor $120$.
\bprog
? vector(#L, i, galoissubcyclo(bnr, L[i]))
@eprog
\noindent will produce their equations. (For a general base field, you would
