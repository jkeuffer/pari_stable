...
$\bullet$ If the argument is an intmod or a $p$-adic, at present only a
...
effect as \kbd{exp(y*ln(x))}. It can be applied to $p$-adic numbers as well
...
For the variants to follow we need a notation: let $\Re_m$ denotes $\Re$ or
$\Im$ depending whether $m$ is odd or even.
...
set to zero. The following script computes all roots in all possible cases:
...
\kbd{nfinit} or \kbd{bnfinit}), assume we are dealing with an ideal
factorization in the number field. The resulting ideal product is given in
HNF\sidx{Hermite normal form} form.
...
using \kbd{gp}'s \tet{debuglevel} default parameter (level 3 shows just the
...
\subsecidx{isperfectpower}$(x,\{\&n\})$: return the maximal $k \geq 2$ such
that $x = n^k$ is a perfect power, or $0$ if no such $k$ exist. If $n$ is
given puts the exact $k$-th root there. 

\syn{isperfectpower}{x, \&n}, the result is a \kbd{long}.
...
\smallskip
$\bullet$ When $E$ is defined over $\R$, \kbd{$E$.roots} is a vector whose
...
$\bullet$ $\tev{bnf}$ denotes a ``Buchmann's number field'', i.e.~a
10-component vector in the format output by \tet{bnfinit}. This contains
$\var{nf}$ and the deeper invariants of the field: units, class groups, as
well as a lot of technical data necessary for some complex functions like
\kbd{bnfisprincipal}.

$\bullet$ $\tev{bnr}$ denotes a ``ray number field'', i.e.~some data
structure output by \kbd{bnrinit}, corresponding to the ray class group
...
\quad -- a $\Z$-basis, in Hermite normal form (HNF) or not. In this case $x$
is a square matrix.
...
variable defining the extension. For example, under \kbd{gp} you can use the
...
computation. If the precision becomes insufficient, \kbd{gp} outputs a warning
...
of the \kbd{ploth*} functions) is completely separated from \kbd{gp} (technically,
a \kbd{fork} is done, and the non-graphical memory is immediately freed in
the child process), which means you can go on working in the current \kbd{gp}
