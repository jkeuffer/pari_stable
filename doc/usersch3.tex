...
\subsecidx{rnfisnorm}$(\var{T},\var{a},\{\fl=1\})$: similar to
\kbd{bnfisnorm} but in the relative case. $T$ is as output by
$\tet{rnfisnorminit}(ext, rel)$. This tries to decide whether the element
$a$ in \var{bnf} is the norm of some $x$ in the extension \var{ext}.

The output is a vector $[x,q]$, where $\var{a}=\var{Norm}(x)*q$.
The algorithm looks for a solution $x$ which is an $S$-integer, with $S$ a
list of places (of \var{bnf}) containing the ramified primes, the generators
of the class group of \var{ext}, as well as those primes dividing \var{a}.
If $\var{ext}/\var{bnf}$ is known to be \idx{Galois}, set $\fl=0$, and a
suitable $S$ will be determined. Otherwise, if $\fl$ is non zero add to $S$
all the places above the primes which: divide $\fl$ if $\fl<0$, or are less
than $\fl$ if $\fl>0$. The answer is guaranteed (i.e.~\var{a} is a norm iff
$q=1$) if the field is Galois, or, under \idx{GRH}, if $S$ contains all
...
\subsecidx{rnfisnorminit}$(\var{bnf},\var{ext},\{\fl=0\})$: 
computes technical data needed by \tet{rnfisnorm} to solve norm equations 
$Nx = a$, for $x$ in \var{ext}, and $a$ in \var{bnf}. If \var{ext}/\var{bnf}
is known to be Galois, set $\fl=1$. 

\syn{rnfisnorm}{\var{bnf},\var{ext},\fl}.
