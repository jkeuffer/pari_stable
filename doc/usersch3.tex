...
\subsecidx{nfgaloisconj}$(\var{nf},\{\fl=0\},\{d\})$: $\var{nf}$
being a number field as output by \kbd{nfinit}, computes the conjugates of
a root $r$ of the non-constant polynomial $x=\var{nf\/}[1]$ expressed as
...
old-timers of PARI, starting with version 2.0.14 this function works much
better than in earlier versions.

If $\fl=2$, as $\fl=0$, but using complex approximations to the roots and
an integral \idx{LLL}. The result is not guaranteed to be complete: some
conjugates may be missing (especially so if the corresponding polynomial
has a huge denominator). In that case, increasing the default precision may
help.

If $\fl=3$, as $\fl=2$, but \var{nf} is allowed to be a polynomial.

If $\fl=4$, checks whether the Galois group is weakly super solvable, that is
contains a super solvable normal subgroup $H$ such that $G=H$ or $G/H = A_4$.
(In particular, $G$ abelian is accepted !) If not, returns $0$. If it is,
computes the conjugates using a generalisation of Kl\"uners's algorithm due
to Allombert (result is guaranteed, \var{nf} can be a polynomial). If
present, $d$ is assumed to be a multiple of the index of the power basis in
the maximal order (used if \var{nf} is a polynomial). This method is much
faster than the default when it can be applied.

In practice, most groups of small order are weakly super solvable, the
exceptions having order 24(1 exception), 36(1), 48(6), 56(1), 60(1), 72(8),
75(1), 80(1), 96(25) and $\geq 100$. Hence $\fl = 4$ permits to quickly check
whether a polynomial is Galois or not.

\syn{galoisconj0}{\var{nf},\fl,d,\var{prec}}.
