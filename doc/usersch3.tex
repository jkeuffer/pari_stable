...
the function \tet{ellinit} (see there), which initializes and returns an
...
they are used directyly. This is useful if several integrations of the same
type are performed (on the same kind of interval and functions, for a given
accuracy), in particular for multivariate integrals.
...
number of decimals, for instance half the number of digits will be wrong if
$\alpha=-1/2$ is omitted.
...
approximately as $\exp(-\alpha x)$. This includ oscillating but quickly
decreasing functions such as $\exp(-x)\sin(x)$. The precise choice of
$\alpha$, while useful in extreme cases, is not critical, and may be off by a
factor of $10$ or more from the correct value.
...
between $[[\pm1],\alpha]$ and $[[\pm1],kI]$, but a good rule of thumb is that
