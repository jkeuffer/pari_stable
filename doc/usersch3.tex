...
factors of $n$, as produced by \kbd{factor(n)}. If $n$ is an integer,
they are the positive divisors, in increasing order.
...
In general, a suitable non-zero value of $\fl$ will improve on the
default behaviour:
\bprog
? \p200
...
\subsubsecidx{for}$(X=a,b,\var{seq})$: evaluate \var{seq}, where 
the formal variable $X$ goes from $a$ to $b$. Nothing is done if $a>b$.
$a$ and $b$ must be in $\R$.

\subsubsecidx{fordiv}$(n,X,\var{seq})$: evaluate \var{seq}, where
...
To avoid storing all divisors, possibly using a lot of space, the
following routine loops over the divisors using essentially constant
space:
\bprog
    FORDIV(N)=
    { local(P, E);
...
\subsubsecidx{forell}$(E,a,b,\var{seq})$: the formal variable $E$ ranging
through all elliptic curves of conductors from $a$ to $b$ the sequence
\var{seq} is evaluated. This requires the elldata database to be installed
and to contain data for the specified conductors.
