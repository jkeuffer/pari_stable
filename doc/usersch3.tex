...
and \var{list} being a list of modules as output by \kbd{ideallist} or
...
\noindent $m$ can be decoded using \kbd{bnfdecodemodule}.
...
\quad 2: in this case, \var{nf} must be a \var{bnf} with units. Output
$[L,U]$, where $L$ is as case 0 and $U$ is a vector of discrete logarithms of
the units. More precisely, $U[i]$ gives the \kbd{ideallog}s of
\kbd{bnf.tufu} with respect to the generators of $L[i]$.
...
vector of vectors of all \var{bid} (see \tet{idealstar}) of all
