...
function searches for the minimal non-zero vectors whenever $b=0$. The
implmentation uses low precision floating point computations for maximal
speed, but gives incorrect result when $x$ has large entries. (The
condition is checked in the code and the routine will raise an error
if large rounding errors occur.) A more robust, but much slower,
