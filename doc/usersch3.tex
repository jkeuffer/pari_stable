...
rational numbers. \fl\ is obsolete and should not be used.

Vector/matrix types are also accepted, in which case the GCD is taken
recursively on each component, with the following meaning: if $y$
is a vector, resp.~matrix, then the result has the same type as $y$, and
components equal to \kbd{gcd(x, y[i])}, resp.~\kbd{gcd(x, y[,i])}. Else
...
commutative. To obtain the gcd of all entries in a vector, use
\kbd{content}.
