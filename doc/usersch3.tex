...
outputs instead the vector of $[\var{ideal}, \var{corresponding
root}]$. If $\fl=0$ or 1, uses the $\sigma$ function, while if
$\fl>1$, uses the Weierstrass $\wp$ function, which is less efficient
and may disappear in future versions (not all special cases have been
implemented in this case). Finally, \fl\ can also be a two-component
vector $[\lambda,\fl]$, where \fl\ is as above and $\lambda$ is the
technical element of bnf necessary for Schertz's method using
$\sigma$. In that case, returns 0 if $\lambda$ is not suitable.
...
irreducible relative polynomial of the Stark unit, 0 being
default. The result of the computation depends on the choice of a
modulus verifying special conditions. By default the function will try
few moduli, choosing the one giving the smallest result. In
some extreme cases where the result is however very large, you can
tell the function to try more moduli by adding $4$ to the value of
flag. Example: 
