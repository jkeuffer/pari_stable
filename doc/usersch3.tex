...
\subsecidx{ellidentify}$(E)$: if $E$ is an elliptic curve as output by
\kbd{ellinit()}, return \kbd{[[N, M],C]}  where $N$ is the name of
the curve in J. E.  Cremona database, $M$ the minimal model and $C$ the
coordinates change (see \kbd{ellchangecurve}).
...
("11A") or a conductor "11". Return all curves in the elldata database with the
given property.  
...
e.g.~\kbd{\var{bnr}.bnf.no} to get $\phi(f)$.
