...
\kbd{floor($x$/$y$)} if $y > 0$, and \kbd{floor($x$/$y$) + 1} if $y < 0$ and
the division is not exact. Hence the remainder \kbd{$x$ - ($x$ \bs $y$) *
$y$} is in $[0, |y|[$.
...
congruent to $x$ modulo $y$. When $y$ is a \typ{REAL}, a \typ{FRAC} or a
\typ{FRACN}, $x \% y$ is defined as \kbd{$x$ - ($x$\bs$y$) * $y$}, which
coincides with the definition for $y$ integer if and only if $x$ is an
integer, but still satisfies $0 \leq x\kbd{\%}y < |y|$. For instance:
\bprog
? (1/2) % 3
%1 = 2
? 0.5 % 3
...
\subsecidx{shift}$(x,n,\{\fl=0\})$ or $x$ \kbd{<<} $n$ (= $x$ \kbd{>>} $(-n)$): shifts
...
same (when $x < 0$) as $x \kbd{\bs} 2^{|n|}$. If $\fl$ is non-zero, this
behaviour is modified and rigt shift of a negative $x$ \var{is} the same
as $x \kbd{\bs} 2^{|n|}$ (which is consistent with $2$-complement semantic
of negative numbers).
...
If $f$ is a vector (not mimicing a factorization, see below) instead of a
factorization, return the product of all elements of $f$, i.e assume it is a
factorization, all of whose exponents are equal to $1$.

Finally, if $f$ is a vector of $2$ vectors of the same length the second of
which being integral (dummy factorization), say of the form $f = [x,e]$,
return the product of the $x[i]^{e[i]}$. For example, in case separate
\typ{VEC} $x$ and $e$ are originally available, it is more convenient (and
legible) to use this trick than to build a legitimate factorization;
compare
\bprog
factorback([x, e])
factorback(concat(Mat(x~), e~))
@eprog
