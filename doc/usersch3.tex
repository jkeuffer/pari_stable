...
For $D<0$, uses the $\sigma$ function. If supplied, $\fl$
is is the technical element $\lambda$ of \kbd{bnf} necessary for Schertz's
method. In that case, returns 0 if $\lambda$ is not suitable.
...
\syn{quadray}{D,f,\fl,prec}, where an omitted $\fl$ is coded as \kbd{NULL}.
