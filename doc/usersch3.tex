...
$[c_1,c_2]$, where $c_1 \leq c_2$ are positive real numbers which
control the execution time and the stack size. To get maximum speed,
set $c_2 = c_1$. To get a rigorous result (under \idx{GRH}) you must take
$c_2=6$. Reasonable values for $c_1$ are between $0.1$ and $2$. More
precisely, the algorithm will \emph{assume} that prime ideals of norm
less than $c_2 (\log |D|)^2$ generate the class group, but the bulk of the
work will be done with prime ideals of norm less than $c_1 (\log |D|)^2$.
A larger $c_1$ means that relations are easier to find, but more relations
are needed and the linear algebra will be harder. The default is $c_1 =
c_2 = 0.2$, so the result is \emph{not} rigorously proven.
