...
The random seed may be obtained via \tet{getrand}, and reset using
\tet{setrand}.

Up to version 2.4 included, these pseudo-random numbers were obtained by
means of linear congruences and were not well distributed in arithmetic
progressions. We now use Brent's XORGEN algorithm, based on Feedback Shift
Registers, see \kbd{http://wwwmaths.anu.edu.au/~brent/random.html}.
...
seed. Useful mainly for debugging purposes, to reproduce a specific chain of
computations. The returned value is technical (reproduces an internal state
array), and can only be used as an argument to \tet{setrand}.
...
deterministically a suitable state array. In particular, running a randomized
