...
\subsecidx{Vecrev}$({x=[\,]})$: as \kbd{Vec}, except when $x$ is a polynomial.
...
\subsecidx{random}$(\{N=2^{31}\})$: returns a (pseudo-)random integer
...
\var{nu} and argument $x$.

\syn{kbessel}{\var{nu},x,\var{prec}}
...
following functions are also available: \funs{weberf}{x,prec},
\funs{weberf1}{x,prec} or \funs{weberf2}{x,prec}.
...
\subsecidx{ffgen}$(P,\{v\})$: return the generator $g=X \pmod{P(X)}$ of the
...
\syn{ffgen}{P,v}, where $v$ is a variable number, an omitted $v$ being coded by $-1$.

\subsecidx{ffinit}$(p,n,\{v=x\})$: computes a monic polynomial of degree
$n$ which is irreducible over $\F_p$. This is intended to be used with \kbd{ffgen} above. For instance if \kbd{P = ffinit(3,2)}, you can represent elements in $\F_{3^2}$ in term of \kbd{g = ffgen(P,g)}.

This function uses a fast variant of Adleman-Lenstra's algorithm.

\syn{ffinit}{p,n,v}, where $v$ is a variable number.

\subsecidx{fflog}$(x,g)$: discrete logarithm of the finite field element $x$ in
base $g$.

\syn{fflog}{x,g}

\subsecidx{fforder}$(x,\{o\})$: multiplicative order of the finite field
element $x$.  If $o$ is present, it is assumed to be a multiple of the order of
the element.

\syn{fforder}{x,o} where an omitted $o$ is coded as \kbd{NULL}.

\subsecidx{ffprimroot}$(x)$: return a primitive root of the multiplicative
...
\subsecidx{isprime}$(x,\{\fl=0\})$: true (1) if $x$ is a prime
...
\kbd{http://oldweb.cecm.sfu.ca/pseudoprime/}).
...
\subsecidx{stirling}$(n,k,\{\fl=1\})$: \idx{Stirling number}
of first kind $s(n,k)$ ($\fl=1$, default) or of the second kind 
$S(n,k)$ (\fl=2).

\syn{stirling}{n,k,\fl}, where $n$ and $k$ must be a \kbd{long}.
Also available are
\tet{stirling1}$(n,k)$ (for $\fl=1$) and
\tet{stirling2}$(n,k)$ (for $\fl=2$).
...
\subsecidx{elleta}$(\var{om})$: returns the quasi-periods $[\eta_1,\eta_2]$
...
\subsecidx{ellsigma}$(E,z,\{\fl=0\})$: $E$ being given by \kbd{ellinit},
...
\subsecidx{ellzeta}$(E,z)$: $E$ being given by \kbd{ellinit}, returns the
...
\var{bnf} contains the units, in which case \kbd{bnf.fu} is recommended
...
Note that when \kbd{D} is an idele, this implies also sign conditions on the
embeddings.
...
\subsecidx{polchebyshev}$(n,\{\fl=1\}, \{v=x\})$: creates the $n^{\text{th}}$
\idx{Chebyshev} polynomial of the first kind $T_n$ ($\fl=1$) or the second 
kind $U_n$ ($\fl=2$), in variable $v$.

\syn{polchebyshev}{n,\fl,v}, where $n$, $\fl$ and $v$ are \kbd{long}
integers ($v$ is a variable number). Also available are
\funs{polchebyshev1}{} and \funs{polchebyshev2}{} for $T_n$ and $U_n$ respectively.
...
\syn{polcyclo}{n,v}, where $n$ and $v$ are \kbd{long}
...
\subsecidx{polhermite}$(n,\{v=x\})$: creates the $n^{\text{th}}$
\idx{Hermite} polynomial $H_n$ in variable $v$.

\syn{polhermite}{n,v}, where $n$ and $v$ are \kbd{long}
...
\syn{pollegendre}{n, v}, where $n$ and $v$ are \kbd{long}s and an omitted $v$
is coded as $-1$.
...
\syn{polsubcyclo}{n,d,v}, where $n$, $d$ and $v$ are \kbd{long}s and $v$ is a
variable number. When $(\Z/n\Z)^*$ is cyclic, you can use
\funs{polsubcyclo}{n,d,v}, where $n$, $d$ and $v$ are \kbd{long} and $v$ is a
variable number.
...
\syn{indexrank}{x}. The two components $y$ and $z$ are \typ{VECSMALL}s.
...
\subsecidx{matinverseimage}$(M,y)$: given a matrix $M$ and
...
$M$ may be obtained by \kbd{matker}.
\bprog
  ? M = [1,2;2,4];
...
\funs{indexsort}{x} (= \funs{vecsort0}{x,\kbd{NULL},1}). This function
returns a \typ{VECSMALL} $v$, where $v[1]\dots v[n]$ contain the indices.
...
\subsecidx{derivnum}$(X=a,\var{expr})$: numerical derivation of \var{expr} with
respect to $X$ at $X=a$.
...
\misctitle{Specifying the behaviour at endpoints.}
...
$\var{sgn}=-1$), $sig$ is the abscissa of integration coded as in \kbd{sumnum},
...
\subsubsecidx{getrand}$()$: returns the current value of the seed used by the
...
\subsubsecidx{setrand}$(n)$: reseeds the random number generator using the
seed $n$. No value is returned. The seed is either a technical array output
by \kbd{getrand}, or a small positive integer, used to generate
deterministically a suitable state array. For instance, running a randomized
computation starting by \kbd{setrand(1)} twice will generate the exact same
output.
