...
will operate a little faster on such objects and memory consumption will be
lower. On the other hand, care should be taken to avoid creating too many
such objects.
...
\item for $\fl=1$: \funs{gmodulo}{x,y}.

\item for $\fl=0$: \funs{gmodulcp}{x,y}.
...
\syn{weber0}{x,\fl,prec}, or \funs{wf}{x,prec}, \funs{wf1}{x,prec} or
\funs{wf2}{x,prec}.
...
\syn{gcarrecomplet}{x,\&n}. Also available is \funs{gcarreparfait}{x}.
...
\syn{ellheight0}{E,z,\fl,\var{prec}}. The Archimedean contribution alone is
given by the library function \funs{hell}{E,z,\var{prec}}. Also available are
...
\funs{weipell}{E,\var{precdl}} for the power series (in
$x=\kbd{polx[0]}$).
...
at this point, \kbd{gp} will issue a warning, then \emph{transform your
...
is \funs{nfbasis}{x,\&d,\fl,fa}, where $d$ will receive the discriminant of
the number field (\emph{not} of the polynomial $x$), and an omitted $fa$ should
be input as \kbd{NULL}. Also available are \funs{base}{x,\&d} ($\fl=0$),
\funs{base2}{x,\&d} ($\fl=2$) and \funs{factoredbase}{x,fa,\&d}.
...
\funs{lllgram}{x,\var{prec}} ($\fl=0$), \funs{lllgramint}{x} ($\fl=1$), and
\funs{lllgramkerim}{x} ($\fl=4$).
