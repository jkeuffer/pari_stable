...
\kbd{!=}. On the other hand, \kbd{=} is definitely \var{not} a synonym for
...
\syn{mpeuler}{\var{prec}} where $\var{prec}$ \var{must} be
...
\syn{mppi}{\var{prec}} where $\var{prec}$ \var{must} be given.
...
$\bullet$ $\var{\idx{ideal}}$ can mean any of the following:

\quad -- a $\Z$-basis, in \idx{Hermite normal form}
(HNF) or not. In this case $x$ is a square matrix.

\quad -- an \var{\idx{idele}}, i.e.~a 2-component vector, the first
...
$\bullet$ An \var{\idx{ideal list}} will be a row vector of (fractional)
ideals of the number field $\var{nf}$.

$\bullet$ A \var{\idx{pseudo-matrix}} will be a pair $(A,I)$ where $A$ is a
...
$\bullet$ The \var{\idx{module}} generated by a pseudo-matrix $(A,I)$ is
the sum $\sum_i{\Bbb a}_jA_j$ where the ${\Bbb a}_j$ are the ideals of $I$
and $A_j$ is the $j$-th column of $A$.

$\bullet$ A pseudo-matrix $(A,I)$ is a \var{\idx{pseudo-basis}} of the module
...
result is \var{not} identical to what \kbd{bnfinit} would yield, but is
...
\subsecidx{galoisfixedfield}$(\var{gal},\var{perm},\{p\})$: \var{gal} 
being be a Galois field as output by galoisinit and \var{perm} an
 element of $\var{gal\/}[6]$ or a vector of such elements, compute the
 fixed field of \var{gal} by the automorphims defined by the
 permutations \var{perm} of the roots $\var{gal\/}[3]$.

Output format is the same that \kbd{nfsubfield}, return $[P,x]$ such
 that P is a polynomial defining the fixed field of $\var{gal\/}[1]$
 by the subgroup generated by \var{perm}, and $x$ is a root of $P$ in
 \var{gal} expressed as a polmod in $\var{gal\/}[1]$.

 $P$ is garanteed to be squarefree modulo $\var{gal\/}[2]$.  If $p$ is
 present, it must be a prime number such that $\var{gal\/}[1]$ is
 squarefree modulo $p$, in this case $P$ is also garanteed to be
 squarefree modulo $p$.

\syn{galoisfixedfield}{\var{gal},\var{perm},p}.

\subsecidx{galoisinit}$(\var{K},\{den\})$: Compute the Galois group and all neccessary informations for computing the fixed fields of of the Galois extension $K/\Q$. $\var{pol}$ can be a defining monic irreducible polynomial in $\Z[X]$ or a number field as output by nfinit.

The field must be Galois with Galois group weakly super-solvable (see \kbd{nfgaloisconj})

\misctitle{Warning:} The interface of this function is experimental, so the described output can be subject to important changes in the near future. However the function itself should works as described. For any remarks about improving this interface , please mail allomber@math.u-bordeaux.fr.

The output is an 8-component vector \var{gal}.

 $\var{gal\/}[1]$ contains the polynomial \var{pol} %(\kbd{\var{nf}.pol}).

 $\var{gal\/}[2]$ contains a prime number $l$ such that \var{pol} totally split modulo $l$.

 $\var{gal\/}[3]$ is a vector $L$ containing the $l$-adic roots of \var{pol} (as an \typ{INTMOD} and not a \typ{PADIC}).

 $\var{gal\/}[4]$ is the inverse of the Van der Monde matrix of the  $l$-adic roots of \var{pol}, multiplied by $\var{gal\/}[5]$.

 $\var{gal\/}[5]$ is a multiple of the least common denominator of the automorphisms expressed as polynomial in a root of \var{pol}.

 $\var{gal\/}[6]$ is the Galois group $G$ expressed as a vector of permutations of $L$.

 $\var{gal\/}[7]$ is a generating subset $S=[s_1,\ldots,s_g]$  of $G$ expressed as a vector of permutations of $L$.

 $\var{gal\/}[8]$ contains the relative orders $[o_1,\ldots,o_g]$ of the generators of $S$.

We have the following properties:

\quad$\bullet$ $G$ is super-solvable if and only if $[o_1,\ldots,o_g]$ is decreasing.
 
\quad$\bullet$ if not , set $H$ the maximal normal supersolvable subgroup of $G$,

\quad$\bullet$ if $G/H\cong A_4$ then $[o_1,\ldots,o_g]$ ends by $[2,2,3]$.

\quad$\bullet$ if $G/H\cong S_4$ then $[o_1,\ldots,o_g]$ ends by $[2,2,3,2]$.

\quad$\bullet$ for $1\leq i \leq g$ the subgroup of $G$ generated by $[s_1,\ldots,s_g]$ is normal, with the exception of $i=g-2$ in the second case and of $i=g-3$ in the third.

\quad$\bullet$ the relative order $o_i$ of $s_i$ is its order in the quotient group $G/\langle s_1,\ldots,s_{i-1}\rangle$, with the same exceptions.

\quad$\bullet$ for any $x\in G$ there exists an unique family $[e_1,\ldots,e_g]$ such that (no exceptions):

-- for $1\leq i \leq g$ we have $0\leq e_i<o_i$ 
	
-- $x=g_1^{e_1}g_2^{e_2}\ldots g_n^{e_n}$

In this implementation the $o_i$ are prime powers. It can be relaxed in the future.

If present $den$ must be a suitable value for $\var{gal\/}[5]$.

\syn{galoisinit}{\var{gal},\var{den}}.

\subsecidx{galoispermtopol}$(\var{gal},\var{perm})$: \var{gal} being a galois field as output by galoisinit and \var{perm} a element of $\var{gal\/}[6]$, return the polynomial defining the Galois automorphism, as output by \kbd{nfgaloisconj}, associated with the permutation \var{perm} of the roots $\var{gal\/}[6]$.
...
technical and cannot be explained here. Note that this is \var{not} the
...
squarefree. When one of the polynomials is irreducible (say \kbd{x}), it is
...
called the \var{\idx{Steinitz class}} of the module $\Z_L$.
...
matrix of maximal rank, finds the \var{upper triangular} \idx{Hermite normal
form} of $x$. If the rank of $x$ is equal to its number of rows, the result
is a square matrix. In general, the columns of the result form a basis of the
...
$\bullet$ 1: \var{\idx{parametric plot}}. Here \var{expr} must be a vector with
...
$\bullet$ 2: \var{\idx{recursive plot}}. If this flag is set, only \var{one}
...
In short, typing \kbd{default()} has the same effect as \b{d}.
...
\var{used} to be the fate of statically linked gp on \kbd{Linux} and
