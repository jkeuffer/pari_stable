...
i.e.~such that $\text{Arg}(\text{sqrt}(x))\in{} ]-\pi/n, \pi/n]$. Integermod
...
\subsubsecidx{type}$(x,\{t\})$: this is useful only under \kbd{gp}. If $t$ is
...
existing type names with the metacommand \b{t}.\label{se:gptype}
  ...
  \kbd{gp} will not let you create meaningless objects in this way where the internal
