...
arithmetic functions in the narrow sense, like \teb{eulerphi}, do \var{not}
...
case, the remaining part is not necessarily prime. See \tet{factorint} for
...
polynomials in the same variable, with rational coefficients).
...
If $\fl=0$, use a combination of (strong) Rabin-Miller tests (with end
matching) for small $x$, Selfridge ``$p-1$'' test if $x-1$ is smooth enough,
and APRCL for general $x$.

If $\fl=1$, use Selfridge-Pocklington-Lehmer ``$p-1$'' test and output a
primality certificate as follows: return 0 if $x$ is composite, 1 if $x$ is a
small prime (currently $x < 341 550 071 728 321$) passing enough Rabin-Miller
tests to guarantee its primality, and a matrix if $x$ is a large prime.  The
matrix has three columns. The first contains the prime factors $p$, the
second the corresponding elements $a_p$ as in Proposition~8.3.1 in GTM~138,
and the third the output of isprime(p,1).  The algorithm fails if one of the
pseudo-prime factors is not prime, which is exceedingly unlikely (and well
worth a bug report).

If $\fl=2$, use Adleman-Pomerance-Rumely-Cohen-Lenstra APRCL.

\syn{gisprime}{x,\fl}, but the simpler function $\teb{isprime}(x)$
which returns a \kbd{long} should be used if $x$ is known to be of
type integer. Also available is $\teb{plisprime}(N,1)$, corresponding to
$\teb{gisprime}(x,1)$ if $x$ is known to be of type integer.
...
If $\fl = 0$, checks whether $x$ is a strong Miller-Rabin pseudo prime
for the base $2$ and a strong Lucas pseudoprime (there are no known composite
numbers passing this test).

If $\fl > 0$, checks whether $x$ is a strong Miller-Rabin pseudo prime for
$\fl$ randomly chosen bases.

\syn{gispseudoprime}{x,\f}, but the simpler function $\teb{ispseudoprime}(x)$
...
\syn{bnrconductor}{a_1,a_2,a_3,\fl}, where an omitted argument
...
\subsecidx{galoissubcyclo}$(N,H,\{fl=0\},\{v\})$: computes the abelian
extension of the rational field given by the modulus \var{N} and the
congruence subgroup \var{H}.

\noindent The modulus \var{N} can be: 

$\bullet$ A module, i.e an ideal in any form for \kbd{nfinit(y)}, or a
2-component row vector containing an ideal and a 1-component row vector of
flags indicating whether the infinite place is included in the modulus. For
instance, a non-zero integer is a valid input, corresponding to a real
abelian field.

$\bullet$ The output of \kbd{bnrinit(bnfinit(y), m, 1)} where \kbd{m} is
a module as above

$\bullet$ The output of \kbd{znstar(n)}, in which case the modulus is
assumed to be \kbd{[n, [1]]}, i.e $n\infty$.

\noindent The subgroup \var{H} can be given by:

$\bullet$ a matrix, taken to be a HNF left divisor of the SNF
giving the ray class group modulo \var{N}.

$\bullet$ a generator, i.e. an integer or an integer modulo \var{n}.

$\bullet$ a vector of generators.

Beware that \var{H} is understood relatively to \var{N}; in particular, if
the infinite place does not divide the module, e.g if \var{N} is an integer,
then it is not a subgroup of $(\Z/N\Z)^*$, but of its quotient by 
$\{\pm 1\}$. For instance, \kbd{galoissubcyclo(8, 3)} is $\Q$ since it has
Galois group $(\Z/8\Z)^* / \{-1,3\} = \{1\}$, whereas
\kbd{galoissubcyclo([8,[1]], 3)} is a quadratic field.

If $fl=0$, compute a polynomial (in the variable \var{v}) defining the class
field associated to $(N,H)$, i.e the subfield of $\Q(\zeta_n)$ fixed by the
subgroup \var{H} of $(\Z/n\Z)^*$ if $\infty | N$ or $(\Z/n\Z)^*/\{\pm1\}$
if not. 
...
$\Q(\zeta_n)$ (of degree less than \kbd{d}, if \kbd{d} is set):
\bprog
subcyclo(n, d = -1)=
{
  local(Z,G, S = []);
  ...
  G = matdiagonal(Z[2]);
  forsubgroup(H = G, d,
    S = concat(S, galoissubcyclo(Z, mathnf(concat(G,H))));
  );
  S
}
@eprog\noindent
Alternatively this one computes subfields of exact degree $d$
\bprog
subcyclo(n, d)=
{
  local(bnr,L);
  bnr = bnrinit(bnfinit(y), [n,[1]], 1);
  L = subgrouplist(bnr, [d], 1);
  vector(#L,i, galoissubcyclo(bnr,L[i]));
}
@eprog\noindent
In the latter example, setting \kbd{L = subgrouplist(bnr, [d])} would produce
subfields of exact conductor $n\infty$.
...
\subsecidx{rnfconductor}$(\var{bnf},\var{pol},\{\fl=0\})$: $\var{bnf}$
being a big number field as output by \kbd{bnfinit}, and \var{pol} a
relative polynomial defining an \idx{Abelian extension}, computes the
class field theory conductor of this Abelian extension. The result is
a 3-component vector $[\var{conductor},\var{rayclgp},\var{subgroup}]$,
where \var{conductor} is the conductor of the extension given as a
2-component row vector $[f_0,f_\infty]$, \var{rayclgp} is the full ray
...
main variable of $\var{ext}[1]$, say $x$ (see \secref{se:priority}).
...
\syn{subcyclo}{n,d,v}, where $n$, $d$ and $v$ are \kbd{long} and $v$ is a
variable number.
...
a rational function. $y$ must be a simple variable name, or a monome of the
form $t^k$ for some simple variable $t$. In the latter case, substitution
may not always be possible since PARI doesn't know about algebraic functions.
...
coefficients, this function computes a \var{multiple} of the determinant of the
...
matrix, finds the \var{upper triangular} \idx{Hermite normal form} of $x$. If
...
Outputs a two-component row vector $[H,U]$, where $H$ is the \var{upper
