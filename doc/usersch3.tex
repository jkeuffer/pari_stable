...
cases of \teb{gegal} which should be used are $\teb{gcmp0}(x)$ ($x==0$ ?),
$\teb{gcmp1}(x)$ ($x==1$ ?), and
$\teb{gcmp_1}(x)$ ($x==-1$ ?).
...
\subsecidx{bitand}$(x,y)$:\label{se:bitand} bitwise \tet{and}\sidx{bitwise
and} of two integers $x$ and $y$, that is the integer
...
exclusive or} of two integers $x$ and $y$, that is the integer $$\sum
(x_i~\kbd{xor}~y_i) 2^i$$
...
vector $x$ (or the single integer $x$) to the table computed upon \kbd{gp}
...
absolute value of $x$ if $x$ is rationnal (\typ{INT} or \typ{FRAC}), and $x$
...
If $y$ is omitted and $x$ is a vector, return the $\gcd$ of all
...
of the type given by \kbd{ellinit(,1)}. If $\fl=0$, this
...
%
In particular, the discriminant is \kbd{$E$.disc}, and the
$j$-invariant is \kbd{$E$.j}.
...
\item The \tev{module} generated by a pseudo-matrix $(A,I)$ is
...
unless the ideal is trivial; \var{bnr} by \kbd{bnrclass} (with $\fl>0$) or
\kbd{bnrinit}. This is the ground field.

\item \emph{module} is either an ideal in any form (see above) or a
...
\kbd{nf}, \kbd{bnf}, \kbd{bnr} respectively. They are meant to take as first
argument a number field of that precise type, respectively output by
\kbd{rnfinit}, \kbd{nfinit}, \kbd{bnfinit}, and \kbd{bnrinit}.
...
the program requires a \kbd{bnf}, the program will \emph{not} launch
...
non-monic polynomial at this point, \kbd{gp} will issue a warning, then
...
Hypothesis\sidx{GRH}. If it is correct, the answer is 1. If not, the program
...
the section or the keyword \kbd{bnf}). Careful use of this parameter may
...
\subsecidx{bnfinit}$(P,\{\fl=0\},\{\var{tech}=[\,]\})$: initialized a
\var{bnf}, used in programs such as \kbd{bnfisprincipal}, \kbd{bnfisunit} or
\kbd{bnfnarrow}. By default, the results are conditional on a heuristic
...
the regulator \kbd{\var{bnf}.reg}, a $1$ (used to contain an obsolete
``check number''), the number of roots of unity and a generator
\kbd{\var{bnf}.tu}, the fundamental units \kbd{\var{bnf}.fu}. If the
precision becomes insufficient, \kbd{gp} outputs a warning (\kbd{fundamental
units too large, not given}) and does not strive to compute the units by
default ($\fl=0$).
...
\kbd{bnrinit}, and that this flag provides negligible time savings. In short,
it is deprecated.
...
the ideal is trivial), and \var{ideal} being either an ideal in any form or a
two-component row vector containing an ideal and an $r_1$-component row
vector of flags indicating which real Archimedean embeddings to take in the
module, computes the ray class group of the number field for the module
\var{ideal}, as a 3-component vector as all other finite Abelian groups
...
output by \kbd{bnfinit} (units are mandatory unless the ideal is trivial),
and \var{list} being a list of modules as output by \kbd{ideallist} of
\kbd{ideallistarch}, outputs the list of the class numbers of the
...
norm up to bound \var{bound}, where the ramified Archimedean places are given
by \var{arch} (unramified at infinity if \var{arch} is void or omitted). If
\fl\ is non-zero, give \var{arch} all the possible values. (See \kbd{bnr} at
...
The components of \kbd{P} may conveniently be accessed by member functions:
\kbd{P.p}, \kbd{P.e}, \kbd{P.f}, and \kbd{P.gen} (returns the vector
$[p,a]$).
...
\subsecidx{O}$(p$\kbd{\pow}$e)$: $p$-adic (if $p$ is an integer greater or
equal to 2) or power series zero (in all other cases), with precision given
by $e$.

\syn{zeropadic}{p,e} for a $p$-adic and \teb{zeroser}(v,e) for a
power series zero in variable $v$ (so $v$ is a long and $p = \kbd{polx[v]}$),
where the precision $e$ is a \kbd{long}.
...
\subsecidx{algdep}$(x,k,\{\fl=0\})$:\sidx{algebraic
dependence}\label{se:algdep} $x$ being real, complex, or $p$-adic, finds a
...
\subsecidx{matfrobenius}$(M,\{\fl=0\})$: returns the Frobenius form of the
square matrix \kbd{M}. If $\fl=1$, returns only the elementary divisors. If
$\fl=2$, returns a two-components vector [F,B] where \kbd{F} is the Frobenius
form and \kbd{B} is the basis change so that $M=B^{-1}FB$.

\syn{matfrobenius}{M,\fl}.
...
functions are mainly useful for GP programming. Their use library mode is
tricky and its explanation will be mostly omitted, although the reader can
try and figure it out by himself by checking the example given for \tet{sum}.

On the other hand, numerical functions code a function (to be integrated,
...
The first is a pointer to a function whose first parameter is a \kbd{GEN} and
the second a \kbd{void*}, and it is assumed that \kbd{eval(x, E)} returns
the value of your function evaluated at \kbd{x}. For instance, one might code
the translation by (a fixed) \kbd{t} via
\bprog
GEN translate(GEN x, void *E) { return gadd(x, (GEN)E); }
@eprog
One would then use \kbd{\&translate} as evaluation function \kbd{eval},
and \kbd{(void*)t} as data \kbd{E}. Of course, you can set \kbd{E} to a
pointer to any \kbd{struct} (typecast to \kbd{void*}), so the above mechanism
handles arbitrary functions. For simple functions without extra parameters,
you may set \kbd{E = NULL} and ignore that argument in your function
definition.
...
integration method has been implemented in the routine \tet{intnum} and its
variants. Romberg integration is still available under the name
\kbd{intnumromb}, but superseded. It is possible to compute numerically
integrals to thousands of decimal places in reasonable time, as long as the
integrand is regular, as is usually the case. It is also reasonable to
compute numerically integrals in several variables, although more than two
becomes lengthy. The integration domain may be non-compact, and the integrand
may have reasonable singularities at endpoints. To use \kbd{intnum}, the user
must split the integral into a sum of subintegrals where the function has
(possible) singularities only at the endpoints. Polynomials in logarithms are
not considered singular, and neglecting these logs, singularities are assumed
to be algebraic (in other words asymptotic to $C(x-a)^{-\alpha}$ for some
$\alpha$ such that $\alpha>-1$ when $x$ is close to $a$), or to correspond to
simple discontinuities of some (higher) derivative of the function. For
instance, the point $0$ is a singularity of $\text{abs}(x)$.
...
\synt{intcirc}{void *E, GEN (*eval)(GEN,void*), GEN a,GEN R,GEN tab, 
long prec}.
...
\synt{intfouriercos}{void *E, GEN (*eval)(GEN,void*), GEN a, GEN b, GEN z, 
GEN tab, long prec}.
...
\synt{intfourierexp}{void *E, GEN (*eval)(GEN,void*), GEN a, GEN b, GEN z, 
GEN tab, long prec}.
...
\synt{intfouriersin}{void *E, GEN (*eval)(GEN,void*), GEN a, GEN b, GEN z, 
GEN tab, long prec}.
...
\synt{intfuncinit}{void *E, GEN (*eval)(GEN,void*), GEN a,GEN b,long m, 
long flag, long prec}. Note that the order of $m$ and \fl are reversed
compared to the \kbd{GP} syntax.
...
time = 700 ms.
%3 = 3.94... E-97 + 4.76... E-98*I \\@com better
...
time = 330 ms.
%7 = 1.07... E-72 + 3.20... E-72*I \\@com too far now...
@eprog

\synt{intlaplaceinv}{void *E, GEN (*eval)(GEN,void*), GEN sig,GEN z, GEN tab, 
long prec}.
...
\synt{intmellininv}{void *E, GEN (*eval)(GEN,void*), GEN sig, GEN z, GEN tab, 
long prec}.
...
time = 51,300 ms. \\@com slow because of $\Gamma(s)^3$.
? tab = intfuncinit(t=[[-1], 3*Pi/2],[[1], 3*Pi/2], gamma(2+I*t)^3, 1);
time = 25,370 ms.
? M(z) = intmellininvshort(2,z,tab)
? M(4) - A
time = 1,040 ms. 
%4 = 0.E-310 + 4.22... E-312*I \\@com 50 times faster than \kbd{intmellininv} and perfect.
? \p 105
? tab = intfuncinit(t=[-1], [1], gamma(2+I*t)^3, 1);
time = 580 ms.
? A = intmellininv(s=2,4,gamma(s)^3);
time = 1190 ms. \\@com at 105D, slow but reasonable.
? M(4) - A
time = 30 ms.
%8 = 3.4... E-108 + 1.20... E-109*I \\@com 40 times faster and perfect.
@eprog\noindent
In the computation of \var{tab} we did not include the exact exponential
decrease of $\Gamma(2+it)^3$, namely $\alpha = 3\pi/2$, although the result
...
? test(m - 3)
time = 120 ms.
%3 = -7.23... E-60 \\@com $64 = 2^6$ times faster, only $60$ correct decimals.
@eprog

\synt{intnall}{void *E, GEN (*eval)(GEN,void*), GEN a,GEN b,GEN tab, 
long prec}, where $\kbd{eval}(x, E)$ returns the value of the function at $x$
and an omitted \var{tab} is coded as \kbd{NULL}.
...
\synt{intnumromb}{void *E, GEN (*eval)(GEN,void*), GEN a, GEN b, long flag, 
long prec}, where $\kbd{eval}(x, E)$ returns the value of the function at $x$.
...
\synt{prodinf}{void *E, GEN (*eval)(GEN,void*), GEN a, long prec} ($\fl=0$), or
...
$sig$ is coded as follows: either it is a real number $\sigma$, and then the
function $f$ is assumed to decrease at least as $1/X^2$ at infinity, but not
exponentially. Or it is a two-component vector $[\sigma,\alpha]$, where
$\sigma$ is as before, $\alpha<-1$, and the function $f$ is then assumed to
decrease like $X^{\alpha}$ (so $\alpha\le-2$ is equivalent to no $\alpha$ at
all). Or it is a two-component vector $[\sigma,\alpha]$, where $\sigma$ is as
before, $\alpha>0$, and the function $f$ is then assumed to decrease like
$\exp(-\alpha X)$. In the latter case it is essential that $\alpha$ be
exactly the rate of exponential decrease, and it is usually a good idea to
increase by $1$ the default value of $m$ used for the integration step.

In practice, if the function is exponentially decreasing the \kbd{sumnum}
function is slower than \kbd{sumpos} or \kbd{suminf}. Uses the \tet{intnum}
routines, and \var{tab} is as in intnum, but if used must be initialized with
\kbd{sumnuminit}. At low precision, if \var{tab} is not initialized, can
be slower than \kbd{sumpos}, when the latter is applicable. However in
general faster for slowly decreasing functions. If \fl is nonzero, assumes
that the function $f$ to be summed is of real type, i.e. satisfies
$\overline{f(z)}=f(\overline{z})$, and then up to twice faster when \var{tab}
is precomputed:
...
time = 1,410 ms.
? tab = sumnuminit(2);
time = 1,620 ms. \\@com slightly slower, but done once and for all.
? b = sumnum(n=1, 2, 1/(n^3+n+1), tab);
time = 460 ms. \\@com 3 times as fast as @kbd{sumpos}
? a - b
%4 = -1.004... E-306 + 0.E-320*I \\@com perfect.
? sumnum(n=1, 2, 1/(n^3+n+1), tab, 1) - a; \\@com function of real type
time = 240 ms. 
%2 = -1.004... E-306 \\@com twice as fast, still perfect and no imaginary part.
...
\synt{sumnum}{void *E, GEN (*eval)(GEN,void*), GEN a,GEN sig,GEN tab,long
flag, long prec}.
...
function for $\Re(X)\ge sig$ (or $sig[1]$). $sig$ is coded as in \kbd{sumnum}.
However for slowly decreasing functions (where $sig$ is coded as
$[\sigma,\alpha]$ with $\alpha<-1$), it is not really important to indicate
$\alpha$. In fact, as for \kbd{sumalt}, the program will often give meaningful
results (usually analytic continuations) even for divergent series.
On the other hand the exponential decrease must be indicated.
Uses the intnum routines, and \var{tab} is as in intnum, but if used must be 
initialized with \kbd{sumnuminit}. If \fl is nonzero, assumes that the 
function $f$ to be summed is of real type, i.e. satisfies 
$\overline{f(z)}=f(\overline{z})$, and then twice faster when
\var{tab} is precomputed: Orders of magnitude slower than \kbd{sumalt}, so not
to be used in practice, but only given for completeness. Warning: the 
expression \var{expr} must {\bf not} include the $(-1)^X$ coefficient. Thus 
$\kbd{sumalt}(n=a,(-1)^nf(n))$ is (approximately) equal to 
$\kbd{sumnumalt}(n=a,sig,f(n))$.

\bprog
? \p 308
? tab = sumnuminit(2,,-1); \\@com abcissa $\sigma=2$, alternating sums.
...
\synt{sumnumalt}{void *E, GEN (*eval)(GEN,void*), GEN a,GEN sig,GEN tab,long
flag, long prec}.
...
\item Finally, note that in the same way that \kbd{printtex} allows you
...
must be under \kbd{gp} to use them}. The reason for that is that you really should
...
this way. Of course \kbd{gp} won't protest if you don't do it.
...
is void. \kbd{gp} uses a variant which ends by a \kbd{longjmp}.
...
the name \var{gpname} in this \kbd{gp} session, with argument code \var{code} (see
...
re-reads the last file that was fed into \kbd{gp}. The return value is the result of
