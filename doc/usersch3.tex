...
\subsec{Quotient by maximal ideals}

For a given maximal ideal \var{pr}, elements in \var{nf} which have
non-negative valuation at \var{pr} can be mapped to the residue field.
A \tev{modpr} structure describes this map, and is obtained via
\teb{nfmodprinit} from \var{nf} and \var{pr}. This is used
in all functions whose name is of the form \kbd{nf...modpr} and
\kbd{nffactormod} (which should have been called \kbd{nffactormodpr}).

An element in the residue field is represented by a canonical
representative in the maximal order, given by a \typ{COL}.

\misctitle{Interface design bug.} There is currently no satisfactory way
to work over finite fields in GP (the \kbd{libpari} situation is much
better due to a wealth of specialized functions). Hence the above
interface is awkward and inefficient: the internal data type used (an
element in $\F_p[X]/(T)$ for some suitable $T$) is different from the
public type available under GP, leading to many unnecessary format
conversions (transparent but slow). This must be improved. In particular,
all \kbd{modpr} functions should disappear, and be replaced by just two
functions: a projection to the residue field and a lift to the maximal
order. All other computations should be done by generic finite fields
functions having no relation to \var{nf} structures. Most of these
functions are implemented internally but are not available under GP.
...
of variables. In other routines, the variable change is lost.

You should probably not tamper with the technical vector but, for
completeness, we explain how it is used. The numbers $0 < c \leq c_2$ control
the execution time and the stack size. Let $\Delta$ be the absolute value of
the field discriminant, and $S(c)$ the set of prime ideals of
norm less than $c (\log \Delta)^2$. The algorithm looks for $S(c)$-smooth
principal ideals; the larger $c$, the easier it is to find smooth elements,
but the number of needed relations increases correspondingly. We then compute
a tentative class group assuming $S(c)$ generates the true class group, then
checks whether prime ideals in $S(c_2)$ are generated by the ideal classes of
the elements of $S(c)$.

The algorithm checks that, \emph{under the GRH}, the primes belonging to
$S(c_2)$ generate the class group, and the result is correct under this
assumption. A famous result of Bach asserts that $c_2 = 12$ is always
sufficient (under GRH) but we use a computational bound instead, due to
Belabas, Diaz y Diaz and Friedman. If this computed bound does not guarantee
our assumption is correct, then the computation is restarted with larger $c$
and $c_2$. The default values are $c = c_2 = 0.3$.
...
relations. Otherwise, reasonable values are between 4 and 20. The default is~4.

\misctitle{Warning.} All \kbd{bnf} routines depend on the correctness of the
GRH (for the underlying field and Hecke $L$-function of unramified
characters)! If the GRH does not hold, any of the class number, class group
structure, class group generators, regulator and fundamental units may be
wrong, independently of each other. Any result computed from such a \kbd{bnf}
may be wrong. The only guarantee is that the units given generate a subgroup
of finite index in the full unit group. You can use \kbd{bnfcertify} to
certify the computations unconditionally.
...
\kbd{bnfisunit} or \kbd{bnfnarrow}. The results are conditional
on the GRH, see \ref{se:GRHbnf}. The result is a 10-component vector
\var{bnf}.
...
\subsecidx{nffactormod}$(\var{nf},x,\var{pr})$: factors of the univariate
polynomial $x$ modulo the prime ideal \var{pr} in the number field
\var{nf}. The coefficients of $x$ are algebraic numbers in \var{nf}
(which are \var{pr}-integers). The prime ideal \var{pr} is as output by
\kbd{idealprimedec} or \kbd{nfmodprinit}. The latter is preferable if
more than one operation modulo the same prime ideal are needed.

The main variable of $\var{nf}$ must be of lower priority than that of
$x$ (see \secref{se:priority}). However if the coefficients of the number
field occur explicitly (as polmods) as coefficients of $x$, the variable
of these polmods \emph{must} be the same as the main variable of $t$ (see
\kbd{nffactor}).
...
\subsecidx{thue}$(\var{tnf},a,\{\var{sol}\})$: solves the equation $P(x,y)=a$
in integers $x$ and $y$, where \var{tnf} was created with
$\kbd{thueinit}(P)$. \var{sol}, if present, contains the solutions of
$\Norm(x)=a$ modulo units of positive norm in the number field defined by $P$
(as computed by \kbd{bnfisintnorm}). If the result is conditional on the GRH,
