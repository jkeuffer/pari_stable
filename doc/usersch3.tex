...
\subsecidx{nfsubfields}$(\var{nf},\{d=0\})$: finds all subfields of degree
$d$ of the number field $\var{nf}$ (all subfields if $d$ is null or omitted).
The result is a vector of subfields, each being given by $[g,h]$, where $g$
is an absolute equation and $h$ expresses one of the roots of $g$ in terms of
the root $x$ of the polynomial defining $\var{nf}$. This uses J.~Kl\"uners
algorithm, and is very slow when no sufficiently inert primes can be found,
e.g.~when \var{nf} is a compositum of many quadratic fields.
\sidx{Galois}\sidx{subfield}

It may also be quite difficult to compute an \var{nf} in the first place, in
particular when the discriminant of the defining polynomial is hard to factor.
In the case of abelian fields such as above, we can solve both problems by
using \tet{galoisinit} and \tet{galoisfixedfield}, as in
\bprog
  subf(pol) = 
  { 
...
@eprog
which returns the same output as \kbd{nfsubfields(nfinit(pol))}. For
non-abelian Galois fields, you can still use \tet{galoisinit} and
\teb{galoisfixedfield}, but you will have to use another package than PARI to
compute the subgroup lattice, since \tet{forsubgroup} is restricted to the
abelian case.
