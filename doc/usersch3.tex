...
$x-y$, where $x$ and $y$ must be in $\R$, and $\teb{gequal}(x,y)$ which can be
...
cases of \teb{gequal} which should be used are $\teb{gcmp0}(x)$ ($x==0$ ?),
$\teb{gcmp1}(x)$ ($x==1$ ?), and
$\teb{gcmp_1}(x)$ ($x==-1$ ?).
...
The argument $x$ must be positive and $s$ is any complex number. The result
returned is $\int_x^\infty e^{-t}t^{s-1}\,dt$. When $y$ is given, assume (of
course without checking!) that $y=\Gamma(s)$. For small $x$, this will speed
up the computation.
...
The argument $x$ must be positive and not too large, $s$ is not a pole of
$\Gamma$. The result returned is $\int_0^x e^{-t}t^{s-1}\,dt$.
...
\subsecidx{O}$(p\hbox{\kbd{\pow}}e)$: $p$-adic (if $p$ is an integer greater or
