...
To finish with our generic simple-minded example, the {\it library\/} function
...
number (which may be an integer if the quotient is exact) and {\it not\/} the
...
\subseckbd{\pow}: The expression $x\hbox{\kbd{\pow}}y$ is \idx{powering}. If
the exponent is an integer, then exact operations are performed using binary
(left-shift) powering techniques. In particular, in this case the first
argument cannot be a vector or matrix unless it is a square matrix (and
moreover invertible if the exponent is negative). If the exponent is not of
type integer, this is treated as a transcendental function (see
\secref{se:trans}), and in particular has the effect of componentwise
powering on vector or matrices.

\syn{gpow}{x,y,\var{prec}} for $x\hbox{\kbd{\pow}}y$.
...
respectively. The library syntax is ${\it co}(x,y)$, where {\it co} is the
...
($x==0$ ?), $\teb{gcmp1}(x)$ ($x==1$ ?), and\sidx{gcmp\string\_1}
\key{gcmp\_1}$(x)$ ($x==-1$ ?).
...
and {\it not\/} a \kbd{GEN} like \kbd{gle} etc.

\smallskip
GP accepts the following synonyms for some of the above functions: since
there is no bitwise \kbd{and} or bitwise \kbd{or}, \kbd{|} and \kbd{\&} are
accepted as\sidx{bitwise and}\sidx{bitwise or} synonyms of \kbd{||} and
\kbd{\&\&} respectively. Also, \kbd{<>} is accepted as a synonym for
\kbd{!=}. On the other hand, \kbd{=} is definitely {\it not\/} a synonym for
\kbd{==} since it is the assignment statement.
...
polynomial. If $y$ is an integer, $x$ must be an integer. If $y$ is a
polynomial, $x$ must be a scalar or a polynomial. The result is put on the
PARI stack.
...
erased later by garbage collecting (see \secref{se:garbage}). In particular,
care should be taken to avoid creating too many such objects, since the heap
is very small (typically a few thousand objects at most).
...
Warning: this is {\it not\/} a substitution function. It is intended to be
...
function $\teb{polcoeff}(x,n)$ gives the coefficient of degree $n$ of the
polynomial or power series $x$, with respect to the main variable of $x$ (to
see the order of the variables or to change it, use the function
\tet{reorder}, see \secref{se:reorder}). In particular if $n$ is less than
the valuation of $x$ or in the case of a polynomial, greater than the degree,
the result is zero (contrary to \kbd{compo} which would send an error
message). If $x$ is a power series and $n$ is greater than the largest
significant degree, then an error message is issued.
...
the $m^{\text{th}}$ {\it row\/} of $x$, and \tet{x[,n]} represents
the $n^{\text{th}}$ {\it column\/} of $x$.
...
\subsecidx{floor}$(x)$: floor of $x$. When $x$ is in $\R$,
...
For real $x$, $n$ is the number of desired significant {\it decimal} digits.
...
integer and set $e$ to the number of error bits, that is the binary exponent
...
function \teb{taille} returns the number of {\it words} instead.
...
exact value, you an use \kbd{length(Str(x))}, which is much slower.

\syn{gsize}{x} which returns a \kbd{long}.

\subsecidx{truncate}$(x,\{\&e\})$: truncate $x$ and set $e$ to the number of
error bits. When $x$ is in $\R$, this means that the part after the decimal
point is chopped away, integer and set $e$ to the number of error bits that
is the binary exponent of the difference between the original and the
truncated value (the ``fractional part''). If the exponent of $x$ is too
...
{\it number\/} of $x$ if it exists, \kbd{BIGINT} otherwise.
...
\kbd{agm} (arithmetic-geometric mean) are implemented. Note that in the case
of a $2$-adic number, $\kbd{sqr}(x)$ is not identical to $x*x$: for example
if $x = 1+O(2^5)$ then $x*x = 1+O(2^5)$ while $\kbd{sqr}(x) = 1+O(2^6)$.
(Remark: note that if we wanted to be strictly consistent with the PARI
philosophy, we should have $x*y= (4 \mod 8)$ when both $x$ and $y$ are
congruent to $2$ modulo $4$, or $\kbd{sqr}(x)=(4\mod 32)$ when $x$ is
congruent to $2$ modulo $4$. However, since an integermod is an exact object,
PARI assumes that the modulus must not change, and the result is hence $0\,
\mod\, 4$ in both cases. On the other hand, $p$-adics are not exact objects,
...
\syn{mpeuler}{\var{prec}} where $\var{prec}$ {\it must\/} be
given. Note that this creates $\gamma$ on the PARI stack. If one does not
want to create it on the stack but stash it for later use under the global
name \teb{geuler} (with no parentheses), use instead
$\teb{consteuler}(\var{prec})$.
...
\syn{mppi}{\var{prec}} where $\var{prec}$ {\it must\/} be given.
Note that this creates $\pi$ on the PARI stack. If one does not want to
create it on the stack but stash it for later use under the global
...
$\int_x^\infty e^{-t}t^{s-1}\,dt$. When $y$ is given, assume (of course
without checking!) that $y=\Gamma(s)$. For small $x$, this will tremendously
...
\subsecidx{sqr}$(x)$: square of $x$. Not identical to $x*x$ in
...
(and the function may not even return).
...
is computed using \idx{Bernoulli numbers} for
...
sense of the word~--- Euler's totient\sidx{Euler totient function}
function, the M\"obius\sidx{moebius} function, the sums over divisors or
powers of divisors etc.--- call, after trial division by small primes, the
same versatile factoring machinery described under \kbd{factorint}.
It includes \idx{Pollard Rho}, \idx{ECM} and \idx{MPQS}
stages, and has an early exit option for the functions \teb{moebius} and (the
integer function underlying) \teb{issquarefree}.
Note that it relies on a (fairly strong) probabilistic primality test:
numbers found to be strong pseudo-primes after 10 successful trials of
the \idx{Rabin-Miller} test are declared primes.
...
inverse modulo $\langle${\it some integermod}$\rangle$'' shows up afterwards,
you have just stumbled over a non-trivial factor. Note that the arithmetic
functions in the narrow sense, like \teb{eulerphi}, do {\it not\/} use this
extra table.
...
\subsecidx{direuler}$(p=a,b,\var{expr})$: computes the \idx{Dirichlet series}
to $b$ terms of the \idx{Euler product} of expression \var{expr} as $p$ ranges
through the primes from $a$ to $b$. \var{expr} must be a polynomial or
rational function in
another variable than $p$ (say $X$) and $\var{expr}(X)$ is understood as the
Dirichlet series (or more precisely the local factor) $\var{expr}(p^{-s})$.
...
(totient)\sidx{Euler totient function} function of $x$.
$x$ must be of type integer.

\syn{phi}{x}.

\subsecidx{factor}$(x,\{\var{lim}=-1\})$: general factorization function.
If $x$ is of type integer, rational, polynomial or rational function,
the result is a
two-column matrix, the first column being the irreducibles dividing $x$
(prime numbers or polynomials), and the second the exponents. If $x$ is a
vector or a matrix, the factoring is done componentwise (hence the result is
a vector or matrix of two-column matrices).

   If $x$ is of type integer or rational, an argument \var{lim} can be added,
meaning that we look only for factors up to \var{lim}, or to \kbd{primelimit},
whichever is lowest (except when $\var{lim}=0$ where the effect is identical
to setting $\var{lim}=\kbd{primelimit}$). Hence in this case, the remaining
part is not necessarily prime. See \teb{factorint} for more information about
the algorithms used.

   The polynomials or rational functions to be factored must have scalar
coefficients. In particular PARI does {\it not\/} know how to factor
multivariate polynomials.

   Note that PARI tries to guess in a sensible way over which ring you want to
factor. Note also that factorization of polynomials is done up to
multiplication by a constant. In particular, the factors of rational
polynomials will have integer coefficients, and the content of a polynomial or
rational function is discarded and not included in the factorization. If
you need to, you can always ask for the content explicitly:

\bprog%
? factor(t\pow2 + 5/2*t + 1)
\%1 =
[2*t + 1 1]
\smallskip%
[t + 2 1]
\smallskip%
? content(t\pow2 + 5/2*t + 1)
\%2 = 1/2%
\eprog
...
the exponents.  If you want only the {\it degrees\/} of the irreducible
...
gives a result which is an integer, while $\kbd{fact}(x)$ gives a real
number.

\syn{mpfact}{x} for $x!$ and
$\teb{mpfactr}(x,\var{prec})$ for $\kbd{fact}(x)$. $x$ must be a \kbd{long}
...
operation-compatible with $\Z/p\Z$. The result is a two-column matrix, the
first column being the irreducible polynomials dividing $x$, and the second
the exponents. If $\fl$ is non-zero, outputs only the {\it degrees} of the
...
\subsecidx{isprime}$(x)$: true (1) if $x$ is a strong pseudo-prime
for 10 randomly chosen bases, false (0) otherwise.

\syn{gisprime}{x}, but the
...
particular the case when $x$ is an integer or a polynomial. This is {\it not}
...
\syn{gissquarefree}{x}, but the
simpler function $\teb{issquarefree}(x)$ which returns a \kbd{long}
should be used if $x$ is known to be of type integer. This \teb{issquarefree}
is just the square of the M\"obius\sidx{moebius} function, and is computed
as a multiplicative arithmetic function much like the latter.
...
\subsecidx{moebius}$(x)$: M\"obius $\mu$-function of $x$. $x$ must be of type
integer.
...
\subsecidx{omega}$(x)$: number of distinct prime divisors of $x$. $x$ must be
of type integer.
...
when $D<-10^{25}$, $D>10^{10}$, or when the {\it structure\/} is wanted.

If $\fl$ is non-zero {\it and\/} $D>0$, computes the narrow class group and
...
\var{tech} is a row vector of the form $[c_1,c_2]$, where $c_1$ and $c_2$
...
\idx{GRH}) you must take $c_2=6$. Reasonable values for $c$ are between
$0.1$ and $2$.
...
can also be a \kbd{bnf}). \fl\ is only meaningful when $D<0$. If it's an odd
integer, outputs instead the vector of $[\var{ideal},
\var{corresponding root}]$.

If $\fl=0$ or 1, uses the $\sigma$ function, while if $\fl>1$, uses the
Weierstrass $\wp$ function, which is less efficient and may disappear in
future versions (not all special cases have been implemented in this case).
Finally, \fl\ can also be a two-component vector $[\lambda,\fl]$, where
\fl\ is as above and $\lambda$ is the technical element of bnf necessary
for Schertz's method using $\sigma$. In that case, returns 0 if $\lambda$
is not suitable.
...
\subsecidx{quadregulator}$(x)$: regulator of the quadratic field of positive
discriminant $x$. Returns an error if $x$ is not a discriminant (fundamental or not) or
if $x$ is a square. See also \kbd{quadclassunit} if $x$ is large.
...
the prime number table. $x$ can also be a single integer. List the current
...
\syn{sumdiv}{x} (= $\teb{sigma}(x)$) or
$\teb{gsumdivk}(x,k)$ (= $\teb{sigma}(x,k)$), where $k$ is a C long integer.
...
\subsecidx{ellak}$(E,n)$: computes the coefficient $a_n$ of the $L$-function
of the elliptic curve $E$, i.e.~in principle coefficients of a newform of
weight 2 assuming \idx{Taniyama-Weil}. $E$ must be a medium or long vector
of the type given by \kbd{ellinit}.
For this function to work for every $n$ and not
just those prime to the conductor, $E$ must be a minimal Weierstrass
equation. If this is not the case, use the function \kbd{ellglobalred} first
before using \kbd{ellak}.
...
coefficients in $\Q$ but need {\it not\/} be minimal.
...
equal to $\exp(2i\pi z)$. It is assumed that the curve is a {\it strong\/}
...
If $\fl = 1$, use Lutz--Nagell ({\it much} slower), $E$ is allowed to be a
medium vector.
...
If $z$ is omitted or is a simple variable, computes the {\it power
series\/} expansion in $z$ (starting $z^{-2}+O(z^2)$). The number of terms
to an {\it even\/} power in the expansion is the default serieslength in
GP, and the second argument (C long integer) in library mode.
...
$\bullet$ $\var{\idx{nf}}$ denotes a number field, i.e.~a 9-component vector
...
$\bullet$ $\var{\idx{bnf}}$ denotes a big number field, i.e.~a 10-component
...
$\bullet$ $\var{\idx{bnr}}$ denotes a big ``ray number field'', i.e.~some data
...
$\bullet$ $\var{\idx{rnf}}$ denotes a relative number field (see below).
\smallskip

$\bullet$ ${\it \idx{ideal}}$ can mean any of the following:

\quad -- a $\Z$-basis, in \idx{Hermite normal form}
(HNF) or not. In this case $x$ is a square matrix.

\quad -- an {\it \idx{idele}}, i.e.~a 2-component vector, the first
being an ideal given as a $\Z$--basis, the second being a $r_1+r_2$-component
row vector giving the complex logarithmic Archimedean information.

\quad -- a $\Z_K$-generating system for an ideal.

\quad -- a {\it column} vector $x$ expressing an element of the number field
...
$\bullet$ a {\it\idx{character}} on the Abelian group
...
essential that all such vectors be {\it column\/} vectors.
...
$\Z$-generating set. If this is not the case, you {\it must\/} absolutely
...
$\bullet$ A {\it\idx{relative matrix}\/} will be a matrix whose entries are
...
$\bullet$ An {\it\idx{ideal list}\/} will be a row vector of (fractional)
ideals of the number field $\var{nf}$.

$\bullet$ A {\it\idx{pseudo-matrix}\/} will be a pair $(A,I)$ where $A$ is a
...
$\bullet$ The {\it\idx{module}\/} generated by a pseudo-matrix $(A,I)$ is
the sum $\sum_i{\Bbb a}_jA_j$ where the ${\Bbb a}_j$ are the ideals of $I$
and $A_j$ is the $j$-th column of $A$.

$\bullet$ A pseudo-matrix $(A,I)$ is a {\it\idx{pseudo-basis}} of the module it
generates if $A$ is a square matrix with non-zero determinant and all the
ideals of $I$ are non-zero. We say that it is in Hermite Normal
Form\sidx{Hermite normal form} (HNF) if
it is upper triangular and all the elements of the diagonal are equal to 1.

$\bullet$ The {\it determinant\/} of a pseudo-basis $(A,I)$ is the ideal
...
the program requires a big number field, the program will {\it not\/} launch
...
{\it transform your polynomial\/} so that it becomes monic. Instead of
the normal
result, say \kbd{res}, you then get a vector \kbd{[res,Mod(a,Q)]}, where
\kbd{Mod(a,Q)=Mod(X,P)} gives the change of variables.
...
them, they {\it must\/} be given in the requested order. For example, if you
...
indefinitely. In {\it no\/} occasion can the program give a wrong answer
...
The result of this function is a vector $v$ with 10 components (it is {\it
not\/} a $\var{bnf}$, you need \kbd{bnfinit} for that), which for ease of
...
\subsecidx{bnf{}init}$(P,\{\fl=0\},\{\var{tech}=[\,]\})$: essentially identical
...
necessary
to reduce the relation matrix to the form explained in subsection 6.5.5
of GTM~138 (i.e.~with a
big $c\times c$ identity matrix on the lower right). Note that in the above
mentioned book, the need to permute the rows of the relation matrices which
occur was not emphasized.
...
desired. Note also that the member functions will {\it not\/} work on
\var{sbnf}, you have to use \kbd{bnfmake} explicitly first.

\syn{bnf{}init0}{P,\fl,\var{tech},\var{prec}}.

\subsecidx{bnf{}isintnorm}$(\var{bnf},x)$: computes a complete system of
...
\syn{bnf{}isintnorm}{\var{bnf},x}.

\subsecidx{bnf{}isnorm}$(\var{bnf},x,\{\fl=1\})$: tries to tell whether the
...
\syn{bnf{}isnorm}{\var{bnf},x,\fl,\var{prec}}, where $\fl$ and
$\var{prec}$ are \kbd{long}s.

\subsecidx{bnf{}issunit}$(\var{bnf},\var{sfu},x)$: $\var{bnf}$ being output by
...
\syn{bnf{}issunit}{\var{bnf},\var{sfu},x}.

\subsecidx{bnf{}isprincipal}$(\var{bnf},x,\{\fl=1\})$: $\var{bnf}$ being the
...
If $\fl=2$, does as if $\fl$ were $0$, but doubles the precision until a result is
obtained.

If $\fl=3$, as in the default behaviour ($\fl=1$), but doubles the precision
until a result is obtained.

The user is warned that these two last setting may induce {\it very\/} lengthy
computations.

\syn{isprincipalall}{\var{bnf},x,\fl}.

\subsecidx{bnf{}isunit}$(\var{bnf},x)$: $\var{bnf}$ being the number field data
output by
...
as output
by \kbd{bnfinit}$(x,3)$, computes the complete \kbd{bnfinit} information. The
result is {\it not\/} identical to what \kbd{bnfinit} would yield, but is
functionally identical. The execution time is very small compared to a
complete \kbd{bnfinit}. Note that if the default precision in GP (or
$\var{prec}$ in library mode) is greater than the precision of the roots
$\var{sbnf}[5]$, these are recomputed so as to get a result with greater
accuracy.

Note that the member functions are {\it not\/} available for \var{sbnf}, you
...
\subsecidx{bnrL1}$(\var{bnr},\{\fl=0\})$:
\var{bnr} being the number field data which is output by
\kbd{bnrinit(,,1)}, returns for each \idx{character} $\chi$ of the
corresponding ray class group, the value at $s = 1$ (or $s = 0$) of
the abelian $L$-functions associated to $\chi$. For the value at $s =
0$, the function returns in fact for each character $\chi$ a vector
$[r_\chi , c_\chi]$ where $r_\chi$ is the order of $L(s, \chi)$ at $s
= 0$ and $c_\chi$ the first non-zero term in the expansion of $L(s,
...
group, so \kbd{bnrL1(bnr2)} returns exactly the same answer as
\kbd{bnrL1(bnr)}!).

\syn{bnrL1}{\var{bnr},\fl,\var{prec}}
...
modulus norm up to bound {\it bound}, where the ramified Archimedean places are
...
isomorphic to $\Q$. \fl\ is optional and may be set to 0 to obtain a
...
$t$ must be of {\it lower\/} priority than that of $x$ (in other words the
variable number of $t$ must be {\it greater\/} than that of $x$). However if
the coefficients of the number field occur explicitly (as polmods) as
coefficients of $x$, the variable of these polmods {\it must\/} be the same as
the main variable of $t$. For example
\kbd{factornf(x\pow 2 + Mod(y,y\pow 2+1), y\pow 2+1)} and
\kbd{factornf(x\pow 2+1, y\pow 2+1)} are legal but
\kbd{factornf(x\pow 2 + Mod(z,z\pow 2+1), y\pow 2+1)} is not.
...
\subsecidx{idealinv}$(\var{nf},x,\{\fl=0\})$: inverse of the ideal $x$ in the
...
If $\fl=1$, uses the different. This is usually slower.

\syn{idealinv0}{\var{nf},x,\fl}. Also available is
$\teb{idealinv}(\var{nf},x)$ ($\fl=0$).
...
\subsecidx{idealmin}$(\var{nf},x,\var{vdir})$: computes a minimum of the
ideal $x$ in the direction \var{vdir} in the number field \var{nf}.

\syn{minideal}{\var{nf},x,\var{vdir},\var{prec}}.
...
\subsecidx{idealred}$(\var{nf},x,\{\var{vdir}=0\})$: \idx{LLL} reduction of
the ideal $x$ in the number field \var{nf}, along the direction \var{vdir}.
Here \var{vdir} must be either an $r1+r2$-component vector ($r1$ and $r2$
number of real and complex places of \var{nf} as usual), or the PARI zero,
in which case \var{vdir} is assumed to be equal to the vector having only
components equal to 1. The notion of reduction along a direction is
technical and cannot be explained here. Note that this is {\it not\/} the
same as the LLL reduction of the lattice $x$ since ideal operations are
involved. The result is the \idx{Hermite normal form} of the LLL-reduced
ideal, which is usually, but not always, a reduced ideal. $x$ may also be a
2-component vector, the first being as above, and the second containing a
matrix of Archimedean information. In that case, this matrix is suitably
updated.

\syn{ideallllred}{\var{nf},x,\var{vdir},\var{prec}}.
...
\synx{ideal\_two\_elt0}{\var{nf},x,a}{ideal\string\_two\string\_elt0}, where
an omitted $a$ is entered as \kbd{NULL}.
...
field defined by the irreducible, preferably monic, polynomial $x$, using the
\idx{round 4} algorithm by default. (This program is the translation into C by
Pascal Letard of a program written by David \idx{Ford} in Maple.) The binary
...
that it does {\it not\/} have to be a complete factorization. This is
...
the number field ({\it not\/} of the polynomial $x$), and an omitted $p$ should
...
\synx{element\_div}{\var{nf},x,y}{element\string\_div}.
...
\synx{element\_divmodpr}{\var{nf},x,y,\var{pr}}{element\string\_divmodpr}.
...
\synx{element\_mul}{\var{nf},x,y}{element\string\_mul}.
...
\synx{element\_mulmodpr}{\var{nf},x,y,\var{pr}}{element\string\_mulmodpr}.
...
\synx{element\_pow}{\var{nf},x,k}{element\string\_pow}.
...
\synx{element\_powmodpr}{\var{nf},x,k,\var{pr}}{element\string\_powmodpr}.
...
\synx{element\_reduce}{\var{nf},x,\var{ideal}}{element\string\_reduce}.
...
\synx{element\_val}{\var{nf},x,\var{pr}}{element\string\_val},
and the result is a \kbd{long}.

\subsecidx{nf{}factor}$(\var{nf},x)$: factorization of the univariate
...
column vector). The main variable of $\var{nf}$ must be of {\it lower\/}
priority than that of $x$ (in other words, the variable number of $\var{nf}$
must be {\it greater\/} than that of $x$). However if the polynomial defining
the number field occurs explicitly  in the coefficients of $x$ (as modulus of
a \typ{POLMOD}), its main variable must be {\it the same\/} as the main
variable of $x$. For example, if $\var{nf}=\hbox{\kbd{nfinit(y\pow 2+1)}}$
then
\hbox{\kbd{nffactor(\var{nf},x\pow 2+Mod(y,y\pow 2+1))}} and
\hbox{\kbd{nffactor(\var{nf},x\pow 2+1)}} are both legal but
\hbox{\kbd{nffactor(\var{nf},x\pow 2+Mod(z,z\pow 2+1))}} is not.

\syn{nf{}factor}{\var{nf},x}.

\subsecidx{nf{}factormod}$(\var{nf},x,\var{pr})$: factorization of the
...
\var{pr} {\it must\/} be in the format output by \kbd{idealprimedec}. The
...
polmods) as coefficients of $x$, the variable of these polmods {\it must\/}
be the same as the main variable of $t$ (see \kbd{nffactor}).

\syn{nf{}factormod}{\var{nf},x,\var{pr}}.
...
\subsecidx{nfgaloisconj}$(\var{nf},\{\fl=0\},\{d\})$: $\var{nf}$ being a number
field as output by \kbd{nfinit}, computes the conjugates of a
root $r$ of the non-constant polynomial $x=\var{nf\/}[1]$ expressed as
polynomials in $r$. This can be used even if the number field $\var{nf}$ is not
\idx{Galois} since some conjugates may lie in the field. As a note to old-timers
of PARI, starting with version 2.0.17 this function works much better than in
earlier versions.


$\var{nf}$ can simply be a polynomial if $\fl\neq 1$.

If no flags or $\fl=0$, if $\var{nf}$ is a number field use a combination of
flag $4$ and $1$ and the result is always complete, else use a combination of
flag $4$ and $2$ and the result is subject to the restriction of $\fl=2$.


If $\fl=1$, use \kbd{nfroots} (require a number field). 

If $\fl=2$, use complex approximations to the roots and an integral
\idx{LLL}. The result is not guaranteed to be complete: some conjugates may be
missing (especially so if the corresponding polynomial has a huge
denominator). In that case, increasing the default precision may help.

If $\fl=4$, use Allombert's algorithm and permutation testing. If the field is
Galois with ``weakly'' super solvable Galois group, return the complete list of
automorphisms, else only the identity element. If present, $d$ is assumed to be
a multiple of the index of the power basis in the maximal order.

A group G is ``weakly'' super solvable if it contains a super solvable normal
subgroup $H$ such that $G=H$ , or $G/H \cong A_4$ , or $G/H \cong S_4$ . Abelian and
nilpotent groups are ``weakly'' super solvable.  In practice, almost all groups
of small order are weakly super solvable, the exceptions having order 36(1
exception), 48(2), 56(1), 60(1), 72(5), 75(1), 80(1), 96(10) and $\geq 108$.

Hence $\fl = 4$ permits to quickly check whether a polynomial of order strictly
less than $36$ is Galois or not. This method is much faster than \kbd{nfroots}
and be applied to polynomial of degree more than $50$. 

\syn{galoisconj0}{\var{nf},\fl,d,\var{prec}}. Also available are $\teb{galoisconj}(\var{nf})$ for $\fl=0$, $\teb{galoisconj2}(\var{nf},n,\var{prec})$ for $\fl=2$ where $n$ is a bound on the number of conjugates, and  $\teb{galoisconj4}(\var{nf},d)$ corresponding to $\fl=4$.
...
\subsecidx{nf{}init}$(\var{pol},\{\fl=0\})$: \var{pol} being a non-constant,
preferably monic, irreducible polynomial in $\Z[X]$, computes a 9-component
vector \var{nf} useful in working in the number field $K$ defined by
\var{pol}.
...
and complex places of $K$.
 ...
 $\var{nf\/}[3]$ contains the discriminant $d(K)$ (\kbd{\var{nf}.disc}) of
the number field $K$.
...
\quad$\bullet$ Finally, $MDI$ has the form $[x,y,n]$, where $(x,y)$ expresses
a $\Z_K$-basis of $d(K)$ times the codifferent ideal
(\kbd{\var{nf}.disc$*$\var{nf}.codiff}, which is an integral ideal) and $n$
is its norm (this ideal is used in \tet{idealinv}).
...
If $\fl=1$: does not compute the different, replace it by a dummy $0$.
...
\syn{nf{}init0}{x,\fl,\var{prec}}.

\subsecidx{nf{}isideal}$(\var{nf},x)$: returns 1 if $x$ is an ideal in
the number field $\var{nf}$, 0 otherwise.

\syn{isideal}{x}.

\subsecidx{nf{}isincl}$(x,y)$: tests whether the number field $K$ defined
...
\syn{nf{}isincl}{x,y,\fl}.

\subsecidx{nf{}isisom}$(x,y)$: as \tet{nfisincl}, but tests
for isomorphism. If either $x$ or $y$ is a number field, a much faster
algorithm will be used.

\syn{nf{}isisom}{x,y,\fl}.
...
polmods {\it must\/} be the same as the main variable of $t$ (see
\kbd{nffactor}).
...
\subsecidx{polcompositum}$(x,y,\{\fl=0\})$: $x$ and $y$ being polynomials in
$\Z[x]$ in the same variable, outputs a vector giving the list of all
...
only squarefree. Returns an error if one of the polynomials is not squarefree.
...
$b$) expresses the root of $x$ (resp. $y$) as a polmod in a root of $z$,
and $k$ is a small integer k such that $a+kb$ is the chosen root of $z$.
...
4: gives {\it all} polynomials of minimal $T_2$ norm (of the two polynomials 
...
$\var{nf}$ {\it must\/} be of lower priority than that of \var{pol}.
...
$\alpha$ is a root of the polynomial defining the base field $\var{nf}$.
  ...
  The main variable of $\var{nf}$ {\it must\/} be of lower priority than that
...
\subsecidx{rnf{}idealabstorel}$(\var{rnf},x)$: $\var{rnf}$ being a relative
number field extension $L/K$ as output by \kbd{rnfinit} and $x$ being an
...
\syn{rnf{}idealabstorel}{\var{rnf},x}.

\subsecidx{rnf{}idealdown}$(\var{rnf},x)$: $\var{rnf}$ being a relative number
...
\syn{rnf{}idealdown}{\var{rnf},x}.

\subsecidx{rnf{}idealhnf}$(\var{rnf},x)$: $\var{rnf}$ being a relative number
...
\syn{rnf{}idealhermite}{\var{rnf},x}.
...
\syn{rnf{}idealmul}{\var{rnf},x,y}.

\subsecidx{rnf{}idealnormabs}$(\var{rnf},x)$: $\var{rnf}$ being a relative
...
\syn{rnf{}idealnormabs}{\var{rnf},x}.

\subsecidx{rnf{}idealnormrel}$(\var{rnf},x)$: $\var{rnf}$ being a relative
number field
...
\syn{rnf{}idealnormrel}{\var{rnf},x}.

\subsecidx{rnf{}idealreltoabs}$(\var{rnf},x)$: $\var{rnf}$ being a relative
number field
extension $L/K$ as output by \kbd{rnfinit} and $x$ being a relative ideal
(which can be, as in the absolute case, of many different types, including
...
\syn{rnf{}idealreltoabs}{\var{rnf},x}.

\subsecidx{rnf{}idealtwoelt}$(\var{rnf},x)$: $\var{rnf}$ being a relative
number field
...
\syn{rnf{}idealtwoelement}{\var{rnf},x}.

\subsecidx{rnf{}idealup}$(\var{rnf},x)$: $\var{rnf}$ being a relative number
field
...
\syn{rnf{}idealup}{\var{rnf},x}.

\subsecidx{rnf{}init}$(\var{nf},\var{pol})$: $\var{nf}$ being a number field in
\kbd{nfinit}
...
are technical), the numbering being very close to that of \kbd{nfinit}.
In the following description, we let $K$ be the base field defined by
$\var{nf}$,
$m$ the degree of the base field, $n$ the relative degree, $L$ the large
field (of relative degree $n$ or absolute degree $nm$), $r_1$ and $r_2$ the
number of real and complex places of $K$.
...
$\var{rnf\/}[11]$ is a vector \var{vabs} with 5 entries describing the {\it
absolute\/} extension $L/\Q$. $\var{vabs}[1]$ is an absolute equation.
...
\syn{rnf{}initalg}{\var{nf},\var{pol},\var{prec}}.

\subsecidx{rnf{}isfree}$(\var{bnf},x)$: given a big number field $\var{bnf}$ as
output by \kbd{bnfinit}, and either a polynomial $x$ with coefficients in
$\var{bnf}$ defining a relative extension $L$ of $\var{bnf}$, or a
pseudo-basis $x$ of such an extension, returns true (1) if $L/\var{bnf}$ is
free, false (0) if not.

\syn{rnf{}isfree}{\var{bnf},x}, and the result is a \kbd{long}.

\subsecidx{rnf{}isnorm}$(\var{bnf},\var{ext},\var{el},\{\fl=1\})$: similar to
...
\syn{rnf{}isnorm}{\var{bnf},ext,x,\fl,\var{prec}}.
...
\subsecidx{rnf{}lllgram}$(\var{nf},\var{pol},\var{order})$: given a polynomial
...
\syn{rnf{}lllgram}{\var{nf},\var{pol},\var{order},\var{prec}}.
...
than \kbd{rnfpolred}, it is much more efficient, the difference being more
dramatic than in the absolute case. This is because the implementation of
\kbd{rnfpolred} is based on an incomplete reduction theory of lattices over
number fields (i.e.~the function \kbd{rnflllgram}) which deserves to be
improved.
...
called the {\it Steinitz class\/} of the module $\Z_L$.
...
field initialized by \kbd{zetakinit} ({\it not\/} by \kbd{nfinit}),
...
able to compute the \idx{Dedekind} zeta and lambda functions (respectively
...
through the \kbd{zetak} function. The only component which can be used if it
has not been computed already is $v[1][4]$ which is the result of the
\kbd{bnfinit} call.
...
digits of relative precision (e.g~\kbd{zetakinit(x\pow 8 - 2)} needs 440MB of
memory at default precision).
...
of the polynomial \var{pol} to precision $r$, the result being a two-column
matrix as in \kbd{factor}. $r$ must be strictly larger than the $p$-adic
valuation of the discriminant of \var{pol} for the result to make any sense.
The method used is \idx{Ford}-Letard's implementation of the \idx{round 4}
...
If $\fl=1$, use a variant of the Newton-Raphson method, which is {\it not}
...
accepted, mainly for $2$-adic computations. Multiple roots are {\it not\/}
repeated.

If $p<100$, you may try setting $\fl=1$, which uses a naive search. In this
case, multiple roots {\it are\/} repeated with their order of multiplicity.
...
{\it not\/} repeated. $p$ is assumed to be a prime.
...
\kbd{\teb{sturm}(\var{pol})} is equivalent to
\kbd{\key{sturmpart}(\var{pol},NULL,NULL)}.
The result is a \kbd{long}.
...
(i.e.~$n=2$, $4$, $p^k$ or $2p^k$ for an odd prime $p$).
...
\smallskip%
? subst(1, x, Mat([0,1]))
\q  ***   forbidden substitution by a non square matrix%
\eprog
...
\subsecidx{thueinit}$(P,\{\fl=0\})$: initializes the \var{tnf} corresponding to
$P$. It is meant to be used in conjunction with \tet{thue} to solve Thue
equations $P(x,y) = a$, where $a$ is an integer. If $\fl$ is non-zero,
certify the result unconditionnaly, Otherwise, assume \idx{GRH}, this being
...
{\it columns\/} of the given matrix.
...
error if $A$ is of any other type. The value of $\fl$ is only significant
for matrices.
...
If $\fl=2$, uses the Hessenberg form. This is faster than the default when the
...
definitely {\it not\/} the case for empty vectors \kbd{[~]} or \kbd{[~]\til}.)
...
? concat([1,2], [3,4])
\%1 = [1, 2, 3, 4]
? concat([1,2]\til, [3,4]\til)
\%2 = [1, 2, 3, 4]\til
? concat([1,2; 3,4], [5,6]\til)
\%3 =
[1, 2, 5]
\smallskip%
[3, 4, 6]
\smallskip%
? concat([\%, [7,8]\til, [1,2,3,4]])
\%4 =
[1 2 5 7]
\smallskip%
[3 4 6 8]
...
matrix of maximal rank, finds the {\it upper triangular\/}
\idx{Hermite normal form}
of $x$. If the rank of $x$ is equal to its number of rows, the result is a
square matrix. In general, the columns of the result form a basis of the
lattice spanned by the columns of $x$.

If $\fl=0$, uses the naive algorithm. If the $\Z$-module generated by the
columns is a lattice, it is recommanded to use \kbd{mathnfmod(x,
matdetint(x))} instead (much faster).

If $\fl=1$, uses Batut's algorithm. Outputs a two-component row vector
$[H,U]$, where $H$ is the {\it upper triangular\/} Hermite normal form
...
$\teb{hnfall}(x)$ ($\fl=1$). To reduce {\it huge} (say $400 \times 400$ and
...
{\it upper triangular\/} \idx{Hermite normal form} of $x$.
...
$\kbd{ker\_mod\_p}(x, p)$, where $x$ has integer entries and $p$ is prime,
which is equivalent to but many orders of magnitude faster than
\kbd{matker(x*Mod(1,p))} and needs much less stack space. To use it under GP,
type \kbd{install(ker\_mod\_p, GG)} first.
...
$$ q(x) = \sum_i a_{ii} (x_i + \sum_j>i a_{ij} x_j)^2 $$
...
\subsecidx{qf{}lll}$(x,\{\fl=0\})$: \idx{LLL} algorithm applied to the
{\it columns}
of the (not necessarily square) matrix $x$. The columns of $x$ must however
be of maximal rank (unless specified otherwise below). The result is a square
transformation matrix $T$ such that $x\cdot T$ is an LLL-reduced basis of the
...
is said to be {\it partially reduced} if $|v_i \pm v_j| \geq |v_i|$ for any
...
\syn{qf{}lll0}{x,\fl,\var{prec}}. Also available are
$\teb{lll}(x,\var{prec})$ ($\fl=0$), $\teb{lllint}(x)$ ($\fl=1$), and
$\teb{lllkerim}(x)$ ($\fl=4$).

\subsecidx{qf{}lllgram}$(x,\{\fl=0\})$: same as \kbd{qflll} except that the
...
\syn{qf{}lllgram0}{x,\fl,\var{prec}}. Also available are
...
\syn{minim}{x,b,m} ($\fl=0$), $\teb{minim2}(x,b,m)$
($\fl=1$), or finally $\key{fincke\_pohst}(x,b,m,\var{prec})$
($\fl=2$).\sidx{fincke\string\_pohst}
...
This function works also if $x$ is a {\it sorted\/} list (see \kbd{listsort}).
...
components are as usual the {\it columns} of $x$. The parameter $y$ is a
...
If $z$ is not omitted, $x$ must be a matrix. $y$ is then the {\it line}
specifier, and $z$ the {\it column} specifier, where the component specifier
is as explained above.

\bprog%
? v = [a, b, c, d, e];
? vecextract(v, 5)         \bs\bs~mask
\%1 = [a, c]
? vecextract(v, [4, 2, 1]) \bs\bs~component list
\%2 = [d, b, a]
? vecextract(v, "2..4")    \bs\bs~interval
\%3 = [b, c, d]
? vecextract(v, "-1..-3")  \bs\bs~interval + reverse order
\%4 = [e, d, c]
? vecextract([1,2,3], "\pow2") \bs\bs~complement
\%5 = [1, 3]
...
contain the indices. Note that the resulting $v$ is {\it not\/} a valid PARI
...
{\it must\/} have $ab>0$, and in fact if for example $b=+\infty$, then it is
...
\bprog%
? prod(i=1, 100, 1-X\pow i);  \bs\bs\ this has degree $5050$~!!
\smallskip%
time = 3,335 ms.
\smallskip%
? prod(i=1, 100, 1-X\pow i, 1+O(X\pow 101))
\smallskip%
time = 43 ms.
\smallskip%
\%2 = 1 - X - X\pow 2 + X\pow 5 + X\pow 7 - X\pow 12 - X\pow 15 + X\pow 22 + X\pow 26 - X\pow 35 - X\pow 40 + X\pow 51
+ X\pow 57 - X\pow 70 - X\pow 77 + X\pow 92 + X\pow 100 + O(X\pow 101)%
\eprog

\synt{produit}{entree *ep, GEN a, GEN b, char *expr, GEN x}.

\subsecidx{prodeuler}$(X=a,b,\var{expr})$: product of expression \var{expr},
initialized at 1. (i.e.~to a {\it real\/} number equal to 1 to the current
...
$\var{expr}(X=a)*\var{expr}(X=b)\le0$. This
routine uses Brent's method. This can fail miserably if \var{expr} is not
defined in the whole of $[a,b]$ (try \kbd{solve(x=1, 2, tan(x)}).
...
\bprog%
? sum(i=1, 5000, 1/i); \bs\bs rational number: denominator has $2166$ digits.
\smallskip%
time = 1,241 ms.
\smallskip%
? sum(i=1, 5000, 1/i, 0.)
...
expression \kbd{expr}
\bprog%
/* compute a\pow 2 + \dots + b\pow 2 */
\obr
\q /* define the dummy variable "i" */
\q entree *ep = gp\_variable("i");
\q /* sum for a <= i <= b */
\q return somme(ep, a, b, "i\pow2", gzero);
\cbr
\eprog
...
(called {\it rectplot} functions, sharing the prefix \kbd{plot}) work as
...
the height and width of the virtual window (called a {\it rectwindow} in the
...
function \kbd{plotcursor}.
  ...
  A number of primitive graphic objects (called {\it rect} objects) can then
...
(both low and high level) have been written for the X11-window system (hence
also for GUI's based on X11 such as Openwindows and Motif), and for
Sunview/Suntools only, though very little code remains which is actually
platform-dependent. A Macintosh, and an Atari/Gem port were provided for
previous versions. These {\it may} be adapted in future releases.
...
\subsecidx{plot}$(X=a,b,\var{expr})$: crude (ASCII) plot of the function
represented by expression \var{expr} from $a$ to $b$.
...
colormaps would be nice, and {\it may\/} be available in future versions.
...
$\bullet$ 1: {\it \idx{parametric plot}}. Here \var{expr} must be a vector with
...
$\bullet$ 2: {\it \idx{recursive plot}}. If this flag is set, only {\it
one\/} curve can be drawn at time, i.e.~\var{expr} must be either a
two-component vector (for a single parametric curve, and the parametric flag
{\it has\/} to be set), or a scalar function. The idea is to choose pairs of
...
to the original curve. For instance you should {\it never\/} plot recursively 
...
same scaling factor on $x$ and $y$. If $\fl$ is non-zero, join points.
...
actually lies within the boundary of $w$. Then {\it move} the virtual cursor
...
\subsecidx{plotlinetype}$(w,\var{type})$: this is intended for the
\kbd{gnuplot} interface and is currently not significant.
...
$Y$ and which are inside $w$. The virtual cursor does {\it not\/} move. This
...
{\it move\/} the virtual cursor to position $(x,y)$.
...
\subsecidx{plotpointtype}$(w,\var{type})$: this is intended for the
\kbd{gnuplot} interface and is currently not significant.
...
\subsecidx{plotstring}$(w,x)$: draw on the rectwindow $w$ the String $x$ (see
Section 2.4), at the current position of the cursor.

\subsecidx{plotterm}$(\var{type})$: this is intended for the \kbd{gnuplot}
...
if they are prime), the \var{seq} is evaluated. Nothing is done if $a>b$.
...
each subgroup $H$ of the {\it abelian} group $G$ (given in 
...
\bprog%
if (reallydoit \&\& longcomplicatedfunction(), $\dots$)%
\eprog
\noindent is a perfectly safe statement.

  Recall that functions such as \kbd{break} and \kbd{next} operate on
{\it loops\/} (such as \kbd{for$xxx$}, \kbd{while}, \kbd{until}). The \kbd{if}
statement is {\it not\/} a loop (obviously!).
...
on $a$ is tested {\it after\/} execution of the \var{seq}, not before as in
\kbd{while}).

\subsubsecidx{while}$(a,\var{seq})$: while $a$ is non-zero evaluate the
expression sequence \var{seq}. The test is made {\it before\/} evaluating
...
the difference between {\it strings\/} and {\it keywords\/} (see
...
and stored as the online help for $S$. If $S$ is a function {\it you\/} have
...
to an existing {\it function\/} name.
This is different from the general user macros in that alias expansion takes
place immediately upon execution, without having to look up any function
code, and is thus much faster. A sample alias file \kbd{misc/gpalias} is
provided with the standard distribution. Alias commands are meant to be read
upon startup from the \kbd{.gprc} file, to cope with function names you are
dissatisfied with, and should be useless in interactive usage.

\subsubsecidx{allocatemem}$(\{x=0\})$: this is a very special operation which
allows the user to change the stack size {\it after\/} initialization. $x$
...
Although it is a function, this must be the {\it last\/} instruction in any GP
...
defaults, and \secref{se:meta} for some shortcut alternatives.
\label{se:default}

If \var{val} is omitted, prints the current value of default \var{key}.
If \var{key} is omitted, prints the current values of all the defaults.
If $\fl$ is set, returns the result instead of printing it.

\subsubsecidx{error}$(\{\var{str}\}*)$: outputs its argument list (each of them
interpreted as a string), then interrupts the running GP program, returning to
the input prompt.
...
%
\bprog%
p = 3 \bs\bs~fix characteristic
...
forprime(p = 2, N, ...)
f(p) = ...
\eprog
since within the loop or within the function's body, the true global value of
\kbd{p} will be hidden. If the statement \kbd{global(p = 3)} appears at the
beginning of the script, then both expressions will trigger syntax errors.
...
\secref{se:emacs}) one {\it must\/} prompt for the string, with a string
...
blunder (this should never happen with a dynamically linked executable). This
{\it used\/} to be the fate of statically linked gp on \kbd{Linux} and
\kbd{OSF1} up to and including version 2.0.3.

Hence, if you intend to use this function, please check first on some
harmless example such as the one above that it works properly on your
machine.
...
degree one), use the \idx{quote} operator: \kbd{x = 'x}.
