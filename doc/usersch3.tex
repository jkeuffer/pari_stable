...
same versatile factoring machinery described under \kbd{factorint}. It
includes \idx{Pollard Rho}, \idx{ECM} and \idx{MPQS} stages, and has an
early exit option for the functions \teb{moebius} and (the integer function
...
M\"obius\sidx{moebius} function, and is computed as a multiplicative
...
\subsecidx{moebius}$(x)$: M\"obius $\mu$-function of $|x|$. $x$ must be of
type integer.
...
\quad$\bullet$ Finally, $MDI$ has the form $[x,y,n]$, where $(x,y)$
expresses a $\Z_K$-basis of $d(K)$ times the codifferent ideal
(\kbd{\var{nf}.disc$*$\var{nf}.codiff}, which is an integral ideal) and $n$
is its norm (this ideal is only used in \tet{idealinv}).
