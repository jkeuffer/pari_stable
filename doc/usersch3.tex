...
field theory. These are defined by a triple
...
unless the modulus is trivial; \var{bnr} by \kbd{bnrinit}. This is the ground
field $K$.
...
extracted also from a \var{bid}).\cr
...
$c_2 = 12$, then everything is correct assuming the GRH holds. You may use
...
output by \kbd{bnfinit}, and \var{list} being a list of modules with units as
output by \kbd{ideallist} or \kbd{ideallistarch}, outputs the list of the
class numbers of the corresponding ray class groups.
...
output by \kbd{bnfinit}, \var{ideal} is a modulus, initializes data linked to
...
divisors, its generators. Note that the latter group are different from the
members of the underlying \var{bnf}, which refer to the class group; use
\kbd{\var{bnr}.bnf.clgp} to access these.
...
\var{list} is a vector of bids, as output by \tet{ideallist} with flag
$0$ to $3$. Return a vector of vectors with the same number of components as
the original \var{list}. The leaves give information about moduli whose
finite part is as in original list, in the same order, and archimedean part
is now \var{arch} (it was originally trivial). The information contained is
of the same kind as was present in the input; see \tet{ideallist}, in
particular the meaning of \fl.
