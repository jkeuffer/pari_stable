...
obtain the \kbd{gcc/g++} compiler. As for all GNU software mentioned
afterwards, you can find the most convenient site to fetch \kbd{gcc} at the
address

\kbd{http://www.gnu.org/order/ftp.html}

\noindent You can certainly compile PARI with a different compiler, but the
PARI kernel takes advantage of some optimizations provided by \kbd{gcc} if it
...
\misctitle{Technical note 2:} \kbd{Configure} lets the following environment
...
the \kbd{gp-dyn} binary (e.g Illegal Instruction on startup). It does not
affect \kbd{gp-sta}.
...
\kbd{make test-kernel} may be applied to an \emph{optimized} gp build (not a
...
you are an avid GP fan, do not forget to delete the old pari libraries once in
a while).
...
\noindent The PARI home page at the address

\centerline{\wwwsite}

\noindent maintains an archive of all discussions, various documentation, as
well as a download area and our Bug Tracking System. If you do not want to
subscribe to those lists, you can write to us at the address
