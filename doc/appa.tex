...
commands.
  ...
  $\bullet$ GNU \kbd{emacs} and the \tet{PariEmacs} package. GP can be run in
an Emacs buffer, with all the obvious advantages if you are familiar with
this editor. Note that \kbd{readline} is still useful in this case since it
...
manual) about functions and concepts. The script handline this online help
...
the same source tree (the builds are independent and can be done
simultaneously).
...
as a further test). More generally, typing \kbd{make} without argument
