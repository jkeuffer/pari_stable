...
PARI kernel takes advantage of optimizations provided by \kbd{gcc}. This
...
in conjunction with \kbd{gp}, but not mandatory. They are probably already
installed somewhere on your system, with the possible exception of
\kbd{readline}, which you should really install first. In any case, get them
before proceeding if you want the functionalities they provide. All of them
are free (you can make a donation to the Free Software Fundation, to help
them write more free software).
...
use \kbd{Configure --with-gmp}. Unless you have backwards compatibility
requirements, you should do this.
...
Note that autodetection of GMP is \emph{disabled} by default, you
...
\kbd{\#\#\#}, which report genuine problems. If anything should have
been found and was not, consider that \kbd{Configure} failed and follow the
instructions below. Look especially for the \kbd{gmp}, \kbd{readline} and
\kbd{X11} libraries, and the \kbd{perl} and \kbd{gunzip} (or \kbd{zcat})
binaries.
