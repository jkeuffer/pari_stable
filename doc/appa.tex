...
\noindent and compare the benches. Also, if insist on using a \kbd{C++}
...
\misctitle{Note2:} there's a known minor problem on Pentium processors
(unrelated to the infamous \kbd{fdiv} bug) which results in a discrepancy in 
bench \kbd{elliptic}: the last few decimals of the components of an \kbd{ell}
structure may be slightly off (with respect to the bench: both results are
correct given the requested precision).
