...
\misctitle{Linux:} several Linux distributions, e.g.~Mandrake and RedHat, have
separate \kbd{readline} and \kbd{readline-devel} packages. You need both of
them installed to compile gp with readline support. If only \kbd{readline}
is installed, \kbd{Configure} will complain. \kbd{Configure} may also
complain about a missing libncurses.so, in which case, you will have to
install the \kbd{ncurses-devel} package.
