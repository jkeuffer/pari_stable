...
see below). If your \kbd{make} program supports parallel make, you can speed
up the process by going to the \kbd{O$xxx$} directory that \kbd{Configure}
created and doing a parallel make here (for instance \kbd{make -j4} with GNU
...
\subsec{Printable Documentation:} Building gp as explained above also builds
its documentation in directory \kbd{doc}. For this to work, you need a
working (plain) \TeX\ installation.

This directory contains various \kbd{dvi} files: \kbd{users.dvi} (manual with
a table of contents and an index), \kbd{tutorial.dvi} (a short tutorial), and
