...
report wall-clock instead of the sum of the time spent by each threads.
...
We provide an \emph{Introduction to parallel GP programming} in the
file \kbd{doc/parallel.dvi}.
...
\kbd{--mt=pthread}. This tends to slow down the shared library libpari
by $15\%$, so you might want to use the static library instead.
See Appendix~D if you want to write multi-threaded libpari program.
Beware that this option change the library ABI.
...
By default, if a dynamic library \kbd{libpari.so} can be built, the static
library \kbd{libpari.a} is not created. If you want it as well, use the target
\kbd{make install-lib-sta}. You can install a statically
