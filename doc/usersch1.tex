...
badly compared to more sophisticated systems like Axiom, Macsyma, Maple,
Mathematica or Reduce on such tasks (e.g.~multivariate polynomials, formal
integration, etc\dots). On the other hand, the three main advantages of the
system are its speed, the possibility of using directly data types which are
familiar to mathematicians, and its extensive algebraic number theory module
which has no equivalent in the above-mentioned systems.
...
at the \kbd{gp} reference card (\kbd{refcard.dvi} or \kbd{refcard.ps}). In
...
\secref{se:start} if you are not interested in hardware technicalities.)
...
are supported, but the GNU \kbd{gcc} compiler is the definite favourite.
...
and should be portable without trouble to any 32 or 64-bit computer
...
The crucial word is \idx{recursiveness}: most of the types PARI knows
...
numbers with at most a few thousand decimal digits. In particular, no effort
has been put into asymptotically fast DFT-based techniques (only divide and
conquer methods are implemented, a la Karatsuba). Hence, although it is
possible to use PARI to do computations with $10^7$ decimal digits,
better programs can be written for such huge numbers.
...
Even if it is working in calculator mode \kbd{gp} where there is a notion of
\idx{default precision}, which is only used to convert exact types to inexact
ones.
