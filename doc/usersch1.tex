...
\quad 3) the compiler GP2C can translate GP code to C, and load them into the
\kbd{gp} interpreter. A typical script compiled by GP2C will typically run 3
to 10 times faster. The C code can also be edited and optimized by hand.

The use of \kbd{gp} is explained in chapters 2 and 3, and the programming in
library mode is explained in chapters 3, 4 and 5. In the present Chapter 1,
we give an overview of the system.

\subsectitle{Important note:} A tutorial for \kbd{gp} is provided in the standard
...
at the \kbd{gp} reference card (\kbd{refcard.dvi} or \kbd{refcard.ps}). In case
...
From that point you may access all sources, some binaries, version
informations, the complete mailing list archives, frequently asked questions
and various tips. All threaded and fully searchable.
...
processors, and finally the whole ix86 family (Intel, AMD, Cyrix) starting at
the 386 (up to the Xbox gaming console!). Finally, there are unmaintained,
much less optimized and not really usable anymore, micro-kernels for the
HP-PA architecture (only for HPUX 9, does not work with HPUX 10), and for the
PowerPC architecture (only for the 601, does not compile anymore).
...
including an assembly kernel. This version has been tested for example on
MIPS based DECstations 3100 and 5000 and SGI computers, on various Macs
(based on 680x0 chips or more recent PowerPCs, like the G3 or G4), new Intel
...
\section{The PARI types} \label{se:start}\sidx{types}

\noindent The crucial word in PARI is \idx{recursiveness}: most of the types
it knows about are recursive. For example, the basic type {\bf Complex}
exists (actually called \typ{COMPLEX}). However, the components (i.e.~the
real and imaginary part) of such a ``complex number'' can be of any type. The
only sensible ones are integers (we are then in $\Z[i]$), rational numbers
($\Q[i]$), real numbers ($\R[i]=\C$), or even elements of $\Z/n\Z$
($(\Z/n\Z)[i]$ when this makes sense), or $p$-adic numbers when $p\equiv 3
\mod 4$ ($\Q_{p}[i]$).
...
number}\cr
%
\+&type \tet{t_INTMOD}& $\Z/n\Z$& Integermods (integers modulo
$n$)\sidx{integermod}\cr
%
\+&type \tet{t_FRAC}& $\Q$& Rational numbers (in irreducible
form)\sidx{rational number}\cr
%
\+&type \tet{t_FRACN}& $\Q$& Rational numbers (not necessarily in irreducible
form)\cr
%
...
%
\+&type \tet{t_RFRACN}& $T(X)$& Rational functions (not necessarily in
irreducible form)\cr
%
...
where the types $T$ in recursive types can be different in each component.
...
\subsec{Integermods, rational numbers (irreducible or not), $p$-adic numbers,
polmods, and rational functions}:\sidx{integermod}\sidx{rational
number}\sidx{p-adic number} \sidx{polmod} these are recursive, but in a
restricted way.

For integermods or polmods, there are two components: the modulus, which must
...
is considered as $\limproj \Z/p^k\Z$, and $\Q_p$ as its field of fractions.
Like real numbers, the codewords contain an exponent (giving essentially the
$p$-adic valuation of the number) and also the information on the precision
of the number (which is in fact redundant with $p^k$, but is included for the
sake of efficiency).
...
Complex and quadratic numbers are partially recursive: the two components $a$
and $b$ can be of type integer, real, rational, integermod or $p$-adic, and
can be mixed, subject to the limitations mentioned above. For example, $a+bi$
with $a$ and $b$ $p$-adic is in $\Q_p[i]$, but this is equal to
...
working in calculator mode \kbd{gp} where there is a \idx{default precision}.
...
exponential of each one. This could easily be done either under \kbd{gp} or in
