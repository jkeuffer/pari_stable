...
matexp(GEN x,long prec)
{
  ...
  x    = gmul(x, realun(prec+1)); /* @Ccom convert to floats */
  xlow = gmul(x, realun(3));      /* @Ccom low precision copy */
  s = gnorml2(xlow);

  /*@Ccom if s$ < 1$ we are happy */
  k = expo(s); /*@Ccom s is guaranteed \typ{REAL} */
  if (k < 0) { n = 0; p1 = x; }
  else     { n = k+1; p1 = gmul2n(x, -n); s = gmul2n(s, -n); }

  /*@Ccom initializations before the loop */
  y = gscalmat(realun(prec+1), lx-1); /*@Ccom scalar identity matrix */
  p2 = p1; r = s; k = 1;
  y = gadd(y, p2);
