...
char *helpmessages_basic[]={
  ...
  "galoisfixedfield(gal,perm,{p}): gal being a galois field as output by galoisinit and perm an element of gal[6] or a vector of such elements, return [P,x] such that P is a polynomial defining the fixed field of gal[1] by the subgroup generated by perm, and x is a root of P in gal expressed as a polmod in gal[1]. If p is present, it must be a prime number such that gal[1] is squarefree modulo p. In this case P is also garanteed to be squarefree modulo p",
