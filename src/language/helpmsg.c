...
char *helpmessages_basic[]={
  ...
  "nfgaloisconj(nf,{flag=0},{den}): list of conjugates of a root of the polynomial x=nf.pol in the same number field. flag is optional (set to 0 by default), meaning 0: use p-adics, LLL on integral basis (some conjugates may be missing); 2 & 3: use complex numbers, LLL on integral basis (not always complete); 4: use Allombert's algorithm, Galois group assumed to be weakly super solvable (in particular, abelian is ok), x can be a polynomial; if den is present, assume the index of the power basis in the maximal order divides it. See manual",
