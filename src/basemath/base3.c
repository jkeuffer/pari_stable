...
element_pow(GEN nf, GEN x, GEN n)
{
  ...
  nf=checknf(nf); N=degpol(nf[1]);
  s=signe(n); if (!s) return gscalcol_i(gone,N);
  if (typ(x) != t_COL)
  {
    x = algtobasis(nf,x);
...
element_powid_mod_p(GEN nf, long I, GEN n, GEN p)
{
  ...
  if (!s || I == 1) return gscalcol_i(gone,N);
  D.nf = nf;
  D.p = p;
  D.I = I;
...
{
  long i, l = lg(v);
  for (i = 1; i < l; i++) v[i] = mpodd((GEN)v[i])? one: zero;
  return v;
}
...
zsigne(GEN nf,GEN x,GEN arch)
{
  ...
  switch(typ(x))
  {
    case t_MAT: /* factorisation */
    {
      ...
      for (i=1; i<l; i++) V[i] = mpodd((GEN)z[i])? one: zero;
      avma = av; return V;
    }
    case t_POLMOD: x = (GEN)x[2];      /* fall through */
    ...
    case t_INT: case t_FRAC:
      ...
      return vecconst(V, (s < 0)? gone: gzero);
  }
  x = Q_primpart(x); M = gmael(nf,5,1);
  for (i = 1; i < l; i++) V[i] = (eval_sign(M, x, archp[i]) > 0)? zero: one;
  avma = av; return V;
}
...
set_sign_mod_idele(GEN nf, GEN x, GEN y, GEN idele, GEN sarch)
{
  ...
  archp = arch_to_perm((GEN)idele[2]);
  s = zsigne(nf, y, archp);
...
element_powmodideal(GEN nf,GEN x,GEN k,GEN ideal)
{
  ...
  return y? y: gscalcol_i(gone,degpol(nf[1]));
}
...
{
  GEN y = element_powmodideal(nf,x,k,(GEN)idele[1]);
  if (mpodd(k))
...
famat_to_nf_modideal_coprime(GEN nf, GEN g, GEN e, GEN id, GEN EX)
{
  ...
  return plus? plus: gscalcol(gone, lg(id)-1);
}
...
Fp_shanks(GEN x,GEN g0,GEN p, GEN q)
{
  ...
  x = modii(x,p);
  if (is_pm1(x) || egalii(p,gtwo)) { avma = av; return gzero; }
...
dethnf(GEN mat)
{
  ...
  if (l<3) return l<2? gone: icopy(gcoeff(mat,1,1));
...
detcyc(GEN cyc)
{
  ...
  if (l<3) return l<2? gone: icopy((GEN)cyc[1]);
  av = avma; s = (GEN)cyc[1];
...
archstar_full_rk(GEN x, GEN bas, GEN v, GEN gen)
{
  ...
  for (     ; i <= nba; i++)  mat[i] = lgetg(nba+1, t_VECSMALL);

  if (x) { x = lllint_ip(x,4); bas = gmul(bas, x); }

  for (r=1;; r++)
  { /* reset */
    vecconst(lambda, (GEN)0);
    if (!x) lambda[1] = r;
    while (increment(lambda, N, r))
    {
      ...
      for (i = 1; i <= nba; i++)
      {
        GEN t = x? gadd((GEN)a[i], gone): (GEN)a[i];
        c[i] = (gsigne(t) < 0)? 1: 0;
      }
...
zarchstar(GEN nf, GEN x, GEN archp)
{
  ...
  p1 = cgetg(nba+1,t_VEC); for (i=1; i<=nba; i++) p1[i] = two;
  y[1] = (long)p1; av = avma;
  if (gcmp1(gcoeff(x,1,1))) x = NULL; /* x = O_K */
  gZ = x? subsi(1, gcoeff(x,1,1)): gminusone; /* gZ << 0, gZ = 1 mod x */
  if (nba == 1)
  {
    y[2] = (long)mkvec(gZ);
    y[3] = (long)gscalmat(gone,1); return y;
  }
...
zlog_add_sign(GEN y0, GEN sgn, GEN lists)
{
  ...
  for (i = lg(s)-1; i > 0; i--) *--y = mpodd((GEN)s[i])? one: zero;
}
...
famat_zlog(GEN nf, GEN g, GEN e, GEN sgn, GEN bid)
{
  ...
  for (i=1; i < l; i++)
  {
    ...
    y = zlog_pk(nf, x, y, pr, prk, (GEN)lists[i], &sgn);
  }
...
compute_gen(GEN nf, GEN u1, GEN gen, GEN bid)
{
  ...
  GEN L = cgetg(c,t_VEC);
  for (i=1; i<c; i++)
    L[i] = (long)famat_to_nf_modidele(nf, gen, (GEN)u1[i], bid);
  return L;
}
static void
add_clgp(GEN nf, GEN u1, GEN cyc, GEN gen, GEN bid)
{
  ...
  c[1] = (long)detcyc(cyc);
  c[2] = (long)cyc;
  if (u1) c[3] = (long)(u1 == gone? gen: compute_gen(nf, u1, gen, bid));
}
...
zidealstarinitall(GEN nf, GEN ideal,long add_gen)
{
  ...
  if (nbp)
  {
    ...
    for (i=1; i<=nbp; i++)
    {
      ...
      for (j=1; j < lg(L2); j++)
      {
        GEN L = (GEN)L2[j], F = (GEN)L[1], G = (GEN)L[3];
        for (k=1; k<lg(G); k++)
        { /* log(g^f) mod idele */
          GEN g = (GEN)G[k], f = (GEN)F[k], a = element_powmodideal(nf,g,f,x);
...
zideallog_sgn(GEN nf, GEN x, GEN sgn, GEN bid)
{
  ...
  switch(typ(x))
  {
    ...
    case t_MAT:
      ...
      y = famat_zlog(nf, (GEN)x[1], (GEN)x[2], sgn, bid);
      goto END;
...
zidealstarinitjoin(GEN nf, GEN bid1, GEN bid2)
{
  ...
  gen = (lg(clgp1)>3 && lg(clgp2)>3)? gone: NULL;
  ...
  U1 = (GEN)bid1[5]; cyc1 = (GEN)clgp1[2]; l1 = lg(cyc1);
  U2 = (GEN)bid2[5]; cyc2 = (GEN)clgp2[2]; l2 = lg(cyc2);
  nbgen = l1+l2-2;
  ...
  if (gen)
  {
    GEN a, b, uv = idealaddtoone(nf,(GEN)f1[1],(GEN)f2[1]);
    a = makeprimetoidealvec(nf,x,uv, (GEN)clgp1[3]);
    lswap(uv[1], uv[2]);
    b = makeprimetoidealvec(nf,x,uv, (GEN)clgp2[3]);
    gen = concatsp(a, b);
    nbp = lg((GEN)fa[1])-1;
  }
...
zidealstarinitjoinarch(GEN nf, GEN bid1, GEN arch, long add_gen)
{
  ...
  GEN module, liste, cyc, y, u1 = NULL, x, sarch, gen = add_gen? gone: NULL;
...
ideallistzstarall(GEN bnf,long bound,long flag)
{
  ...
  GEN nf,p,z,z2,p1,p2,fa,ideal,lu,lu2, U, sgnU, embunit;
  ...
  for (p[2] = 0; p[2] <= bound; )
  {
    ...
    {
      GEN pr = (GEN)fa[j]; 
      long f = itos((GEN)pr[4]), q = itos_or_0(gpowgs(p,f));
      ...
      for (l=2; ;l++)
      {
        ...
        for (i = q; i <= bound; i += q)
        {
          ...
          for (k = 1; k < lp1; k++)
            if (big_id)
              p2[k] = (long)zidealstarinitjoin(nf, (GEN)p1[k], ideal);
            else
