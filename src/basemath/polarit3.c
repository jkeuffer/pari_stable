...
FpV_roots_to_pol(GEN V, GEN p, long v)
{
  ...
  for(i=1;i<lg(V);i++)
    g[i] = (long)deg1pol_i(gen_1,modii(negi((GEN)V[i]),p),v);
...
FpXX_red(GEN z, GEN p)
{
  ...
  for(i=2;i<lg(res);i++)
    ...
    else
    {
      ...
      if (lg(res[i])<=3)
      {
        if (lg(res[i])==2) {avma=av;res[i]= (long)gen_0;}
...
FqX_Fq_mul(GEN P, GEN U, GEN T, GEN p)
{
  ...
  for(i=2; i<lP; i++) res[i] = (long)Fq_mul(U,(GEN)P[i], T,p);
  return FpXQX_renormalize(res,lg(res));
}
...
FpX_factorff_irred(GEN P, GEN Q, GEN l)
{
  ...
  else
  {
    ...
    res = cgetg(d+1,t_COL);
    for(i=1;i<=d;i++)
      res[i]=(long)RgM_to_RgXX((GEN)V[i],vp,vq);
  }
...
ZX_init_CRT(GEN Hp, ulong p, long v)
{
  ...
  for (i=2; i<l; i++)
    H[i] = lstoi(Fl_center(Hp[i], p, lim));
  return H;
}
...
ZM_init_CRT(GEN Hp, ulong p)
{
  ...
  for (j=1; j<l; j++)
  {
    ...
    c = cgetg(m, t_COL);
    H[j] = (long)c;
    for (i=1; i<l; i++) c[i] = lstoi(Fl_center(cp[i],p, lim));
  }   
  return H;
}
...
FpY_FpXY_resultant(GEN a, GEN b0, GEN p)
{
  ...
  * P_{-n}(-X), where P_i is Lagrange polynomial: P_i(j) = delta_{i,j} */
  ...
  if (i == dres)
  {
    x[++i] = (long)gen_0;        y[i] = (long)FpX_eval_resultant(a,b, (GEN)x[i], p,la);
  }
  x = FpV_polint(x,y, p);
...
ZX_caract_sqf(GEN A, GEN B, long *lambda, long v)
{
  ...
  switch (typ(B))
  {
    case t_POL: dB = degpol(B); if (dB > 0) break;
      B = dB? (GEN)B[2]: gen_0; /* fall through */
    default:
...
QXQ_inv(GEN A0, GEN B0)
{
  ...
  if (typ(B0) != t_POL) err(notpoler,"QXQ_inv");
  if (typ(A0) != t_POL)
  {
    if (is_scalar_t(typ(A0))) return ginv(A0);
    err(notpoler,"QXQ_inv");
  }
  ...
  for(;;)
  {
    ...
    if (low_stack(avlim, stack_lim(av,1)))
    {
      if (DEBUGMEM>1) err(warnmem,"QXQ_inv");
