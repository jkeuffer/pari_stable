...
galoisconj2pol(GEN x,long nbmax, long prec)
{
  ...
  if (gisirreducible(x) == gzero) err(redpoler,"galoisconj2pol");
  ...
  v=varn(x); y=cgetg(nbmax+1,t_VEC); y[1]=(long)polx[v];
  for (i=2; i<=n && nbauto<nbmax; i++)
  {
    w[n+1]=polr[i]; p1 = lindep2(w, prec);
    if (signe(p1[n+1]))
    {
      setlg(p1, n+1);
...
galoisconj2(GEN nf,long nbmax,long prec)
{
  ...
  y=cgetg(nbmax+1,t_VEC); y[1]=(long)polx[varn(x)];
  for (i=2; i<=n && nbauto<nbmax ; i++)
  {
    w[n+1]=polr[i]; p1 = lindep2(w, prec);
    if (signe(p1[n+1]))
...
{
   nf=checknf(nf);
   return galoisconj2(nf,degree((GEN)nf[1]),prec);
}
struct galois_lift
{
  GEN T;
...
initlift(GEN T, GEN den, GEN p, GEN borne, GEN L, struct galois_lift * gl)
{
  ...
  gl->L = L;
  ltop = avma;
  gl->e = itos(gmax(gdeux, gceil(gdiv(glog(gmul2n(borne, 1), DEFAULTPREC), glog(p, DEFAULTPREC)))));
  lbot = avma;
  gl->Q = gpowgs(p, gl->e);
...
GEN 
bezout_lift_fact(GEN pola, GEN aprov, GEN p, GEN pev, long e)
{
  long ev;
...
/*
 * test tous les lifts possibles a l'aide de frobeniuslift, pour le nombre
 * premier p, le sous-groupe sg et l'exposant e. F factorisation p-adique de
 * T. retourne gzero ou une permutation de L
 */
long 
...
long numberofconjugates(GEN T, long pdepart)
{
  ...
  for (p = 0, primepointer = diffptr; nbtest < nbmax && card > 1 ;)
  {
    ...
    if (DEBUGLEVEL >= 6)
	fprintferr("CardAutomorphism:Nbtest=%d,card=%d,p=%d\n", nbtest,card, p);
    nbtest++;
    avma=ltop2;
  }
  if (DEBUGLEVEL >= 2)
    fprintferr("CardAutomorphism:card=%d,p=%d\n",card, p);
  avma=ltop;
  return card;
}
GEN
galoisconj0(GEN nf,long flag,GEN d,long prec)
{
  ...
  switch(flag)
  {
    ...
    else 
    {
      ...
      {
	if ( typ(nf) == t_POL )
	  return galoisconj2(nf,prec,card);
	else
    ...
    case 2: return galoisconj2(nf,degree(T),prec);
    case 4: 
      G=galoisconj4(nf,d);
      if ( typ(G) != t_INT )
	return G;
