...
vandermondeinversemod(GEN L, GEN T, GEN den, GEN mod)
{
  ...
  for (i = 1; i < n; i++)
  {
    ...
    P = FpX_Fp_mul(FpX_div(T, deg1pol_i(gen_1,negi((GEN) L[i]),x),mod), z, mod); 
    M[i] = lgetg(n, t_COL);
    for (j = 1; j < n; j++)
  ...
  for (i = 1; i < lg(isom); i++)
  {
    ...
    if (DEBUGLEVEL >= 6)
      fprintferr("S4GaloisConj:Computing isomorphisms %d:%Z\n", i,
		 (GEN) isom[i]);
    isominv[i] = (long) FpXQ_ffisom_inv((GEN) isom[i], (GEN) Tmod[i],p);
  }
  for (i = 1; i < lg(isom); i++)
    for (j = 1; j < lg(isom); j++)
	  ...
	  const struct galois_analysis *ga)
{
  ...
  Pg=cgetg(lg(O),t_VECSMALL);
  {
    ...
    Tp = FpX_red(T,ip);
    Sp = sympol_aut_evalmod((GEN) V[1],deg,sigma,Tp,ip);
