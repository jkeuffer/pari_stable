...
add_ser_scal(GEN y, GEN x, long vy)
{
  ...
  if (l > 0)
  {
    ...
    for (i=3; i<=l+1; i++) z[i] = (long)gen_0;
...
{ return mul_rfrac((GEN)x[1],(GEN)x[2], gen_1,y); }
static GEN
...
div_scal_ser(GEN x, GEN y) { /* TODO: improve */
  ...
  z[2] = (long)x; for (i=3; i<ly; i++) z[i] = (long)gen_0;
...
gdiv(GEN x, GEN y)
{
  ...
  if (tx==t_INT && is_const_t(ty)) /* optimized for speed */
  {
    ...
    if (!s) {
      ...
      z = cgetg(3,t_INTMOD); icopyifstack(y[1],z[1]); z[2] = (long)gen_0;
      return z;
    }
    if (is_pm1(x)) {
