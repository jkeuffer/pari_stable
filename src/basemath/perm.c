...
/*
 * Multiply (compose) two permutations, putting the result in the
 * second one.
 */
static GEN
...
GEN group_subgroups(GEN G) 
{
  ...
  {
    p1 = concat(p1, sg3);
    /*We need to perform some fixup to avoid the D4 subgroups of 
     *S4 to be in non supersolvable format.*/
    if (n==5)
      for(j = 3; j <= 5; j++)
        if (!perm_commute(gmael3(p1,j,1,1),gmael3(p1,j,1,3)))
        {
          if (perm_commute(gmael3(p1,j,1,2),gmael3(p1,j,1,3)))
          {
            long z = mael3(p1,j,1,1); 
            mael3(p1,j,1,1) = mael3(p1,j,1,2); 
            mael3(p1,j,1,2) = z;
          }
          else
            perm_mul_inplace(gmael3(p1,j,1,2), gmael3(p1,j,1,1));
        }
  }
  return gerepileupto(ltop,p1);
}
