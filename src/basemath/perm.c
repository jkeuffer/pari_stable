...
{
  pari_sp av = avma;
  return gerepilecopy(av, vecperm_orbits_i(_vec(v), lg(v)-1));
}
...
perm_order(GEN v)
{
  ...
  GEN c;
  long i, d;
  c = vecperm_orbits_i(_vec(v), lg(v)-1);
...
trivialsubgroups(void)
{
  ...
  p2[1] = (long)_vec2(cgetg(1,t_VEC), cgetg(1,t_VECSMALL));
  return p2;
}
...
cyclicgroup(GEN g, long s)
{
  ...
  p2[1] = (long)_vec( vecsmall_copy(g) );
  p2[2] = (long)_vecsmall(s); return p2;
}
