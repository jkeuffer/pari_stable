...
keri(GEN x)
{
  ...
  for (j=k=1; j<=r; j++,k++)
  {
    ...
    for (i=k+1; i<=n; i++) p[i]= (long)gen_0;
  }
...
ker0(GEN x, GEN a)
{
  ...
  for (j=k=1; j<=r; j++,k++)
  {
	...
	p[i] = (long)gen_0;
    p[k]= (long)gen_1; for (i=k+1; i<=n; i++) p[i]= (long)gen_0;
  }
...
GEN
vec_ei(long n, long i) { GEN e = zerocol(n); e[i] = (long)gen_1; return e; }
GEN
  ...
  for (k=1; k<=n; k++)
  {
    ...
    if (j>m)
    {
      if (deplin) {
        ...
        c[k] = (long)gen_1; for (i=k+1; i<=n; i++) c[i] = (long)gen_0;
