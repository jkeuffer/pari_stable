...
matid_F2m(long n)
{
  ...
  long i;
  if (n < 0) pari_err(talker,"negative size in matid_F2m");
...
eigen(GEN x, long prec)
{
  ...
  for(;;)
  {
    ...
    if (l == 1 || ly + (l-1) > n)
      pari_err(syntaxer, "missing eigenspace. Compute the matrix to higher accuracy, then restart eigen at the current precision",NULL,NULL);
