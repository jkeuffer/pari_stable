...
err_cat(GEN x, GEN y)
{
  err(talker,"impossible concatenation: %s %Z . %s %Z",
...
deplin(GEN x0)
{
  ...
  if (k == 1) { avma = av; return gscalcol_i(gone,nc); }
  y = cgetg(nc+1,t_COL);
  y[1] = (long)ck[ l[1] ];
  for (q=d[1],j=2; j<k; j++)
  {
...
gauss_pivot_ker(GEN x0, GEN a, GEN *dd, long *rr)
{
  ...
  for (k=1; k<=n; k++)
  {
    ...
    { /* pivot for column k on row j */
      c[j]=k; d[k]=j; p = gdiv(gminusone,gcoeff(x,j,k));
      coeff(x,j,k) = (long)gminusone;
      /* x[j,] /= - x[j,k] */
      for (i=k+1; i<=n; i++)
...
gauss_pivot(GEN x0, GEN *dd, long *rr)
{
  ...
  for (k=1; k<=n; k++)
  {
    ...
    {
      c[j]=k; d[d0[k]]=j; p = gdiv(gminusone, gcoeff(x,j,k));
      for (i=k+1; i<=n; i++)
	coeff(x,j,i) = lmul(p,gcoeff(x,j,i));
...
ker0(GEN x, GEN a)
{
  ...
  for (j=k=1; j<=r; j++,k++)
  {
	...
	p[i] = zero;
    p[k]=one; for (i=k+1; i<=n; i++) p[i]=zero;
  }
...
GEN
vec_ei(long n, long i) { GEN e = zerocol(n); e[i] = one; return e; }
GEN
...
Flm_indexrank(GEN x, ulong p)
{
  ...
  res=cgetg(3,t_VEC);
  p1 = cgetg(r+1,t_VECSMALL); res[1] = (long)p1;
  p2 = cgetg(r+1,t_VECSMALL); res[2] = (long)p2;
  if (d)
  {
    for (i=0,j=1; j<=n; j++)
...
Flm_mul(GEN x, GEN y, ulong p)
{
  ...
  if (lx==1)
  {
    for (i=1; i<ly; i++) z[i]=lgetg(1,t_VECSMALL);
    return z;
  }
  l=lg(x[1]);
  for (j=1; j<ly; j++)
  ...
  for (k=1; k<=n; k++)
  {
    ...
    if (j>m)
    {
      if (deplin) {
        ...
        c[k] = one; for (i=k+1; i<=n; i++) c[i] = zero;
...
Flm_gauss_pivot(GEN x, ulong p, long *rr)
{
  ...
  for (k=1; k<=n; k++)
  {
    for (j=1; j<=m; j++)
      if (!c[j])
      {
        coeff(x,j,k) = ((ulong) coeff(x,j,k)) % p;
        if (coeff(x,j,k)) break;
      }
    if (j>m) { r++; d[k]=0; }
    else
    {
...
Flxq_gerepile_gauss_ker(GEN x, GEN T, ulong p, long k, long t, pari_sp av)
{
  ...
  for (u=t+1; u<=m; u++)
    if (isonstack(coeff(x,u,k))) 
      coeff(x,u,k) = (long) Flx_rem(gcoeff(x,u,k),T,p);
  for (i=k+1; i<=n; i++)
    for (u=1; u<=m; u++)
      if (isonstack(coeff(x,u,i))) 
        coeff(x,u,i) = (long) Flx_rem(gcoeff(x,u,i),T,p);
...
FlxqM_ker_i(GEN x, GEN T, ulong p, long deplin)
{
  ...
  if (typ(x)!=t_MAT) err(typeer,"FlxqM_ker");
  n=lg(x)-1; if (!n) return cgetg(1,t_MAT);
  vs = mael3(x,1,1,1);
  ...
  for (k=1; k<=n; k++)
  {
    for (j=1; j<=m; j++)
      if (!c[j])
      {
        coeff(x,j,k) = (long) Flx_rem(gcoeff(x,j,k), T, p);
        if (lgpol(coeff(x,j,k))) break;
      }
    if (j>m)
    {
      if (deplin) {
        c = cgetg(n+1, t_COL);
        for (i=1; i<k; i++) c[i] = (long)Flx_rem(gcoeff(x,d[i],k), T, p);
        c[k] = (long) Fl_to_Flx(1,vs); 
        for (i=k+1; i<=n; i++) c[i] = (long) zero_Flx(vs);
        return gerepileupto(av0, c); 
      }
      r++; d[k]=0;
      for(j=1; j<k; j++)
    ...
    else
    {
      ...
      for (i=k+1; i<=n; i++)
	coeff(x,j,i) = (long) Flxq_mul(piv,gcoeff(x,j,i), T, p);
      for (t=1; t<=m; t++)
      {
        if (t==j) continue;
        ...
        coeff(x,t,k) = (long) zero_Flx(vs);
        for (i=k+1; i<=n; i++)
          coeff(x,t,i) = (long)Flx_add(gcoeff(x,t,i),
  ...
  for (j=k=1; j<=r; j++,k++)
  {
    ...
    for (i=1; i<k; i++)
      ...
      {
	GEN p1=gcoeff(x,d[i],k);
	C[i] = (long) Flx_rem(p1, T, p); gunclone(p1);
      }
      else
	C[i] = (long) zero_Flx(vs);
    C[k] = (long) Fl_to_Flx(1,vs);
    for (i=k+1; i<=n; i++) C[i] = (long) zero_Flx(vs);
  }
  return gerepile(av0,tetpil,y);
...
det_simple_gauss(GEN a, int inexact)
{
  ...
  av=avma; s=1; x=gone; a=dummycopy(a);
  for (i=1; i<nbco; i++)
  {
    p=gcoeff(a,i,i); k=i;
    if (inexact)
...
det(GEN a)
{
  ...
  a = dummycopy(a); s = 1;
  for (pprec=gone,i=1; i<nbco; i++,pprec=p)
  {
    GEN *ci, *ck, m, p1;
