...
loop(decomp_t *S, long nv, long Ea, long Fa, GEN ns)
{
  ...
  { /* beta tends to a factor of chi */
    ...
    { /* Look for a root delt of nug in Fp[phi] such that vp(gamma - delta) > 0
      ...
      GEN eta, chie, nue, W = (GEN)w[i]; /* monic linear polynomial */
      delt = gneg_i( ch_var((GEN)W[2], v) );
      eta  = gsub(gamm, delt);	
      if (fm)
      {
  ...
  for(cmpt=1; ; cmpt++)
  {
   ...
   /* compute MW: W_i*W_j = sum MW_k,(i,j) W_k */
    ...
    for (i=1; i<=n; i++)
      for (j=i; j<=n; j++)
      {
        GEN z = RgXQX_rem(gmul((GEN)Waa[i],(GEN)Waa[j]), pol, nfT);
        long tz = typ(z);
...
get_d(GEN nf, GEN pol, GEN A)
{
  ...
  for (j=1; j<=n; j++)
    for (i=j; i<=n; i++)
    {
      GEN c = RgXQX_mul((GEN)W[i],(GEN)W[j], nfT);
