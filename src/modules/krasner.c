...
static GEN
GetRamifiedPol(GEN p, long e, long f, long j, long v, long flag)
{
  long r;
  GEN pols;
  KRASNER_t data;
...
/* return a minimal list of polynomials generating all the extensions of
   Q_p of given degree m and discriminant p^d. If d < 0, return all 
   the possible extensions of this degree */
...
{
  long j, nd, av = avma, v = fetch_user_var("y");
  GEN rep = zerovec(0), dec, pls;

  if (flag == 2) rep = gen_0;

  if (d < 0)
  {
    /* maximal possible discriminant m*(1+log(m)/log(p)) */
    long ds = itos(ground(gmulsg(m, gaddsg(1, gdiv(glog(stoi(m), DEFAULTPREC), 
						   glog(p, DEFAULTPREC))))));
    for (j = 0; j <= ds; j++)
    {
      GEN p1 = padicfields(p, m, j, flag);
      if (flag == 2) rep = addii(rep, p1);
      else
	if (lg(p1) > 1) rep = concat(rep, p1);
    }
  ...
  if( DEBUGLEVEL>0)
    fprintferr("Looking for extension(s) of degree %ld and discriminant %Ps^%ld of Q_%Ps\n", m, p, d, p); 
  
  /* first, we compute the possible decomposition */
  dec = PossibleDecomposition(p, m, d);
  nd = lg(dec) - 1;
  
  if (DEBUGLEVEL>1)
    fprintferr("  Possible decomposition(s) [f, e, j] = %Ps\n", dec);
  ...
  for (j = 1; j<= nd; j++)
  {
    ...
    if (DEBUGLEVEL>1)
      fprintferr("  Computing extensions with decomposition [f, e, j] = %Ps\n", p1);
    pls = GetRamifiedPol(p, p1[2], p1[1], p1[3], v, flag);
    if (flag == 2) 
      rep = addii(rep, pls);
    else
      rep = concat(rep, pls);
