...
LogHeight(GEN x, long prec)
{
  int i, n = lg(x)-1;
  GEN LH = gen_1;
  for (i=1; i<=n; i++) LH = gmul(LH, gmax(gen_1, gabs((GEN)x[i], prec)));
...
Conj_LH(GEN v, GEN *H, GEN r, long prec)
{
  ...
  for (j = 1; j < l; j++)
  {
    if (! (e = get_emb((GEN)v[j], r, prec)) ) return NULL; /* FAIL */
    M[j] = (long)e;
...
SmallSols(GEN S, int Bx, GEN poly, GEN rhs, GEN ro)
{
  ...
  for (x = -Bx; x <= Bx; x++)
  {
    ...
    {
      P[j] = (long)mulii(X, (GEN)poly[j]);
      X = mulis(X, x); 
    }
    P[2] = (long)subii((GEN)P[2], rhs); 
    r = nfrootsQ(P); 
...
LargeSols(GEN tnf, GEN rhs, GEN ne, GEN *pro, GEN *pS)
{
  ...
  *pS = cgetg(1, t_VEC);
  ...
  Vect = cgetg(r+1,t_COL); for (i=1; i<=r; i++) Vect[i]= (long)gen_1;
